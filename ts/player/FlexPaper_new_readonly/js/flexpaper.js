...
window.FlexPaperViewerEmbedding = window.$f = function(id, args) {
    ...
    _jsDirectory 		= (config.jsDirectory!=null?config.jsDirectory:BaseURL+"player/FlexPaper_new_readonly/js/");
    _cssDirectory 		= (config.cssDirectory!=null?config.cssDirectory:BaseURL+"player/FlexPaper_new_readonly/css/");
...
(function() {
	...
	var f = extend(window.flashembed, {
		...
		getHTML: function(opts, conf) {
			...
			if (conf) {
				for (var k in conf) {
                    if (conf[k] && k!='Toolbar' && k!='BottomToolbar') {
						var val = conf[k];

...
class Adaptive_payments extends MX_Controller 
{
 ...
 function Pay_parallel() {
		...
		$Receiver = array(
						'Amount' => '2', 											// Required.  Amount to be paid to the receiver.
						'Email' => 'per@toadsquare.com', 												// Receiver's email address. 127 char max.
						'InvoiceID' => 'PER', 											// The invoice number for the payment.  127 char max.
						'PaymentType' => 'SERVICE', 										// Transaction type.  Values are:  GOODS, SERVICE, PERSONAL, CASHADVANCE, DIGITALGOODS
						'PaymentSubType' => '', 									// The transaction subtype for the payment.
						'Phone' => array('CountryCode' => '', 'PhoneNumber' => '', 'Extension' => ''), // Receiver's phone number.   Numbers only.
						'Primary' => 'false'												// Whether this receiver is the primary receiver.  Values are boolean:  TRUE, FALSE
		...
		$Receiver = array(
						'Amount' => '3', 											// Required.  Amount to be paid to the receiver.
	...
	function Payment_details($trackingId)
	{
		//$payKey ='AP-1X559609RX561452B';
		...
		if($PayPalResult)
		$PayResult = json_decode($PayPalResult);		
				
		if(!$this->paypal_adaptive->APICallSuccessful($PayResult->responseEnvelope->ack))
		{
			$insert['trackingId'] = $trackingId;							
	        $this->db->insert('PaypalTrackingLog', $insert);			   
	        return redirect(base_url(lang().'/payment/process/transactionerror'));
		}
		else
		{ 
			 return $this->insertData($PayPalResult);
		}	
		
	}
	
	
 function insertData($PayPalResult,$orderId=1) {
		
		$sendResult  = $PayPalResult;
		$PayPalResult = json_decode($PayPalResult);
		//echo "<pre/>";
		//print_r($PayPalResult);die;
				$timestamp = $PayPalResult->responseEnvelope->timestamp;
				$ack = $PayPalResult->responseEnvelope->ack;
				$correlationId = $PayPalResult->responseEnvelope->correlationId;
				$build = $PayPalResult->responseEnvelope->build;
		
			foreach ($PayPalResult->paymentInfoList->paymentInfo as $res=>$item){
				
			//	print_r($PayPalResult->paymentInfoList->paymentInfo);die;
			//	foreach ($item->receiver as $receiver){					
					
							$recAmount =  $item->receiver->amount;						
							$recEmail = $item->receiver->email;					
							$recPrimary = $item->receiver->primary;							
							$recInvoiceId =  $item->receiver->invoiceId;							
							$recPaymentType =  $item->receiver->paymentType;
							$recAccountId = $item->receiver->accountId;	 						
							
							$insert['ordId'] =$orderId ;
							$insert['transactionId'] = $PayPalResult->paymentInfoList->paymentInfo[0]->transactionId ;
							$insert['transactionStatus'] =$PayPalResult->paymentInfoList->paymentInfo[0]->transactionStatus ;
							$insert['receiver_amount'] =$recAmount;
							$insert['receiver_email'] =$recEmail;
							$insert['receiver_primary'] =$recPrimary;
							$insert['invoiceId'] =$recInvoiceId;
							$insert['receiver_paymentType'] =$recPaymentType ;							
							$insert['receiver_accountId'] = $recAccountId ;							
							$insert['pendingRefund'] = $item->pendingRefund ;
							$insert['senderTransactionId'] = $item->senderTransactionId ;
							$insert['senderTransactionStatus'] = $item->senderTransactionStatus ;
							$insert['pendingReason'] =$item->pendingReason ;
							$insert['senderEmail'] = $PayPalResult->senderEmail ;
							$insert['status'] =$PayPalResult->status ;
							$insert['payKey'] = $PayPalResult->payKey ;
							$insert['actionType'] =$PayPalResult->actionType ;
							$insert['feesPayer'] =$PayPalResult->feesPayer ;
							$insert['sender_accountId'] = $PayPalResult->sender->accountId;
							$insert['timestamp'] =$timestamp;
							$insert['ack'] =$ack;
							$insert['correlationId'] =$correlationId;
							$insert['build'] =$build;
							$insert['trackingId'] = $PayPalResult->trackingId;		
					
	                 $flag= $this->db->insert('PaypalTransactionLog', $insert);				
				}	
	...
	function testpay($payKey='')
	{
		$payKey ='AP-5C028635TS3049223';
		...
		echo "<pre>";
		print_r(json_decode($PayPalResult));die;

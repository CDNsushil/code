...
class Adaptive_payments_test extends MX_Controller
{
...
print_r($config);
	...
	function Pay()
	{
		...
		$PayRequestFields = array(
								...
								'TrackingID' => '7862'									// Unique ID that you specify to track the payment.  127 char max.
								);
	...
	function test(){
		...
		$PayRequestFields = array(
								...
								'ReverseAllParallelPaymentsOnError' => '', 			// Whether to reverse paralel payments if an error occurs with a payment.  Values are:  TRUE, FALSE
								'SenderEmail' => 'per@toadsquare.com', 								// Sender's email address.  127 char max.
								'TrackingID' => ''
...
// Whether this receiver is the primary receiver.  Values are boolean:  TRUE, FALSE
		...
		$Receiver = array(
						'Amount' => '10', 											// Required.  Amount to be paid to the receiver.
						'Email' => 'buy7_1349083520_per@cdnsol.com ', 												// Receiver's email address. 127 char max.
...
print_r($PayPalResult)	;die;	
	...
	function Pay_chained_demo()
	{
		...
		$AccountIdentifierFields = array(
										'Email' => 'test1@test.com', 								// Sender's email address.  127 char max.
										'Phone' => array('CountryCode' => '', 'PhoneNumber' => '', 'Extension' => '')								// Sender's phone number.  Numbers only.
										);
	...
	function Payment_details()
	{
		...
		$PaymentDetailsFields = array(
									'PayKey' => 'AP-5NM888137N471372G', 							// The pay key that identifies the payment for which you want to retrieve details.  
		...
		echo "<pre>";
		print_r(json_decode($PayPalResult));
		if(!$this->paypal_adaptive->APICallSuccessful($PayPalResult['Ack']))
		{
			$errors = array('Errors'=>$PayPalResult['Errors']);
			//$this->load->view('paypal_error',$errors);
	...
	function Get_verified_status()
	{
		...
		$GetVerifiedStatusFields = array(
										'EmailAddress' => 'per@toadsquare.com', 					// Required.  The email address of the PayPal account holder.
										...
										'MatchCriteria' => 'per@toadsquare.com'					// Required.  The criteria must be matched in addition to EmailAddress.  Currently, only NAME is supported.  Values:  NAME, NONE   To use NONE you have to be granted advanced permissions
										);

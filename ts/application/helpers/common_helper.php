...
if(! function_exists('getProjectImage')){
	function getProjectImage($entityId=0,$elementId=0,$projectId=0,$notificationProjectType='',$imageSize='_xxs'){
		...
		if(isset($entityId) && isset($elementId) && isset($projectId)){
			switch ($tableName)
			{
				case 'TDS_Posts':
					...
					if((file_exists($filePath)) && isset($getProjectDetails) && !empty($getProjectDetails)){
						$default_urgent_Image = $CI->config->item('defaultBlogImg_s');
						...
						$projectImage = getImage($smallImg,$CI->config->item('defaultBlogImg_s'));
					}
				...
				case 'TDS_Project':
					...
					$fileFName = 'projBaseImgPath';
					$elementFName = 'projId';
					$projectFName = '';
					$getProjectDetails = $CI->model_common->getProjectDetails($tableName,$elementId,$projectId,$fileFName,$elementFName,$projectFName);	
					$filePath = ROOTPATH.$getProjectDetails;
					if((file_exists($filePath)) && (!empty($getProjectDetails))){
						$default_urgent_Image = $CI->config->item($notificationProjectType.'Image_xs');
						$thumbImage = addThumbFolder(@$getProjectDetails,'_s');				
						$projectImage = getImage(@$thumbImage,$default_urgent_Image);
					}else{
						$getElementImageByType= getElementImageByType($elementId,$notificationProjectType);
						if($getElementImageByType){
							
							$default_urgent_Image = $CI->config->item($notificationProjectType.'Image_xs');
							$thumbImage = addThumbFolder(@$getElementImageByType,'_s');				
							$projectImage = getImage(@$thumbImage,$default_urgent_Image);
							
							if(is_array($getElementImageByType)){
								if($getElementImageByType['isExternal']=="t"){
									$projectImage = $getElementImageByType['filePath'];
								}
							}
							
						}else{
							$projectImage = getImage($smallImg,$CI->config->item($notificationProjectType.'Image_xs'));
						}
					}
				break;
				case 'TDS_FvElement':
					$getProjectImageData = $CI->model_common->getProjectFilePathDetails($elementId,$projectId);
					if(!empty($getProjectImageData)) {
						if(empty($getProjectImageData->imagePath)){
							$imageType = $CI->config->item('filmNvideoImage_xs');
							$fileName=isset($getProjectImageData->fileName)?trim($getProjectImageData->fileName):'';
							$filePath=isset($getProjectImageData->filePath)?trim($getProjectImageData->filePath):'';
							$thumbImage = getVideoThumbFolder(@$filePath.$fileName,'_xs');	
							$projectImage = getImage(@$thumbImage,$imageType);
						}else{
							$projectImage = checkMediaDetails($tableName,$elementId,$projectId,'filmNvideoImage_xs');
						}
					}else{
						$projectImage = checkMediaDetails($tableName,$elementId,$projectId,'filmNvideoImage_xs');	
					}
				...
				break;
				case 'TDS_MaElement':
					$projectImage = checkMediaDetails($tableName,$elementId,$projectId,'musicNaudioImage_xs');
				break;
				case 'TDS_PaElement':
					$projectImage = checkMediaDetails($tableName,$elementId,$projectId,'photographyNartImage_xs');
				break;
				case 'TDS_ReviewsElement':
					$projectImage = checkMediaDetails($tableName,$elementId,$projectId,'defaultReviewsImg_xs');
				break;
				case 'TDS_NewsElement':
					$projectImage = checkMediaDetails($tableName,$elementId,$projectId,'defaultNewsImg_xs');
				break;
				case 'TDS_WpElement':
					$projectImage = checkMediaDetails($tableName,$elementId,$projectId,'writingNpublishingImage_xs');
				break;
				case 'TDS_EmElement':
					$projectImage = checkMediaDetails($tableName,$elementId,$projectId,'educationMaterialImage_xs');
				break;
				...
				case 'TDS_Product':
					...
					if(isset($getProductProDetails) && !empty($getProductProDetails))
					{
						...
						if((isset($filePath)) && (file_exists($filePath))){
							...
							$projectImage=addThumbFolder($imgType,'_m');
				...
				case 'TDS_UserShowcase':
						...
						if($res){
							...
							if(isset($res['0']->tdsUid) && !empty($res['0']->tdsUid))
							{
								...
								$enterprise=$getUserShowcase['enterprise'];
								$userDefaultImage=($creative=='t')?$CI->config->item('defaultCreativeImg'):(($associatedProfessional=='t')?$CI->config->item('defaultAssProfImg'):(($enterprise=='t')?$CI->config->item('defaultEnterpriseImg'):''));
								if(!isset($userDefaultImage) || $userDefaultImage=='') $userDefaultImage=$CI->config->item('defaultMemberImg_m');
 ...
 */ 
 ...
 if ( ! function_exists('getProjectElementImage')){		
	function getProjectElementImage($projId=0,$entityId=0,$elementId=0,$sufix='_s'){
		$CI =&get_instance();
		$getElementData = $CI->model_common->getProjectElementImage($projId,$entityId,$elementId);
	...
	*/
		function getBlogIndustryList($section=''){
			...
			if(isset($section) && !empty($section) && $section='search') {
				...
				$data = getDataFromTabel('MasterIndustry','IndustryId,IndustryName',$whereField,'','IndustryOrder','ASC');
			}
			return $data;
		}
	}
   ...
   */ 
    ...
    */ 
     ...
     if(! function_exists('frentendUserId')){
        ...
        $moduleArray          =    array('mediafrontend');//defined module names
        $methodArray          =    array('showcase/index','showcase/developementpath','showcase/aboutme','showcase/videos','showcase/mycraves','showcase/cravingme','showcase/mypaylist');//defined module names
    ...
    */ 
     ...
     if(! function_exists('mediaFileCount')) {
        ...
        function mediaFileCount($entityId="0",$elementId="0",$indusrtyName="",$isShippable=0,$mediaFileType=0,$shippableStatus='') {

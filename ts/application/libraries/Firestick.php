...
class Firestick {
    ...
    function resolve_profiling() {
        ...
        if (rand(0, 99) < $this->log_frequency) {
			...
			// Initial Check Added By ANOOP for PostgreSQL
			if((!$result) && $this->CI->db->_error_number()=="")
			{
			    // Create new table for each new day
				$this->CI->db->query('CREATE TABLE ' . $table_name . ' AS SELECT * FROM ' . $this->db_name . '.firestick_log_template');
                // Insert again
                $this->CI->db->query($sql);
			}
        }

...
if(isset($data) && is_object($data) && count($data) > 0){
		...
		//----------make element default project image code start---------//
			$thumbImage = '';
			if((!empty($data->filePath) && !empty($data->fileName)) || (isset($data->NatureId) && $data->NatureId==1)) {
				...
				$getPojrectImage = getEventsPrimaryImage($data->EventId,'.eventId',$projectEntityId,'_b');
				if($getPojrectImage){
						$thumbImage = $getPojrectImage;
				}
			}
			$thumbFinalImg = getImage($thumbImage,$this->config->item('defaultEventImg_m'));
			...
			<a href="<?php echo $recordDetailUrl;?>"  target="_blank" >
			    <div class="cnt_box  bg_fff  ">
                  ...
                  <div class="p15">
                     ...
                     <p><?php echo $data->OneLineDescription;?> 
                     </p>
                  </div>

...
<div class="tds-button-top fr mr0 mt10 mr-15 showhidebookmark width_20 height_21">
	...
	<div id="showBookmark" class="dn">
		<ul>
			<?php
			// Build all top level categories
			foreach($bookmarks as $row)
			{
				echo '<li>'.anchor('help/posts/'.$this->topics_m->getCategoryId($row['bookmark_topic_id']).'/'.$row['bookmark_topic_id'].'', $row['bookmark_topic_title']).'</li>';
			}
			?>
		</ul>
	</div>

...
Class blog extends MX_Controller {
	...
	*/ 
	function blogForm($blogId=0)
	{
		...
		$imagePath = MEDIAUPLOADPATH.LoginUserDetails('username').'/project/blog/';

		$projectFolderPath = MEDIAUPLOADPATH.LoginUserDetails('username').'/project';
		$blogFolderPath = MEDIAUPLOADPATH.LoginUserDetails('username').'/project/blog';

		$cmd = 'chmod -R 777 '.MEDIAUPLOADPATH;
		exec($cmd);
		...
		if (!file_exists($blogFolderPath)) {
			if (!@mkdir($blogFolderPath, 777, true)) {
		...
		if(count($data) > 0){
			...
			$dataBlog['values'] = $blogResult[0];
			 ...
			 //Function to upload the image for work
				...
				if($this->input->post('submit') == 'Save'){
					if($this->form_validation->run())
					{
					 ...
					 if($_FILES['userfile']['name'] !=''){
						$Upload_File_Name = $this->do_upload_image('userfile',$imagePath);
						if((isset($Upload_File_Name['error'])) && ($Upload_File_Name['error'] !='')){
							set_global_messages($Upload_File_Name['error'], 'error');
							redirect('blog/blogForm');
						}else
						{						 
							$Upload_File_Name = $Upload_File_Name['upload_data']['file_name'];
						}
						}
					  $dataBlog['values'] = $this->model_blog->saveBlog($Upload_File_Name); 

					  /*
					   * 
					   * 
						  $catArray['categoryTitle'] = $this->input->post('categoryTitle');
						  $catArray['categoryTitleEdit'] = $this->input->post('categoryTitleEdit');
					   * 
					   * 
					   */
	 	...
	 	if(isset($Upload_File_Name) && $Upload_File_Name!='')
			$dataBlog['values']->blogImgPath =  $Upload_File_Name;

		//To load the languages form Language table

		$resultlangs = $this->model_blog->loadLanguage();
		
		foreach ($resultlangs as $resultlang)
		{
			$dataBlog['workLang'][$resultlang->langId] = $resultlang->Language_local;
		}
	...
	**/
	...
	/**
		*	All Posts related related with parent post
	**/
	...
	**/
	...
	{			
		...
		$catPostsData  = $this->model_blog->getCatPosts($catId,$sortPostBy);
	...
	*/
	...
	{		
		...
		$archivePostDetailResults = $this->model_blog->previewArchivesPost($archiveMonth,$archiveYear);
	...
	function postForm($postId=0)
	{
		...
		$config = array(
		  ...
		  {		
			...
			if($errorFlag == 1) set_global_messages('Please fill the blog setting first', 'error');
...
**/		
	...
	{		
		...
		if($postId==0) $dataPost = $this->model_blog->savePost($blogId,$postId);
		else	$dataPost = $this->model_blog->updatePost($blogId,$postId);
...
**/
	...
	{	
		...
		{
			 $blogId = 0;
			 set_global_messages('Please fill the blog setting first', 'error');
			 redirect('blog');
		}
		else
		{			
 ...
 **/
	function saveAppendCat($blogId=0)
	{
		...
		$catId = $this->model_blog->saveBlogCategory($catArray,$this->userId);
			...
			$editArr = array('title'=>$cat['label']['edit'],
			'class'=>"EditCategoryId",
			'id'=>"EditCategoryId", 
			'categoryTitle'=> $catArray['categoryTitle'],
			'categoryId'=>$catId, 
			'replaceId'=>$catId									
			);
			
		$categoryTitleToEdit = anchor('javascript://void(0);', $catArray['categoryTitle'],$editArr);
		//$categoryTitleToEdit = $catArray['categoryTitle'];
		$attr = array("onclick"=>"removeCategoryRow('$catId','0')");
		echo $newCatRecord = '<div id="removeID_'.$catId.'">
								<div class="row">	
									<div class="cell empty_label_wrapper"></div>
										<div class="cell frm_element_wrapper">
											<div class="cell width135px dot_orange">
												'.$categoryTitleToEdit.'						
											</div>
											<div class="cell" style="padding-left:5px;">
											<div class="tds-button-top">'.anchor("javascript://void(0);","<span><div class=\"projectDeleteIcon\"></div></span>",$attr).'</div><!--End tds-button-top-->							
										</div>

...
Class model_blog extends CI_Model {
	...
	*/	
	...
	function saveBlogCategory($catArray,$userId){
		...
		$insertcatArray = $catArray['categoryTitle'];		
	...
	*/		
	...
	{	
	  ...
	  *
	  */		
	function saveBlog($Upload_File_Name='')
	{	
		$blogRecord = $_POST;
	...
	**/
	...
	/**
		...
		* return array
	**/
	function previewArchivesPost($archiveMonth,$archiveYear)
	{	
		...
		$query .= ' AND "public"."TDS_Posts"."custId" = '.$this->userId;
	...
	/**
	  ...
	  * @return	object
	**/
	function getChildPosts($parentPostId)
	{
	...
	**/
	function previewPost($postId){
		...
		$this->db->select('tdsUid,firstName,lastName,image,postTitle,postDesc,dateCreated,blogId'); 	
		
		$this->db->where('postId', $postId);
		
		$this->db->join('UserProfile', 'UserProfile.tdsUid = Posts.custId');
	...
	**/
	...
	{		
		...
		$query = 'SELECT date_part(\'YEAR\',"dateCreated") as YearExtracted from "'.$table.'" where "custId" = \''.$this->userId.'\' group by YearExtracted 
	...
	**/
	...
	function showArchivesMonths($year)
	{
		...
		$query .=' AND "TDS_Posts"."postArchived" = \''.$postArchived.'\'';
		$query .=' AND "TDS_Posts"."postPublish" = \''.$postPublish.'\'';
		$query .=' AND "TDS_Posts"."custId" = \''.$this->userId.'\'';

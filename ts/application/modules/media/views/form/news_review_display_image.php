<?php if (!defined('BASEPATH')) exit('No direct script access allowed'); 
    ...
    $embedImage = array(
        'id'	=> 'embbededURL',
        'name'	=> 'embbededURL',
        'class'	=> 'fl width605 p14 min-height_25  bdr_adadad required',
        'rows' => 2,
        'cols' => 45,
        'value'=> $embeddUrl,
    );
   ...
   if(isset($displayImageType) && $displayImageType == 1) {
    ...
    //next page url 
    $nextUrl  =  $baseUrl.'/articleinfo/'.$projectId.'/'.$elementId ;
    // set upload form title
    if(!empty($imagePath)) {
        $uploadHeadTitle = $this->lang->line('editMediaReplaceImage');
        $data['skipPage'] = '/articleinfo/'.$projectId.'/'.$elementId ;
    } else {
...
<div class="TabbedPanelsContentGroup design_wrap width_665 m_auto ">
    <div class="TabbedPanelsContent width635 m_auto clearb TabbedPanelsContentVisible">
        <?php echo form_open($baseUrl.'/newsreviewfilepost',$displayImageForm); ?>
        ...
        <?php 
        // set back page
        $data['backPage'] = '/uploaddisplayimage/'.$projectId.'/'.$elementId;
        // set next form name
    ...
    $(document).ready(function() {
        ...
        $('.uploadFileAction').click(function() {
            ...
            if((fileInput != '' || embbededURL != '') && (fileErrorInput=='0')) {
                var fromData = $("#displayImageForm").serialize();
                var url = '<?php echo $baseUrl;?>'+'/newsreviewfilepost';

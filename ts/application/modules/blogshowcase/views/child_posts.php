...
<td valign="top" class="bg_blog_container">
	...
	if(!empty($parentPost->postTitle)) { 
				$parent_postYear = date('Y',strtotime($parentPost->dateCreated)); 	
					$parent_postDate = date('d',strtotime($parentPost->dateCreated)); 	
					$parent_postMon = date('M',strtotime($parentPost->dateCreated)); 	
									
					//if filepath is set for any of the post type it will show the respective image else show the no-image 
					if(!isset($parentPost->filePath) || @$parentPost->filePath!=''){
						 $imagePathForEvent = @$parentPost->filePath.'/'.@$parentPost->fileName;
						 $smallImg = addThumbFolder(@$imagePathForEvent,'_s');
					}
					else $smallImg = '';

					$finalSmallImg = getImage($smallImg,$this->config->item('defaultBlogImg_s'));
					$parent_postMediaSrc = '<img class="maxWidth165px maxHeight120px ma"  src="'.$finalSmallImg.'" />';						
					$parenthref=($currentMethod=='preview')?'/blogs/preview/'.$parentPost->custId.'/'.$parentPost->postId.'/frontPostDetail':'/'.$currentClass .'/'.$postlinkmethod.'/'.$parentPost->custId.'/'.$parentPost->postId;
					$parenthref=base_url(lang().$parenthref);	
					$LogSummarywhere=array(
					'entityId'=>$entityId,
					'elementId'=>@$parentPost->postId
					);
...
<div class="row p10" id="frontPostsInfo">
	...
	<div class="bolggradierbg bdr_ccc ptr " onclick="window.location.href='<?php echo $parenthref;?>'">
	<table width="100%" cellspacing="0" cellpadding="0">
		<tbody>
		<tr>
			<td class="blog_headingpatern font_arial clr_white font_weight font_size28 pl30 pr30 pb12 pt12 lH30 height_83">
				  <?php echo getSubString($parentPost->postTitle,50);?>
			</td>
		</tr>
		</tbody>
	</table>
	...
	<div class="gradientforstrock width_102 pt1 pb1 pr1 blogbtndropsheado fl mt5">		
		<div class=" bg_444">
			...
			<div class="clear"></div>
				<div class="fl font_arial font_size42 clr_bdbdbd lH30 ml6"><?php echo $parent_postDate; ?></div>
				<div class="fl font_arial font_weight clr_f1592a font_size17 width_40 ml8 lH15"><?php echo $parent_postMon.' '.$parent_postYear; ?> </div>
			<div class="clear"></div>
			<div class="seprator_12"></div>
	...
	<div class="clear"></div>
	<?php if(!empty($parentPost->categoryTitle)){?>
	<div class="seprator_5"></div>
	<div class="font_opensansSBold font_size12 text_alignR bdr_Bpost pb5 mr10"><?php echo $parentPost->categoryTitle;?></div>
	
	<?php } else echo '<div class="seprator_20"></div>';?>
	<div class="blogdescstyle minH66 pt5">
		<?php echo getSubString($parentPost->postOneLineDesc,130); ?>		
	</div>
	
	<div class="row blogreadmore width_100 tar ptr " onclick="window.location.href='<?php echo $parenthref;?>'">
		<?php echo anchor($parenthref,$this->lang->line('blogRead'),array('class'=>'orange'));?>									
	</div>	
...
<?php
	...
	<div class="bolggradierbg bdr_ccc ptr " onclick="window.location.href='<?php echo $href;?>'">
	<table width="100%" cellspacing="0" cellpadding="0" padding="0">
		<tbody>
		<tr>
			<td class="postres_patern font_arial font_weight font_size18 lineH24 pl30 pr30 pb15 pt10 height_60 clr_444">
				 <span class="lineH24 pb10"> <?php echo getSubString($row->postTitle,50);?></span>
				 <span class="clr_f1592a bdr_B949494 lineH14"><?php echo $userInfo['userFullName'];?></span>
			</td>
		</tr>
		</tbody>
	</table>
...
<?php
	...
	$this->load->view('blogs/advertise.php');

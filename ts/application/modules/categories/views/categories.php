...
<div class=" font_opensans font_size18 clr_white categordbtnline">Categories</div>
	...
	<ul>
	<li> <a href="<?php echo site_url().'forums'; ?>" class="fl dash_link_hover"> All Categories </a>  <div class="clear"></div> </li>	
   <?php
	$t=1;
	$countRow=0;
    foreach($categories as $row)
		{
			...
			$sub_categories = $this->categories_m->get_sub_categories($row['CategoryID'],'forums');
	if($row['parentID']==0)
		{	
			 ?>
   
    <li> <a href="<?php echo site_url().'forums/topics/'.$row['CategoryID']; ?>" class="fl <?php if(isset($selectedCategory)){ if($selectedCategory==$row['CategoryID']){ echo "selected_category_orange"; } } ?> dash_link_hover"> <?php echo $row['Name']; ?>&nbsp;[<?php echo  $categories_count; ?>] </a> 
    ...
    <?php  foreach($sub_categories as $sub_row)
			...
			<li> <a href="<?php echo site_url().'forums/topics/'.$sub_row['CategoryID']; ?>">- <?php echo $sub_row['Name']; ?>  [<?php echo $categories_count; ?>] </a> </li>
    <?php  } ?>
    </ul>
    </div>
    <?php } ?>
...
?>
<div class="text_alignR clr_white font_size18 font_opensan bdrB_6e6e6e pb4"><a href="<?PHP echo site_url();?>help" class="clr_white dash_link_hover">Tips Forum</a> <span class="aerrow_forum"></span></div>

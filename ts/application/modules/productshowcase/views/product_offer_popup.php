...
<div class="popup_gredient bg_f3f3f3" id="contactMeDiv">
  ...
  <?php echo form_hidden('productId', $productId); ?>
  
  <input type="hidden" name="recipientsId"  value="<?php echo $ownerId ;?>">
  ...
  <div class="cell shadow_wp strip_absolute left_240">
   ...
   <?php if(isset($isWorkProfile) && !empty($isWorkProfile)) { ?>	
	
    <div class="cell textarea_belowcheck mt5 ml15 dn">   
		   ...
		   <div class="Fright pr10 pt2"><?php echo $this->lang->line('attachWorkProfile');?></div>
 ...
 <div class="tds-button Fright mr9"> <button type="button" onclick="javascript:$(this).parent().trigger('close');" onmousedown="mousedown_tds_button(this)" onmouseup="mouseup_tds_button(this)" class="font_arial"><span><div class="Fleft"><?php echo $this->lang->line('close');?></div> <div class="icon-form-close-btn"></div> </span> </button>  </div>
      <div class="tds-button Fright"><button type="submit" onmousedown="mousedown_tds_button(this)" onmouseup="mouseup_tds_button(this)" class="font_arial"><span><div class="Fleft"><?php echo $this->lang->line('send');?></div> <div class="icon-send-btn"></div></span> </button> </div>
      <div class="clear"></div>
    </div>
	...
	$(document).ready(function(){	
		$("#productOffer").validate({
			  submitHandler: function() {
				var fromData=$("#productOffer").serialize();
				fromData = fromData+'&ajaxHit=1';
				$.post(baseUrl+language+'/tmail/offerProductWanted',fromData, function(data) {
				  if(data){
					  $('#contactMeDiv').html('<div class="p10"><?php echo $this->lang->line('meassageSent').' '.@$ownerName;?></div>');
				  }
				});
			 }
		});

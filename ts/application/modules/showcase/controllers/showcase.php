...
class showcase extends MX_Controller {
	...
	**/
	...
	{	
        ...
        //add full screen js
       // $this->head->add_js($this->config->item('template_new_js').'jquery.simplyscroll.js',NULL,'lastAdd');
    ...
    function cravedata($userId=0,$cravingme=false,$crvmePT=''){	
		...
		if((int)$userId > 0){
			...
			$this->data['cravingMeCount']=$this->model_showcase->cravingme($userId,$crvmePT,'','',0,0,true);
			$this->data['myCravesCount']=$this->model_showcase->craveList($userId,'','',0,0,true);
			
			if($cravingme){
                $countResult = $this->data['cravingMeCount'];
            }else{
                $countResult = $this->data['myCravesCount'];
            }
    ...
    public function mycraves($userId=0){
		...
		if(!((int)$userId >0)){
			...
			if($ajaxRequest){
                ...
                $this->load->view('showcase/showcase/craves',$this->data) ;
			}			   
			else{
				$this->data['ajaxRequest']=false;
                $this->data['topHeader'] = 'showcase/showcase/crave_header';
                $this->index($userId,$multilangId,'mycraves');
			}
		}
    }
    
    public function cravingme($userId=0){
        $userId=$userId>0?$userId:isloginUser();
		if(!((int)$userId >0)){
			redirect('home');
	    }else{
			...
			if($ajaxRequest){
				...
				$this->data['ajaxRequest']=false;
                $this->data['topHeader'] = 'showcase/showcase/crave_header';
                $this->index($userId,$multilangId,'cravingme');
			}
		}
    }
...
{	
    ...
    */
    ...
    public function updateShowcaseDetails() {
        ...
        if(!empty($postData)) {
            ...
            $this->session->set_userdata(array('firstName'=>$postData['firstName'],'lastName'=>$postData['lastName'],'countryId'=>$postData['countryId'],'countryName'=>getCountry($postData['countryId'])));
            // set mext page url
            $reditectUrl = base_url(lang().'/showcase/aboutsection');
            $type = 'success';
            $msg = $this->lang->line('UpdatedAboutSection');
    ...
    public function setmutilingualdata() {
        ...
        if(!empty($postData)) {
            ...
            if($langSection == 'otherlanguage') {
                // update user's showcase type id
                $showcaseData = array('showcaseId'=>$showcaseId,'tdsUid'=>$this->userId,'langId'=>$postData['langaugeId']);
    ...
    public function yourvideo($videoType=0) {
        ...
        // set media file data
        if(!empty($showcaseRes->introductoryFileId) || !empty($showcaseRes->interviewFileId)) {
            // get media file data 
    ...
    */
	...
	public function membershipcart() {
        ...
        $this->data['packagestageheading']  = $this->lang->line('manageExtraSpace');

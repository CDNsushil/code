...
if($loggedUserId > 0){
    ...
    if($isPublished=='t' && $moduleMethod!='preview') {  ?>
        ...
        <li>
            <a href="javascript:void(0)"  onclick="<?php echo $function;?>" title="<?php echo $toolTip ?>" > <?php echo $buttonTitle; ?>
             <i id="craveDiv<?php echo $entityId.''.$elementId?>" class="ab_share creav_me <?php echo $craved;?>"></i></a>
        </li>

    <?php  } else {  ?>

        <li ><a href="javascript:void(0)"> <?php echo $buttonTitle; ?> <i  id="craveDiv<?php echo $entityId.''.$elementId?>" class="ab_share creav_me <?php echo $craved;?>"></i></a></li>

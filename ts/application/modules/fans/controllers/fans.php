...
class fans extends MX_Controller {
	 ...
	 */
	 ...
	 function __construct() {
		//Load required Model, Library, language and Helper files
			...
			$this->dirCacheMedia = ROOTPATH.'cache/fans/';  
			$this->dirUploadMedia = 'media/'.LoginUserDetails('username').'/fans/'; 
   ...
   public function index() {
        ...
        $this->data['navigationHeading'] = $this->lang->line('fans');
        $this->landingpage('countMember');
	}
   ...
   /*
    public function photographyart() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('PaFans');
    ...
    public function musicaudio() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('MaFans');
    ...
    public function writingpublishing() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('WpFans');
    ...
    public function performingarts() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('ArtFans');
    ...
    public function craved() {
        ...
        $this->data['navigationHeading'] = $this->lang->line('FvFans');
    ...
    public function reviews() {
        ...
        $this->data['navigationHeading'] = $this->lang->line('reviewsFans');
    ...
    public function news() {
        ...
        $this->data['navigationHeading'] = $this->lang->line('newsFans');
        $this->data['comingsoon']= array('msg'=>$this->lang->line('ComingSoon'));
        $this->data['innerView'] = 'landingpage/one_column';
        $this->landingpage('countNews');
	}
    */
    public function landingpage($cd = 'countMember') {
		$this->data['projectType'] = 'fans';
        $this->getNavigation($cd);
        if($this->data[$cd]){
            if(!isset($this->data['innerView'])){
                $this->data['innerView']= 'landingpage/four_column';
                $this->data['bgColor']= 'bge8e8e8 fans_land';
    ...
    function getNavigation($cd = 'countCravedProject'){
        $where=array('sc.isPublished'=>'t', 'sc.fans'=>'t');
        $industryId = $this->config->item('fansSectionId');
		
        $this->data['countMember'] = $this->model_common->countMembers($where);
        ...
        /*$this->data['countMemberFv'] = $this->model_common->countMembers($where,true);
        if($cd == 'countMemberFv'){
        ...
        $this->data['navigations']['writingpublishing'] = array('title'=>$this->lang->line('WpFans'),'url'=>$url);

        $url = ($method == 'reviews')?'javascript:void(0);':base_url(lang().DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.'reviews');
        $this->data['navigations']['reviews'] = array('title'=>$this->lang->line('reviewsFans'),'url'=>$url);

        $url = ($method == 'news')?'javascript:void(0);':base_url(lang().DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.'news');
        $this->data['navigations']['news'] = array('title'=>$this->lang->line('newsFans'),'url'=>$url);*/
	...
	function landingPageProject($IndustryId=0){	
		...
		$defaultProfileImage = $this->config->item('defaultFansImg_m');
		...
		if($result && is_array($result) && count($result)>0){
			foreach($result as $data){
				$returnData[]=$this->load->view('landingpage/fans_listing',array('data'=>$data,'defaultProfileImage'=>$defaultProfileImage), true);
			}
		}
    ...
    function landingPageNews(){	
		...
		$industryKey = $this->config->item('FansTYPE');

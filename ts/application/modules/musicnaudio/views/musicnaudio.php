...
<?php 	
//Common Banner
	$topCravedArray = array('entityId'=>$projectEntityId,'projectType'=>'musicNaudio');
	$topCravedElementArray = array('entityId'=>25,'projectType'=>'musicNaudio');
	
	$topCravedData['defaultProfileImage'] = $this->config->item('musicNaudioImage_m');	
	...
	if(is_array($topCravedData['data']) && count($topCravedData['data'])>0)		
		$topCravedHtml = $this->load->view('common/top_craved_musicnaudio',$topCravedData,true);
	else
		$topCravedHtml = '';
		
			
	$bannerarray['imgarray']=array('banner_front_music_air-your-music-and-audio-files_HR.jpg','banner_front_music_rock-ears_HR.jpg');
	...
	$addUpcomingButton = 	 anchor($href,$this->lang->line('promote_upcoming'),array('onclick'=>$dashboardUpcomingUrl, 'onmousedown'=>'mousedown_promote_btn(this)', 'onmouseup'=>'mouseup_promote_btn(this)','class'=>'promote_btn dash_link_hover')); 
		...
		if((isset($news_array) && is_array($news_array) && count($news_array) > 0 ) || (isset($reviews_array) && is_array($reviews_array) && count($reviews_array) > 0 )){?>
         <div class="row">
			...
			$newsReview['main_div_class'] =  "cell bdr_skyBlue10 global_shadow bg_white ml40 width_420";
			$newsReview['first_li_class'] =  "wp_tab width_80 wp_tab_selected clr_naviBlue dash_link_hover";
			$newsReview['second_li_class'] =  "wp_tab width_100 clr_naviBlue dash_link_hover";
           ...
           $mlClass="ml20";
            ...
            <div class="cell">
              ...
              <div class="bdr_skyBlue10 global_shadow bg_white <?php echo $mlClass; ?> width_420">
                <div class="wp_project_heading ml11 clr_naviBlue"><?php echo $this->lang->line('LatestProjects');?></div>
                <div class="seprator_3"></div>
                <div id="slider3" class="slider MA_news_scroll_btn_box mt_minus30">
                  ...
                  <div class="viewport wp_project_scroll_container ml10">
                    <ul class="overview">
                       <?php
						...
						$recordDetailUrl = base_url(lang().'/mediafrontend/searchresult/'.$projDetail->tdsUid.'/'.$projDetail->projId.'/musicaudio');
						
						$projUserInfo = showCaseUserDetails($projDetail->tdsUid);
													
						//$thumbFinalImg = getImage(@$projDetail->projBaseImgPath,$this->config->item('musicNaudioImage_s'));
						$thumbImage = addThumbFolder(@$projDetail->projBaseImgPath,'_s');				
						$thumbFinalImg = getImage(@$thumbImage,$this->config->item('musicNaudioImage_s'));	
			  ...
			  ?>
              <div class="bdr_skyBlue10 global_shadow bg_white <?php echo $mlClass; ?> width_420">
                <div class="wp_project_heading ml11 clr_naviBlue"><?php echo $this->lang->line('LatestUpcomingProjects');?></div>
                <div class="seprator_3"></div>
                <div id="slider4" class="slider MA_news_scroll_btn_box mt_minus30">
				...
				<div class="position_relative mr10">
                  ...
                  <div class="viewport wp_project_scroll_container ml10">
                    <ul class="overview">
						...
						$upcomingDetailUrl = base_url(lang().'/upcomingfrontend/viewproject/'.$upcomingDetail['tdsUid'].'/'.$upcomingDetail['projId'].'/musicaudio');	

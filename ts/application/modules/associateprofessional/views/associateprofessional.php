<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	$topCravedArray = array('entityId'=>$showcaseEntityId,'projectType'=>'associatedprofessionals');
	
	//echo '<pre/ >';print_r($topCravedData);
	$topCravedData['defaultProfileImage'] = $this->config->item('defaultAssProfImg_m');
	$topCravedData['data'] = topCraved($topCravedArray);	
	
	if(is_array($topCravedData['data']) && count($topCravedData['data'])>0)	
		$topCravedHtml = $this->load->view('common/top_craved_associatedprofessional',$topCravedData,true);
	else
		$topCravedHtml = '';
		
	$bannerarray['imgarray']=array('banner_front_associated_01.jpg','banner_front_associated_02.jpg');
                        ...
                        <a target="_blank" href="<?php echo $memberDetailUrl;?>">
							<div class="wp_project_box <?php echo $mr8;?> height226px ptr">
								<div class="dash_link_hover">
									...
									<div class="ml13 font_size11 dash_link_hover"><b><?php echo substr_unicode($userFullName, 0, 15); //getSubString($userFullName_formated,15);?></b></div>
								</div>

...
class associateprofessional extends MX_Controller {
	private $data = array();
	 ...
	 */
	 ...
	 function __construct() {
		//Load required Model, Library, language and Helper files
			$load = array(
				'model'		=> 'model_associateprofessional',  	
				'language' 	=> 'showcase',							
				'config'		=>	'media/media'   		
			);
			parent::__construct($load);		
			
			$this->dirCacheMedia = ROOTPATH.'cache/associateprofessional/';  
			$this->dirUploadMedia = 'media/'.LoginUserDetails('username').'/associateprofessional/'; 
	 ...
	 */
     ...
     public function index() {
        ...
        $this->data['navigationHeading'] = $this->lang->line('FvProfessionals');
        $this->landingpage('countMemberFv');
	}
    public function photographyart() {
        $IndustryId = $this->config->item('photographynartSectionId');
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('PaProfessionals');
    ...
    public function musicaudio() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('MaProfessionals');
    ...
    public function writingpublishing() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('WpProfessionals');
    ...
    public function performingarts() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('ArtProfessionals');
    ...
    public function reviews() {
        ...
        $this->data['navigationHeading'] = $this->lang->line('reviewsProfessionals');
    ...
    public function news() {
        ...
        $this->data['navigationHeading'] = $this->lang->line('newsProfessionals');
        $this->data['comingsoon']= array('msg'=>$this->lang->line('ComingSoon'));
        $this->data['innerView'] = 'landingpage/one_column';
    ...
    public function landingpage($cd = 'countCravedProject') {
		$this->data['projectType'] = 'associatedprofessionals';
    ...
    function getNavigation($cd = 'countCravedProject'){
        $industryId = $this->config->item('associateprofessionalSectionId');
        $where=array('sc.isPublished'=>'t', 'sc.associatedProfessional'=>'t');
        ...
        $this->data['navigations']['writingpublishing'] = array('title'=>$this->lang->line('WpProfessionals'),'url'=>$url);

        $url = ($method == 'reviews')?'javascript:void(0);':base_url(lang().DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.'reviews');
        $this->data['navigations']['reviews'] = array('title'=>$this->lang->line('reviewsProfessionals'),'url'=>$url);

        $url = ($method == 'news')?'javascript:void(0);':base_url(lang().DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.'news');
        $this->data['navigations']['news'] = array('title'=>$this->lang->line('newsProfessionals'),'url'=>$url);
    }
	...
	function cravedProject(){	
		...
		$defaultProfileImage = $this->config->item('defaultAssProfImg_m');
	...
	function landingPageNews(){	
		...
		$industryKey = $this->config->item('APTYPE');

...
class Buyer_comment extends MX_Controller {
	...
	public function index($userId){
		...
		$this->data['perPageRecord'] = 10;

		// Add by Amit to set cookie for Results per page
		if($this->input->post('ipp')!=''){
			$isCookie = setPerPageCookie('buyerCommentPerPageVal',$this->data['perPageRecord']);	
		}else {
			$isCookie = getPerPageCookie('buyerCommentPerPageVal',$this->data['perPageRecord']);		
		}
		...
		{
			$this->template_front_end->load('template_front_end','view_buyer_comment',$this->data);
		}	

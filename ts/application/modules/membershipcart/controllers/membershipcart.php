  ...
  */  
  ...
  public function membershipInvoiceEmail($orderId=0)
  {
    ...
    if(!empty($membershipDetails)){
        ...
        $emailSendData = $this->_membershipInvoiceEmailNTmail($membershipDetails);
   ...
   * @return: html (string) 
   */ 
   private function _membershipInvoiceEmailNTmail($membershipDetails){
    ...
    switch($orderType){
        ...
        case '3';
            ...
            $purchaseEmailData = $this->_toolPurchaseEmailNTmail($membershipDetails);
        break;

        case '2';
        case '4';
            //call method for tool refund notification 
            $purchaseEmailData = $this->_toolRefundEmailNTmail($membershipDetails); 
        break; 
    }
  ...
  private function _toolPurchaseEmailNTmail($membershipDetails){
  ...
  private function _toolRefundEmailNTmail($membershipDetails){
      
    //tool refund prepare email body and subject
    $whereCondi         =  array('purpose'=>'membershiprefund','active'=>1);
    $toolPurchaseBody   =  getDataFromTabel('EmailTemplates','templates,subject',  $whereCondi, '','', '', 1 );
    $toolPurchaseBody   =  $toolPurchaseBody[0];
    $site_url           =  base_url('');
    $site_base_url      =  site_base_url();
    ...
    $linkedin_url       =  $this->config->item('linkedin_follow_url');
    $toolBody           =  $toolPurchaseBody->templates;
    $refund_cost        =  $item_price;
    $memdership_type    =  $item_name;
    $space              =  $item_size;
    ...
    $replaceArray       =  array($crave_us,$facebook_url,$linkedin_url,$refund_cost, $memdership_type, $space, $mail_url, $paypal_email, $site_url,$site_base_url,$image_base_url,$invoice_url);
    $emailBody          =  str_replace($getArray, $replaceArray, $toolBody);
    $emailSubject       =  $toolPurchaseBody->subject;

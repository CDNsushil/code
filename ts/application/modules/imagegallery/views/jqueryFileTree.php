...
//
echo _showpath;
echo $_POST['dir'] = urldecode($_POST['dir']);
	...
	if( count($files) > 2 ) { /* The 2 accounts for . and .. */
		...
		foreach( $files as $file ) {
			if( file_exists($root . $_POST['dir'] . $file) && $file != '.' && $file != '..' && is_dir($root . $_POST['dir'] . $file) ) {
				echo "<li class=\"directory collapsed\"><a href=\"#\" rel=\"" . htmlentities($file) . "/\">" . htmlentities($file) . "
				</a>";
				?>
                <span class="op_menu" onclick="javascript:upload_file('<?php echo htmlentities($_POST['dir'] . $file); ?>/');">options</span>
                <?php
        ...
        -->
        <?php
		foreach( $files as $file ) {/*echo '<script language="javascript">alert("'.$file.'")</script>';*/
			if( file_exists($root . $_POST['dir'] . $file) && $file != '.' && $file != '..' && !is_dir($root . $_POST['dir'] . $file) ) {

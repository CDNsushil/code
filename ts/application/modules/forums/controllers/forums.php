...
class Forums extends MY_Controller {
	...
	//   	$offset = $this->uri->segment(3); // For pagination
		...
		**/
		$data = array(
			...
			'categories' 			=> $this->categories_m->get_categories('forums'),
		...
		**/ 
        $breadcrumbItem=array('Forum');
		$breadcrumbURL=array('forums/');
		$breadcrumbString=set_breadcrumb("","", $breadcrumbItem ,$breadcrumbURL);
		$data['breadcrumbString']=$breadcrumbString;
		
			
		$data['page']=$this->forums_page_construct($page, $title, $data, 'forums');
		
		$this->template_front_end->load('template_front_end','forums_innerTemplate',$data);	
	...
	**/		
	...
	{		
        ...
        * @per_page - Setting for topics to show per page.
		**/
		$config['base_url'] = site_url().'forums/topics/'.$this->uri->segment(4).'';
		...
		**/ 
        ...
        $breadcrumbItem=array('Forum','Category');
		$breadcrumbURL=array('forums','forums/topics/'.$category);
		$breadcrumbString=set_breadcrumb("","", $breadcrumbItem ,$breadcrumbURL);
		$data['breadcrumbString']=$breadcrumbString;
	...
	**/		
	public function posts($category_id=NULL, $topic_id=NULL, $offset=NULL)
    {
		...
		**/
		...
		{
			$config['base_url'] = site_url().'forums/posts/'.$this->session->userdata('cat_id').'/'.$this->session->userdata('post_id').'';
		...
		{
				$config['base_url'] = site_url().'forums/posts/'.$this->uri->segment(4).'/'.$this->uri->segment(5).'';
				$config['total_rows'] = $this->topics_m->count_posts($topic_id);
		...
		**/ 
	    ...
	    $breadcrumbItem=array('Forum','Discussion');
		$breadcrumbURL=array('forums','forums/posts/'.$category_id.'/'.$topic_id);
		$breadcrumbString=set_breadcrumb("","", $breadcrumbItem ,$breadcrumbURL);
		$data['breadcrumbString']=$breadcrumbString;
	...
	**/		
		...
		**/	 
		...
		{	
			...
			**/	
			...
			$categories = $this->categories_m->get_categories_new_topic('forums');
			...
			**/	
			...
			foreach($categories as $row)		
			{
				...
				$subCategories = $this->categories_m->get_sub_categories($row['CategoryID'],'forums');
			...
			**/	
			...
			$search_tag = array(			
				...
				'class' 			=> 'textbox width253px formTip',
			);			 		
	
			$comments = array(			
				'name' 				=> 'comment',			
				'id' 				=> 'comment',	
				'class'				=> 'textarea width480px',
				'cols'				=> '62'
			);	
			...
			*  Add breadcrumb in view
			**/ 
			$breadcrumbItem=array('Forum','New Discussion');
			$breadcrumbURL=array('forums/','forums/new_topic');
			$breadcrumbString=set_breadcrumb("","", $breadcrumbItem ,$breadcrumbURL);
			$data['breadcrumbString']=$breadcrumbString;
    ...
    public function edit_topic($userId=0,$topic_id=0)
    {
		
	 $loginUser = isLoginUser();	 
	  if( isset($userId) && ($userId>0) && ($loginUser!=$userId)){
	    redirectToNorecord404();
	   } 
	   
		$createdUser =  $this->session->userdata('username');
		//If forum record not exist then redirect to nofound page
		if(isset($topic_id) && !empty($topic_id) && isset($createdUser)){
			$userDataWhere = array('TopicID'=>$topic_id,'CreatedBy'=>$createdUser);
			checkUsersProjects('forum_topics',$userDataWhere);
		}
		
		if(empty($topic_id) || !isset($topic_id)){
			redirectToNorecord404();
		}
		/**
		* Store the page the user came from.
			...
			**/	
            ...
            {   
				...
				**/ 
				...
				$breadcrumbItem=array('Forum','Edit Topic');
				$breadcrumbURL=array('forums','forums/edit_topic/'.$userId.'/'.$topic_id);
				$breadcrumbString=set_breadcrumb("","", $breadcrumbItem ,$breadcrumbURL);
				$data['breadcrumbString']=$breadcrumbString;
		...
		if ($this->ion_auth->logged_in())
		{
			...
			foreach($post as $row)
			{
				...
				**/ 
				...
				$breadcrumbItem=array('Forum','Edit Post');
				$breadcrumbURL=array('forums','forums/edit_post/'.$userId.'/'.$postID);
				$breadcrumbString=set_breadcrumb("","", $breadcrumbItem ,$breadcrumbURL);
				$data['breadcrumbString']=$breadcrumbString;
	...
	public function search($limit=NULL, $offset=NULL)
	{
		...
		switch($action) {
			...
			case 'display':
				...
				if($this->input->get('search')=='Search Forum...')
				{
				...
				$results = $this->core_m->search_forums($query_array, $this->sSettings[0]['topicsPerPage'], $this->uri->segment(6));
				...
				**/ 
				...
				$breadcrumbItem=array('Forum','Search','Display');
				$breadcrumbURL=array('forums','forums/search/display/'.$query_id,'forums/search/display/'.$query_id);
				$breadcrumbString=set_breadcrumb("","", $breadcrumbItem ,$breadcrumbURL);
				$data['breadcrumbString']=$breadcrumbString;
		...
		**/	 
		...
		if ($this->ion_auth->logged_in())
		{
				...
				$body = array(
					...
					'type' => 'text',
					'rows' => '4',
					'class' => 'textarea_small font_arial rz height_60 width_402',
					'onkeyup' => "checkWordLen(this,50,'contactmeMessageLimit')",
					'wordlength' => '5,50',
	...
	public function request_sub_cat()
	{
		...
		if($user_id)
		{
				$request_return  = $this->posts_m->request_sub_cat($user_id);
				...
				$this->email->subject(sprintf('Forum subcategory request', $this->config->item('website_name', '')));
				...
				$replaceArray = array('Forum', $username , $parent_cate_name, $request_cate_name, $this->config->item('website_name', ''));

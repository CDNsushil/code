	...
	if(is_array($listValues) && count($listValues) > 0){	
			...
			<div class="all_list_item">
			   <div class="row">
				 ...
				 <div class="cell ml10 maxWidth59px">
					...
					?>
					<script>
						var editDataMedia<?php echo $mediaId;?> = <?php echo $editJsonMedia; ?>;
					</script>
					<?php
						...
						'onclick' => 'editMediaWs(editDataMedia'.$mediaId.',\''.$toggleId.'\');'
						);	
						
						 $newImg=getImage($imagePath.$imageName);
	 ...
	 <div class="promoAction">	
		<div class="cell pro_btns">	        
					<div class="small_btn">						
						<?php 
						if(strcmp($this->router->method,'launchEventForm')==0) 
...
var deleData<?php echo $mediaId;?> = <?php echo $jsonMedia; ?>;
	...
	?>		
		...
		<?php 
			...
			'title'=>$this->lang->line('edit'),
			'fileName' => $imageName ,
			'onclick' => '$(\'#'.@$toggleId.'-Content-Box\').slideDown(\'slow\');'
			);					
	...
	<?php } 
		...
		echo '<div id="addLink" >';  		
			$noRecBtnArr = array('class' => 'a_orange','onclick' => '$(\'#PromoForm-Content-Box\').slideDown(\'slow\');');						
...
{	
	...
	$('.GalId').click(function(){
		...
		$('#<?php echo @$toggleId?>promoImage').attr('src',new_img_src);		
		$('#mediaId').val(mediaPromoId);
		$('#fileId').val(mediafileId);
		$('#mediaTitle').val(galTitle);
		$('#mediaDesc').val(galAltText);

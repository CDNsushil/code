...
class model_cart extends CI_Model { 
 ...
 function getAllUserBasketItems($itemId=0,$currency=0){	  
  ...
  function getUserBillingDetails($id){	  
	$this->db->select('bill.billing_firstName,bill.billing_lastName,bill.billing_lastName,bill.billing_address1,bill.billing_address2,bill.billing_city,bill.billing_state,bill.billing_country,bill.billing_zip,bill.billing_phone,bill.billing_email,bill.EuVatIdentificationNumber,MasterCountry.countryName,MasterCountry.countryGroup,EuVatIdentificationNumber');	
  ...
  function getBillingShipDetails($id){	  
	...
	$this->db->select('ship.shipping_firstName,ship.shipping_lastName,ship.shipping_address1,ship.shipping_address2,ship.shipping_state,ship.shipping_country,ship.shipping_city,ship.shipping_zip,ship.shipping_phone,ship.shipping_email,MasterCountry.countryName as shippingcountryName');	
 ...
 */   
	...
	function get_purchased_details($offset=0,$limit=0)
	{
		$this->db->select('SalesOrder.ordId,SalesOrder.custName,SalesOrder.ordNumber,SalesOrder.ordDateComplete,SalesOrder.ordCurrency,SalesOrderItem.sectionId,SalesOrderItem.itemId,SalesOrderItem.entityId,SalesOrderItem.sellerId,SalesOrderItem.elementId,SalesOrderItem.projId,SalesOrderItem.itemQty,SalesOrderItem.basePrice, SalesOrderItem.receiverTransactionId, SalesOrderItem.itemName,SalesOrderItem.transactionId,SalesOrderItem.invoiceId,SalesOrderItem.purchaseType,SalesOrderItem.sellerInfo');
		$this->db->select('SalesItemDownload.dwnId,SalesItemDownload.dwnDate,SalesItemDownload.dwnMaxday,SalesItemDownload.dwnStatus,SalesItemDownload.userId,SalesItemDownload.ownerId');
		$this->db->from($this->tableSalesOrder);	  	
 ...
 */   
	...
	function get_sales_details($offset=0,$limit=0)
	{
		$this->db->select('SalesOrder.ordId,SalesOrder.custName,SalesOrder.customerUid,SalesOrder.ordNumber,SalesOrder.ordDateComplete,SalesOrder.ordCurrency,SalesOrderItem.sectionId, SalesOrderItem.receiverTransactionId, SalesOrderItem.itemId,SalesOrderItem.entityId,SalesOrderItem.sellerId,SalesOrderItem.elementId,SalesOrderItem.itemQty,SalesOrderItem.basePrice,SalesOrderItem.itemName,SalesOrderItem.invoiceId,SalesOrderItem.purchaseType,BuyerComments.rateSeller,BuyerComments.comments,SalesOrderItem.sellerInfo');
		$this->db->from($this->tableSalesOrder);	  	
	...
	function salesDetailsForExport()
	{
		$this->db->select('SalesOrder.ordDateComplete as  Date, SalesOrderItem.receiverTransactionId as Invoice,SalesOrder.custName 
		as 	Buyer,SalesOrderItem.sellerInfo as seller,SalesOrderItem.itemName as Item,SalesOrderItem.itemQty as 
...
function getPurchaseComment($orderId){
 ...
 /*******
  ...
  function get_membership_item($orderId)
	...
	function get_purchased_listing($offset=0,$limit=0,$returnRow=false)
	{
		$this->db->select('o.orderId,o.ordNumber,o.orderType,o.tdsUid,o.custName');
		$this->db->select('i.*');
	...
	function export_invoice_details()
	{
		$this->db->select('o.createDate as Date, o.ordNumber as invoice, o.paypalEmail, o.custName as Buyer');

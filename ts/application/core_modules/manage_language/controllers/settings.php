...
class Settings extends MX_Controller{
	...
	function __construct(){
		...
		$this->load->model(array('model_language','language_model'));
	...
	function lang_list($dir=FALSE){
		...
		if($dir===FALSE){
			...
			$this->toad_admin_template->load('toad_admin_template','language/dir_list',$data);
		}else{
			$data['sel_dir']=$dir;
			$data['dir'] = $this->model_language->get_languages();
			$data['files'] = $this->model_language->get_list_lfiles($dir);
			$this->toad_admin_template->load('toad_admin_template','language/file_list',$data);
		}
	}
	...
	function lang_file($l=FALSE,$file=FALSE){
		...
		if($l!==FALSE && $file!==FALSE && is_dir(APPPATH."language/$l/") && file_exists(APPPATH."language/$l/$file")){
			...
			$this->toad_admin_template->load('toad_admin_template','language/edit_lang_file',$data);
		...
		if($this->input->post('create')){
			...
			if(!empty($l) && !is_dir(APPPATH."language/$l/")){
				...
				if(mkdir(APPPATH."language/$l/")){
					...
					// ---- Added By Anoop ---
					$ll="english";
					$source=APPPATH."language/$ll/";
						$dir_handle = @opendir($source) or die("Unable to open");
						while ($file = readdir($dir_handle)) 
						{
							if($file!="." && $file!=".." && !is_dir("$source/$file"))
							{
								copy(APPPATH."language/$ll/$file",APPPATH."language/$l/$file");
							}	
						}
						closedir($dir_handle);
					$this->model_language->add_new_db_language($l);
					// ---- Added By Anoop ---
		...
		if($this->input->post('delete')){ //check if form was submitted
			...
			if(!empty($l) && is_dir(APPPATH."language/$l/")){
				if(delete_files(APPPATH."language/$l/", TRUE) && rmdir(APPPATH."language/$l/")){
					...
					$this->model_language->delete_db_language($l);
				}else{
	...
	function prepare_str($string){
		$from = array('a','c','e','l','ó','n','s','z','z',' '); ///polish signs
	...
	function set_user_language($language){
			$update_lang = $this->language_model->update_user_language($language);
			if($update_lang) return true;
				else return flase;			
	}
	...
	function set_language(){
	  ...
	  // echo $language;die;
		$cur_url    = BASEURL.$this->input->post('cur_url');

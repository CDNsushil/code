...
<script type="text/javascript" charset="utf-8">
 ...
 <div id="wrapperL">
	...
	<div id="showUserList">
		<div id="container">
			<?php if(isset($isComposeSection)) { ?>
				<div id="add" class="tds-button Fright mr23 mb5 dn" onclick="sendNewsletterEmail();"> <button type="button"  onmousedown="mousedown_tds_button(this)" onmouseup="mouseup_tds_button(this)" class="font_arial dash_link_hover">
					<span><div class="Fleft">Send</div> <div class="icon-save-btn"></div> </span></button>
				</div>
			<?php } else { ?>
				<div id="add" class="tds-button Fright mr23 mb5 dn" onclick="setValInTo(),$(this).parent().trigger('close');"> <button type="button"  onmousedown="mousedown_tds_button(this)" onmouseup="mouseup_tds_button(this)" class="font_arial dash_link_hover">
					<span><div class="Fleft">Add</div> <div class="icon-save-btn"></div> </span></button>
				</div>
			<?php } ?>
			<div id="selectall" class="tds-button Fright mr23 mb5"> <button type="button"  onmousedown="mousedown_tds_button(this)" onmouseup="mouseup_tds_button(this)" class="font_arial dash_link_hover"><span><div class="Fleft">Select all</div> </span></button></div>
			<div id="unselectall" class="tds-button Fright mb5 dn"> <button type="button"  onmousedown="mousedown_tds_button(this)" onmouseup="mouseup_tds_button(this)" class="font_arial dash_link_hover"><span><div class="Fleft">Unselect all</div> </span></button></div>
			<div id="show_updated" class="orange_color hidden tac f16 pr137"> Record Updated</div>
			<form action="<?php echo base_url('admin/settings/manage_newsletter/manage_users_grid'); ?>" method="post" accept-charset="utf-8" name="userForm" id="userForm" novalidate="novalidate">
...
$(document).ready(function(){
	...
	$("#unselectall").click(function () {	
		$('#add').hide();
		$('.case:checkbox').removeAttr('checked');
		$('#unselectall').hide();
	  });
	...
	{
		var BASEPATH = "<?php echo base_url();?>";
		var newsletterId = "<?php echo $newsletterId;?>";
		var userVal = [];
		$(':checkbox:checked').each(function(i){
				  userVal[i] = $(this).val();  
		});	
		var form_data = {userIds: userVal,newsletter: newsletterId,newsletterAjax: '1'};
		$.ajax
		({
			type: "POST",
			url: BASEPATH+"admin/settings/manage_newsletter/send_mail",
			data: form_data,
			beforeSend: function() {
			...
			success: function(data)
			{		
				window.location.href = BASEPATH+'admin/settings/manage_newsletter';
			}
		});
		return false;	
	}

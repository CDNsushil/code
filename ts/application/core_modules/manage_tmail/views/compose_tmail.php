...
$headText = $this->lang->line('composeTmail');
...
echo ''.form_open(base_url(SITE_AREA_SETTINGS.'manage_tmail/compose_mail'),$formAttributes).'
...
<div class="page_list width_530px">
	<ul class="ul_relative"> 
		...
		?>
            <a class="formTip" href="javascript:void(0);" onclick="openLightBox('popupBoxWp','popup_box','/admin/settings/manage_tmail/manage_users_grid/<?php echo $messageId;?>')" original-title="Add Users">
                <span>
...
$(document).ready(function(){
	$("#composeTmailForm").validate({
		submitHandler: function() {
			...
			$('#mBody').html(body);
			userIds = $('#userIds').val();
			var fromData=$("#composeTmailForm").serialize();
			...
			$.post(baseUrl+'admin/settings/manage_tmail/send_mail',fromData, function(data) {
				if(data){
					//alert(data);
					window.location.href=baseUrl+'admin/settings/manage_tmail';
				}
			});
		}
	});
...
function setUserEmail()
{
	...
	({
		type: "POST",
		url: BASEPATH+"/admin/settings/manage_tmail/setEmails",
		data: form_data,

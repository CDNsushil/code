...
class settings extends MY_Controller {
	...
	function __construct() {
		...
		function index($pressReleaseId=0) 
		{
			...
			$this->toad_admin_template->load('toad_admin_template','manage_pressRelease/admin_pressRelease_list', $data);
		}
	...
	*/
	function add_edit_pressRelease($pressReleaseId=0){ 	 
        ...
        $this->toad_admin_template->load('toad_admin_template','manage_pressRelease/add_edit_pressRelease', $this->pressReleaseData);
    }
	...
	**/	
	function save_pressRelease()
	{
		...
		if($this->input->post('save')=='Save' && $this->form_validation->run()){
			...
			$redirect='admin/settings/manage_pressRelease/add_edit_pressRelease/'.$pressReleaseId;
			redirect($redirect);
	...
	*/
	function delete_pressRelease($pressReleaseId)
    {
		...
		if($pressReleaseId > 0 )
		{
			...
			redirect('admin/settings/manage_pressRelease');
		}
		else{

...
class Settings extends MX_Controller{
	...
	function tips_list($id=FALSE) {		
		...
		if($id===FALSE)
		{
			...
			$this->toad_admin_template->load('toad_admin_template','manage_tips/tip_list', $data);
		}
		else
		{  
			$tips = $this->model_admin_tips->get_tips();
	...
	*/
	public function add_tip() {	
		if(!$this->lang_library->login_check()){
			...
			**/
			$this->toad_admin_template->load('toad_admin_template','manage_tips/add_tip',$data);	
		}	
	}
	...
	function submit_tip()
	{
		...
		**/	
		if($this->input->post('title')!='' && $this->input->post('subtitle')!='')
		{
			...
			redirect(SITE_AREA_SETTINGS."manage_tips/tips_list");
		} else {
			$this->session->set_flashdata('error',$this->lang->line('tipAddError'));
			redirect(SITE_AREA_SETTINGS."manage_tips/add_tip");
		}
	...
	*/
    public function update_tip($tipId=FALSE)
    {
		...
		redirect(SITE_AREA_SETTINGS.'manage_tips/tips_list/'.$tipId);
		} else {
			$this->session->set_flashdata('error',$this->lang->line('tipAddError'));
			redirect(SITE_AREA_SETTINGS.'manage_tips/edit_tip/'.$tipId);
		}
	}
	...
	*/
	public function delete_tip($titleId)
    {
		...
		if($this->model_admin_tips->remove_tip($titleId))
		{
			...
			redirect(SITE_AREA_SETTINGS.'manage_tips');
		}
		else{

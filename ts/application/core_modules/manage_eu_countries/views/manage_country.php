...
$vatPercentage = array(
	...
	'id' 		=> 'vatPercentage',
	'type' 		=> 'text',
	'class' 	=> 'textbox width450px NumGrtrZero required',
	'min'       =>  '0.1',
	'value'	=> set_value('vatPercentage',$countryData[0]['vatPercentage']),
);
$status = array(
	'name'        => 'status',
	'id'       	  => 'status',
...
echo ''.form_open(base_url(SITE_AREA_SETTINGS.'manage_eu_countries/add_country'),$formAttributes).'
<div class="page_list">
	<ul class="ul_relative"> 
		...
		<div id="oneLineDescription" class="label_wrapper_topic">
			<label class="select_field_topic"> '.$this->lang->line('vatPercentage').' </label>
		</div>
		<li>'.form_input($vatPercentage).'</li>
...
$(document).ready(function(){
	...
	$("#countryForm").validate({
		submitHandler: function() {
			...
			$.post(baseUrl+'admin/settings/manage_eu_countries/update_country',fromData, function(data) {
				if(data){
					if(countryId!=''){
						window.location.href=baseUrl+'admin/settings/manage_eu_countries/manage_country/'+countryId;
					}else{
						window.location.href=baseUrl+'admin/settings/manage_eu_countries';
					}
				}
			});
		}

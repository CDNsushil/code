...
class CI_DB_active_record extends CI_DB_driver {
	...
	{
		if(!is_bool($val) && !is_array($this->ar_distinct) && strlen($val)>=2){
			$this->ar_distinct = $val;
		}else{
	...
	protected function _compile_select($select_override = FALSE)
	{
		...
		{
			if($this->ar_distinct && !is_array($this->ar_distinct) && strlen($this->ar_distinct) >=2){
				$destinct=$this->ar_distinct;
				$destincts=explode('.',$this->ar_distinct);
				
				$destinctString='';
				foreach($destincts as $k=>$destinct){
					if(strstr($destinct,'"')){
						
					}else{
						$destinct='"'.$destinct.'"';
					}
					$destinctString.=$destinct;
					if(($k+1)<count($destincts)){
						$destinctString.='.';
					}
				}
				
				$sql = 'SELECT DISTINCT ON('.$destinctString.') ';
			}else{

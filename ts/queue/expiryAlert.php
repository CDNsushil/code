		...
		while ($row = pg_fetch_assoc($res)) {
			...
			if(isset($row['entityId']) && isset($row['elementId']) && $row['entityId'] > 0 && $row['elementId'] >0){
				...
				switch($row['entityId']){
					...
					case 54: 
						if($row['pkgSections']=='{1}'){
							$dashboard_url.='filmNvideo';
						}elseif($row['pkgSections']=='{2}'){
							$dashboard_url.='musicNaudio';
						}elseif($row['pkgSections']=='{3}' || $row['pkgSections']=='{3:1}' || $row['pkgSections']=='{3:2}'){
							$dashboard_url.='writingNpublishing';
						}elseif($row['pkgSections']=='{4}'){
							$dashboard_url.='photographyNart';
						}elseif($row['pkgSections']=='{10}'){
							$dashboard_url.='educationMaterial';
						}
						break;
				...
				if(is_array($template) && count($template) > 0) {
					...
					$searchArray = array("{tool_url}","{tool_title}" ,"{tool_text}" ,"{tool_item_title}" , "{dashboard_url}" , "{site_url}" , "{site_base_url}", "{image_base_url}","{crave_us}","{subject}","{facebook_url}","{linkedin_url}","{twitter_url}","{gPlus_url}");
					$replaceArray = array($tool_url,$tool_title,$tool_text,$tool_item_title,$dashboard_url,$site_url,$site_base_url,$image_base_url,$crave_us,$subject,$facebook_follow_url,$linkedin_follow_url,$twitter_follow_url,$google_follow_url);

...
//class MY_Lang extends CI_Lang {
    ...
    ***************************************************/
    ...
    function MY_Lang()
    {
       ...
       /* date:09-05-2012 added by sushil to redirct all browser language to english start */ 
        if((!empty($this->lang_code)) && $this->lang_code != 'en'){
					 // set default language
			  $CFG->set_item('language', $this->languages['en']);
			  
			  $uri = (!empty($this->uri)) ? $this->uri: $this->default_uri;
				 $uri = ($uri[0] != '/') ? '/'.$uri : $uri;
				 $uri = str_replace('/'.$this->lang_code,'en',$uri);
			  $new_url = $CFG->config['base_url'].$uri;
			  header("Location: " . $new_url, TRUE, 302);
			  exit;
		}
		/* added bu sushil to redirct all browser language to english End */  

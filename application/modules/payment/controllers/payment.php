		...
		if(isset($items) && is_array($items) && count($items)) {
			foreach($items as $item){
				...
				$insertSalesOrderItem = array(
                    ...
                    'fileSize' =>(!empty($item->fileSize))?$item->fileSize:0,
                );	
                
                
                // ---------- Insert in Your toadsquare Function ------------------//
                $yourtoadArray = array('entityid'=>$entityId,'elementid'=>$elementId,'projectid'=>$projId,'sectionid'=>$sectionId);
                yourToadsquareNotification($yourtoadArray,'shoppingcart');
				...
				if(is_numeric($salesItemId) && $salesItemId > 0){
					...
					if($item->purchaseType==1){
                        ...
                        //-----if project shippabele and download and payper view start-----//&&
                        // ($elementId == $projId)
                        if(($entityId == 54 || $entityId == 12) ){
                        ...
                        //-----if project shippabele and download and payper view end-----//
					...
					foreach($get_sales_record['get_result'] as $get_invoice_data)
					{
						...
						if($get_Items_Detils['get_num_rows'] > 0)
						{
							...
							foreach($get_Items_Detils['get_result'] as $itemsDetils)
							{
								...
								$itemNameListing[$list]['itemUrl'] = getFrontEndLinkNew($itemsDetils->entityId, $itemsDetils->elementId);
							    $event_date_data = ''; 
						...
						if(isset($get_invoice_data->grandTotal) && $get_invoice_data->grandTotal>0) {
							...
							{
								$item_body .=  '<a target="_blank" class="gryclr hoverOrange" href="'.$itemListing['itemUrl'].'" >'.$itemListing['itemName'].'</a>';
								if($itemCount > $iCount)
								{
									$item_body .=', ';	
								}
							...
							$purchase_page_url = base_url_lang().'cart/mypurchases';
							$comment_url = base_url_lang().'cart/mypurchases';
							$dashboard_url = base_url_lang('home');
							...
							$image_base_url = site_base_url().'images/email_images/';
							$crave_us = $this->config->item('crave_us');
							$salespage_url = base_url_lang().'cart/mysales';
							$dashboard_url = base_url_lang('home');
							$facebook_url = $this->config->item('facebook_follow_url');
							$linkedin_url = $this->config->item('linkedin_follow_url');
							$twitter_url = $this->config->item('twitter_follow_url');
						...
						{	
							...
							$twitter_url = $this->config->item('twitter_follow_url');
							$gPlus_url = $this->config->item('google_follow_url');
							$purchasePageUrl = base_url_lang().'cart/mypurchases';
                            $event_showcase_url = '';

...
class Language extends MX_Controller{
	...
	function lang_list($dir=FALSE){
		if($dir===FALSE){
			...
			$this->admin_template->load('admin/admin_template','language/dir_list',$data);
		}else{
			$data['sel_dir']=$dir;
			$data['dir'] = $this->model_language->get_languages();
			$data['files'] = $this->model_language->get_list_lfiles($dir);
			$this->admin_template->load('admin/admin_template','language/file_list',$data);
		}
	}
	...
	function lang_file($l=FALSE,$file=FALSE){
		if($l!==FALSE && $file!==FALSE && is_dir(APPPATH."language/$l/") && file_exists(APPPATH."language/$l/$file")){
			...
			$this->admin_template->load('admin/admin_template','language/edit_lang_file',$data);

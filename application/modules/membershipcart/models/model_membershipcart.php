...
/* PAYPAL TRANSACTION DETAILS */	
 ...
 function getRefundItemSize($userContainerId,$orderId){		
	...
	function userNotUsedDefaultProduct($userId)
  {
	...
	/*
	 * @access: public
	 * @description: This function is use to get workprofile details
	 * @return array
	 */ 
	function getWorkProfileDetails($workProfileId=0) {
		...
		if($workProfileId > 0){
			 ...
			 $this->db->select($this->tableUserShowcase.'.optionAreaName,'.$this->tableUserShowcase.'.enterprise,'.$this->tableUserShowcase.'.enterpriseName');
			 ...
			 $this->db->join($this->tableUserShowcase, $this->tableUserShowcase.".tdsUid = ".$this->tabelWorkprofile.".tdsUid", 'left');
			 $this->db->join($this->tableUserProfile, $this->tableUserProfile.".tdsUid = ".$this->tabelWorkprofile.".tdsUid", 'left');
			 $this->db->join($this->tableMasterCountry, $this->tableMasterCountry.'.countryId = CAST("'.$tabelWorkprofile.'"."profileCountry" as int)', 'left');
	...
	/*
	 * @access: public
	 * @description: This function is use to get blog data on the basis of logged in User 
	 * @return array
	 */
	function getUserBlog($userId) {
		 $this->db->select('blogId,blogTitle,blogOneLineDesc,blogTagWords,blogDesc,blogIndustry,blogLanguage,dateCreated,blogToFacebook,blogToTwitter,blogTwitterLink,blogToShareOn,blogToDonate,blogFor,blogCraveCount,isProfileCoverImage,custId,blogViewCount,'.$this->tableBlogs.'.isPublished,dateModified,filePath,fileName,rating');
		 $this->db->select($this->tableBlogs.'.fileId');
		 $this->db->select($this->tableUserContainer.'.*');
		 $this->db->from($this->tableBlogs);
		 $this->db->join($this->tableMediaFile,$this->tableMediaFile.'.fileId = '.$this->tableBlogs.'.fileId', 'left');
		 $this->db->join($this->tableUserContainer, $this->tableUserContainer.".userContainerId = ".$this->tableBlogs.".userContainerId", 'left');

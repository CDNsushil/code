    ...
    <div class="width445 pl115 music_showcase"><?php 
        if($userPlaylistData && getMyPlaylistCount($tdsUid)){
            $loggedUserId = isLoginUser();
                echo Modules::run("player/audioplaylistplay",$isPlayAll);
             ?>
          
            
               <div class="playall  muct_p_list fr text_alignR pb15"><span class="pr12  fr"><p class="fl pt5">PLAY ALL</p> <button class="play_btn playallaudio fr mt0 <?php echo ($isPlayAll)?"playing":""; ?>" type="button"></button></span></div>
             <div  id="myplaylist">
...
<script type="text/javascript">
    ...
    $(".playallaudio").click(function(){
        ...
        var fromData = {"userid":'<?php echo $tdsUid; ?>'};
        var url = '<?php echo $baseUrl.'/mypaylistview/'.$tdsUid.'/'.$playFlagData;?>';
        $.post(url,fromData, function(data) {
            if(data){
               $('.playloaddiv').html(data);
            }
        },"text");

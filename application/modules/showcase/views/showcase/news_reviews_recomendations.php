...
//if(!empty($recommendationList) || !empty($reviewList) || !empty($externalNews)){
    ...
    <div class="bg_f7f7f7 recomdation_wrap mt3 width100_per display_inline_block" >
        ...
        // Flag check for only one view //
            ...
            if($showcaseData->recommendMe == 't'){?>
            <li><a class="<?php if($flagcursor < 2){ echo "ptr_none"; } ?>" name="#tab1"  id="current">Recommendations</a></li>
        <?php }
            if($showcaseData->reviewMe == 't'){ ?>
            <li><a  class="<?php if($flagcursor < 2){ echo "ptr_none"; } ?>" name="#tab2">Reviews</a></li>
        <?php }
            if(!empty($externalNews)){ ?>
            <li><a  name="#tab3">News</a></li>
        <?php } ?>
        ...
        <div id="content_tabs" class=" recom_cnt  clearb ">
            ...
            if($showcaseData->reviewMe == 't'){?>
                <div id="tab2"><?php 
                    ...
                    $this->load->view('media/reviewViewNew',array('reviewdesign'=>'3','reviewProjectId'=>$showcaseId,'reviewElementId'=>$showcaseId,'reviewEntityId'=>$entityId,'reviewIndustryId' =>$industryId,'isPublished'=>$showcaseData->isPublished));
                    ?>
                </div>
                <?php
            }

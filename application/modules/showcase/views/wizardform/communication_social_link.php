...
<div class="content display_table  TabbedPanelsContent width635 m_auto">
    <div class="wra_head clearb">
        <h3 class="red   fs21 fnt_mouse bb_aeaeae">Add links to your social media sites to your Showcase Homepage</h3>
        ...
        <?php $this->load->view('wizardform/socialmedia_form');?>
        <?php
         $i = 0;
        
         if(!empty($socialMediaData)) { ?>
            <div id="FaduoutWhholeDiv">
        ...
        <?php 
           ...
           <ul class="mt25 clearbox">
            ...
            <input type="hidden" id="mediaAddinpitid" value ="<?php echo $i; ?>" >
    </div>
</div>
        ...
        confirmBox("Do you really want to delete this social link?", function () {
           var fromData = {profileSocialLinkId: profileSocialLinkId};
            $.post(baseUrl+language+'/dashboard/removeSocialMedia',fromData, function(data) {
              if(data) {
                      // To delete Complete div
                      var i = $('#mediaAddinpitid').val();
                      i = parseInt(i);
                     
                      if(i == 1)
                      {
                        $('#FaduoutWhholeDiv').fadeOut();
                      }
                      else
                        {
                            $('#socialLi'+profileSocialLinkId).fadeOut();
                            i--;
                            $('#mediaAddinpitid').val(i);
                        }   		      
                   }
            });
        }); 
	...
	$('#saveMediaSection').click(function() {
		...
		if(sectionIds.length>0) {
			...
			({
				...
				{	
					if(data) {	
						window.location.href = '<?php echo $baseUrl.'/previewnpublishhowcase'?>';	
					}
				}
			});
		} else {
			customAlert('Please select section first!');
		}
		return false;
	});
</script>           

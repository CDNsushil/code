...
class workprofilefrontend extends MX_Controller {
   ...
   function index($viewProfileId='')
   {
		...
		// get workprofile social links
		$data['wpSocialMedia'] = $this->model_common->getDetailSocialMedia(array('workProfileId'=>$currentWorkProfileId));
		$data['userInfo'] = $userData;
			
	  // echo "<pre>";print_r($data);die;
	   if($norecord == true)
			//redirectToNorecord404();
			$this->new_version->load('workprofile_new_version_template','new_version/workprofile', $data);	 
	   else
			$this->new_version->load('workprofile_new_version_template','new_version/workprofile', $data);	  
   ...
   function employeHistory($viewProfileId='')
	{
		...
		if($norecord == true)
			//redirectToNorecord404();
			$this->new_version->load('workprofile_new_version_template','new_version/emp_history',$data);
	    else		
			$this->new_version->load('workprofile_new_version_template','new_version/emp_history',$data);	
	 ...
	 * 	Function to Get the profile personal details
	
	***/
   
   
   function personalDetails($viewProfileId='') {
		$norecord = true; 
		if(isset($viewProfileId) && ($viewProfileId!='')){				
			//$this->userId = decode($viewProfileId);
			//$data['accessUserProfile'] = $this->userId;			
		...
		// get workprofile visa list 
		$data['WorkProfileVisa'] = $this->model_common->getDataFromTabel('WorkProfileVisa','countryId,visaType',$whereField);
		
		// get profile languages
		$data['profileLanguages']= $this->model_workprofilefrontend->getProfileLanguage($this->userId);
		
		// get Location  list
		$data['locationList'] = $this->model_common->getDataFromTabel('WorkProfileLocation','*',array('workProfileId'=>$currentWorkProfileId));
		...
		if($norecord == true)
			//redirectToNorecord404();
			$this->new_version->load('workprofile_new_version_template','new_version/personal_details',$data);
	    else		
			$this->new_version->load('workprofile_new_version_template','new_version/personal_details',$data);	
	...
	***/
    ...
    function employeShowcase($viewProfileId='')
	{
		...
		if($norecord == true)
			...
			$this->new_version->load('workprofile_new_version_template','new_version/emp_showcase',$data);
			//	redirectToNorecord404();
	    else		
			$this->new_version->load('workprofile_new_version_template','new_version/emp_showcase',$data);	 	
	} 
	...
	***/
	...
	{		
		...
		$data['getMediaData']= $this->getProfileMedia($currentWorkProfileId);					
		$this->new_version->load('workprofile_new_version_template','new_version/emp_images',$data);		

...
<?php 					
				$topCravedArray = array('entityId'=>$showcaseEntityId,'projectType'=>'creatives');
				
				$topCravedData['defaultProfileImage'] = $this->config->item('defaultCreativeImg_m');
				$topCravedData['data'] = topCraved($topCravedArray);
				
				if(is_array($topCravedData['data']) && count($topCravedData['data'])>0)	
					$topCravedHtml = $this->load->view('common/top_craved_creative',$topCravedData,true);			
				else
					$topCravedHtml = '';
	
				$bannerarray['imgarray'] = array('banner_front_creative_show-off-talent_HR.jpg','banner_front_creative_Who-your-fav-star_HR.jpg');
				...
				$pageUserId = @$this->uri->segment(4)>0?$this->uri->segment(4):isLoginUser();
				$openLi ='<li>';
				$closeLi ='</li>';
		...
		if((isset($news_array) && is_array($news_array) && count($news_array) > 0 ) || (isset($reviews_array) && is_array($reviews_array) && count($reviews_array) > 0 )){?>
         ...
         <?php
            ...
            <div class="cell bdr_lightorange10 global_shadow bg_white <?php echo $mlClass; ?> width_420">
               <div class="wp_project_heading ml11 orange_clr_imp">Latest Members</div>
                        ...
                        <a target="_blank" href="<?php echo $memberDetailUrl;?>">
							<div class="wp_project_box <?php echo $mr8;?> height226px ptr">
								<div class="dash_link_hover">
									...
									<div class="ml13 font_size11 dash_link_hover"><b><?php echo substr_unicode($userFullName,0,15);?></b></div>
								</div>

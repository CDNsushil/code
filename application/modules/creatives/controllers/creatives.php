...
class creatives extends MX_Controller {
	private $data = array();
	 ...
	 */
	 ...
	 function __construct() {
		//Load required Model, Library, language and Helper files
			$load = array(
				'model'		=> 'model_creatives',  	
				'language' 	=> 'showcase',							
				'config'		=>	'media/media'   		
			);
			parent::__construct($load);		
			$this->dirCacheMedia = ROOTPATH.'cache/creatives/';  
			$this->dirUploadMedia = 'media/'.LoginUserDetails('username').'/creatives/'; 
    ...
    public function musicaudio() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('MaCreatives');
    ...
    public function writingpublishing() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('WpCreatives');
    ...
    public function performingarts() {
        ...
        $this->data['dataURL'] = base_url(lang().'/'.$this->router->fetch_class().'/landingPageProject/'.$IndustryId);
        $this->data['navigationHeading'] = $this->lang->line('ArtCreatives');
    ...
    public function reviews() {
        ...
        $this->data['navigationHeading'] = $this->lang->line('reviewsCreatives');
    ...
    public function news() {
        ...
        $this->data['navigationHeading'] = $this->lang->line('newsCreatives');
        $this->data['comingsoon']= array('msg'=>$this->lang->line('ComingSoon'));
        $this->data['innerView'] = 'landingpage/one_column';
        $this->landingpage('countNews');
	}
    
    public function landingpage($cd = 'countMemberFv') {
		$this->data['projectType'] = 'creatives';
        $this->getNavigation($cd);
        
        if($this->data[$cd]){
            if(!isset($this->data['innerView'])){
                $this->data['innerView']= 'landingpage/four_column';
                $this->data['bgColor']= 'bg_f3f3f3';
    ...
    function getNavigation($cd = 'countCravedProject'){
        $industryId = $this->config->item('creativesSectionId');
        $where=array('sc.isPublished'=>'t', 'sc.creative'=>'t');
        ...
        $this->data['navigations']['writingpublishing'] = array('title'=>$this->lang->line('WpCreatives'),'url'=>$url);

        $url = ($method == 'reviews')?'javascript:void(0);':base_url(lang().DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.'reviews');
        $this->data['navigations']['reviews'] = array('title'=>$this->lang->line('reviewsCreatives'),'url'=>$url);

        $url = ($method == 'news')?'javascript:void(0);':base_url(lang().DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.'news');
        $this->data['navigations']['news'] = array('title'=>$this->lang->line('newsCreatives'),'url'=>$url);
	...
	function cravedProject(){	
		...
		$defaultProfileImage = $this->config->item('defaultCreativeImg_m');
	...
	function landingPageNews(){	
		...
		$industryKey = $this->config->item('CreativesTYPE');

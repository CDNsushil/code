...
$activeAllClass=($selectSection=='' || $selectSection=='all' )?'red':'';

//echo "<pre>";
//print_r($section_notification_count);die;
if($section_notification_count){
$notificationTypeString['all']='<li><a class="'.$activeAllClass.'" href="'.base_url(lang()).'/notifications/index" > '.$this->lang->line('all').' <span class="red"> '.count($section_notification_count).'</span></a></li>';
	...
	* */
	foreach($count_array_notification_projectType as $key=>$notificationsCount){
		...
		$activeClass= $key==$selectSection?'red':'';

...
class media extends MX_Controller {
    ...
    function __construct() {
        //Load required Model, Library, language and Helper files
            $load = array(
                'model'		=> 'media/model_media + dashboard/model_dashboard + membershipcart/model_membershipcart + tmail/model_tmail + messagecenter/model_message_center',  	
                'language' 	=> 'media',							
                'config'	=> 'media/media',
                'library'   => 'pagination_new_ajax + tmail/Tmail_messaging + messagecenter/lib_message_center'
            ); 
...
/*============================Film and Video Section==================================================*/	
    ...
    public function projectDescription($indusrty='filmNvideo',$projectId=0,$action="",$method='',$projectElementId='') {
        ...
        if($this->input->post('submit')=='Save' && $this->form_validation->run())
        {
            ...
            if($insertFlag){
                redirectPage('media/'.$indusrty.'/editProject/uploadMedia/'.$projectId);
            }
        }
        if($projectId > 0){
    ...
    public function uploadMedia($indusrty='filmNvideo',$projectId=0,$action="",$method='',$projectElementId='') {
        ...
        if(!$projectId > 0){
            redirectPage('media/'.$indusrty.'/newProject/projectDescription');
        }
    ...
    function writeCacheFile($industryType='',$projectId=0,$isArchive='f'){
        ...
        if(!$this->data['projects'] && $isArchive=='f'){
                ...
                if($projectTotalCount>0){
                        ...
                        redirectPage('media/'.$industryType);
                }
        }
    ...
    public function filmvideo() {
        ...
        if(isset($arg_list[0])){
            ...
            if (method_exists($this,$arg_list[0])) {
                ...
                $currentStage = $arg_list[0].DIRECTORY_SEPARATOR.$arg_list[1].DIRECTORY_SEPARATOR.$arg_list[2];
                // update current stage data
                $this->updatecurrentstage($currentStage,$arg_list[0],$arg_list[1]);
            } else {
    ...
    public function setupshowcase($arg_list) {
        ...
        // unset media edit session
        $this->session->unset_userdata('isEditMedia');
        $this->session->unset_userdata('isAddElement');
        ...
        if(!empty($indusrty)){
			...
			$subcripDetails  = $this->model_common->getDataFromTabel('UserSubscription', 'endDate,subscriptionType',  $whereSubcrip, '', $orderBy='subId', $order='ASC', $limit=1, $offset=0, $resultInArray=false);
			if(!empty($subcripDetails)) {
				$subscriptionData['subscriptionType']  = $subcripDetails[0]->subscriptionType;
				$subscriptionData['subscription_end_date']  = $subcripDetails[0]->endDate;
			}
    ...
    function saveMediaTools($arg_list){
        ...
        if(!empty($post)){
            ...
            if($this->updateDataInFile($post)){
                if(($post['addSpace'] == 1) || ($post['subscriptionType']==1 && (empty($post['availableContainer']) || !$post['availableContainer']))){
                    ...
                    if($projId = $this->saveProjectData($post)){
        ...
        redirectPage($this->redirectUrl.$nextStep); 
    }
    ...
    public function saveProjectData($postData='') {
        ...
        if(!empty($fileData) && is_array($fileData)){
            ...
            if($fileData['projSellstatus'] == 'f') {
                ...
                $fileData['projDonations'] = 't'; // set donation status true for free project
            }
            // set project donation
            $projDonation = (isset($postData['projDonations'])) ? $postData['projDonations'] : 'f';
    ...
    public function membershipcart($arg_list) {
        ...
        if(!empty($indusrtyName) && !empty($projectId) && empty($renewProjectId)) {
            ...
            $this->session->unset_userdata('mediaCartId');
            $this->session->unset_userdata('renewProjectId');
            // set value for free user first stage
			$this->data['addNewContainer'] = true;
        }
        ...
        $this->data['projectId']        = $projectId;
        $this->data['projectType']      = $indusrtyName;
    ...
    public function membershipcartpost() {
        ...
        // set post container id for renew
        $renewProjContainerId = (isset($data['projectContainerId']))?$data['projectContainerId']:'';
        ...
        if(isset($cartId) && !empty($cartId)) {
            ...
            // get renew container project id
			$renewProjectId = $this->session->userdata('renewProjectId');
			if(!empty($renewProjectId) && !empty($renewProjContainerId)) {
				$elementId   = $renewProjectId;
				$nextStep    = 'renewbillingdetails'; // set next step as billing page
				$containerId = $renewProjContainerId;
            ...
            if(!empty($projectContainerId)) {
                ...
                if( !empty($availableContainer) && count($availableContainer) > 0  && empty($renewProjContainerId)) {
    ...
    private function addContainerMediaItem($cartId,$data,$cartValues,$pkgRoleId) {
        ...
        $type = $this->config->item('membership_item_type_1'); // set type as new container
		// get renew container project id
		$renewProjectId = $this->session->userdata('renewProjectId');
		if(!empty($renewProjectId)) {
			$type = $this->config->item('membership_item_type_3'); // set type as new container
		}
    ...
    private function updatebuyerdata($billingDataArray,$billingData,$cartId) {
        ...
        // get renew container project id
		$renewProjectId = $this->session->userdata('renewProjectId');
		if(!empty($renewProjectId) && isset($billingData['isRenewSection'])) {
			$nextStep = '/renewpurchasesummary'; // set next step as purchase summary
		}
    ...
    public function purchasesummary($argList) {
        ...
        $this->data['projectType']      = $argList['indusrty'];
        $this->data['projectId']        = $projectId; 
    ...
    public function paymentsuccess($arg_list) {
        ...
        if(!empty($mediaContainerId)) {
            ...
            if(!empty($addSpaceProjectId) || !empty($renewProjectId)) {
                $projId = $addSpaceProjectId;
                if(!empty($renewProjectId)) {
					$projId = $renewProjectId;
					// update expiry date
					$this->renewcontainerexpiry($indusrtyName);
				}
            ...
            if(!empty($renewProjectId)) {
				$this->data['innerPage']  = 'media/form/renew_payment_success';
				$this->data['renew4menu'] = 'TabbedPanelsTabSelected';
				$this->data['packagestageheading'] = $this->lang->line('renewYourMediaShowcase');
				$this->new_version->load('new_version','form/renew_cart_header',$this->data);
			} else {
    ...
    /*
     * @access: public
     * @description: This function is used to renew container expiry
     * @return void
     */ 
    private function renewcontainerexpiry($indusrtyName='') {
		// set project id in session for renew container
		$renewProjectId  = $this->session->userdata('renewProjectId');
		// get project entity id 
		$entityId = $this->entityId;
		//get logged user proj container data
        $where    = array('entityId' => $entityId,'elementId' => $renewProjectId );
        $containerRes  = $this->model_common->getDataFromTabel('UserContainer', 'expiryDate,userContainerId',  $where);
        
        if(!empty($containerRes) && !empty($renewProjectId)) {
            $expiryDate  = (!empty($containerRes[0]->expiryDate))?$containerRes[0]->expiryDate:currntDateTime();; // set expiry Date
			//set next year date
            $expiryDate = date('Y-m-d h:i:s', strtotime("+12 months ".$expiryDate));
			// update added space
			$this->model_common->editDataFromTabel('UserContainer', array('expiryDate'=>$expiryDate), array('userContainerId' => $containerRes[0]->userContainerId));     
			// send renew email
			$this->renewinvoiceemail($renewProjectId,$indusrtyName,$expiryDate);
    ...
    public function saveCurrency($arg_list) {
        ...
        $nextStep='';
		$projId = $post['projId'];
        if(isset($post['seller_currency']) && is_numeric($post['seller_currency']) && empty($post['isCurrencySet'])) {
        ...
        } else if($post['isCurrencySet'] == 1) {
			
			$seller_currency = LoginUserDetails('seller_currency');
			$this->session->set_userdata(array('seller_currency'	=> $seller_currency));
            $nextStep='setpriceformat'.DIRECTORY_SEPARATOR.$projId;
		}	
        redirectPage($this->router->fetch_class().DIRECTORY_SEPARATOR.$this->router->fetch_method().DIRECTORY_SEPARATOR.$nextStep); 
    }
    ...
    *  @return: void
    */ 
    public function managepublishstatus($arg_list) {
		// get project id
        $projectId = $arg_list[1];
		...
		// set publish value
        $isPublished = ($projRes->isPublished == 'f') ? 't' : 'f';
        
		if(((int)$projectId > 0) ) {
       
            $saveProjData = array('isPublished'=>$isPublished);
            if($isPublished == 't') {
				$saveProjData['isProjectCompleted'] = 't';
			}
            $this->model_common->editDataFromTabel('Project', $saveProjData, 'projId', $projectId);
            ...
            if($isPublished == 't') {
                ...
                $redirectPage = $baseUrl.'/publicise/'.$projectId; 
            } else {
				 $redirectPage = $baseUrl.'/editproject/'.$projectId; 
			}
        }
    ...
    function saveProjectStatus($arg_list){
        ...
        // set base url
		$baseUrl = formBaseUrl();  
        $nextStep = $baseUrl.'/editproject/'.$post['projId'];
        if( isset($post['isPublished']) && ((int)$post['projId'] > 0) ) {
			
            $isPublicise = ($post['isPublished']=='t') ? 't' : 'f';
            if($isPublicise == 't') {
				$nextStep = $baseUrl.'/share/'.$post['projId'];
            }
        }
        redirectPage($nextStep); 
    }
    ...
    public function share($arg_list) {
        ...
        if($projId >= 1){
            ...
            redirectPage($this->router->fetch_class().DIRECTORY_SEPARATOR.$this->router->fetch_method());
        }
    }
    ...
    public function prreinterviews($arg_list) {
        ...
        $this->data['nexturl'] = formBaseUrl().'/editproject/'.$projId;
        $arg_list['mode']='edit';
        $arg_list['loadPage']='form/wizard_additionalInfo';
        $this->loadMediaWizardView($arg_list);
    }
    ...
    */ 
    ...
    public function pickupshipping($arg_list) {
        ...
        $this->wizardheadingtext(); // set heading text of nav bar
        $this->new_version->load('new_version','form/wizard',$this->data);
    ...
    private function manageshippingnexturl($projRes,$pickupData,$isDomestic=0) {
        $nextStep = '';
        $shippingOptions = $projRes->printDeliveryOptions;
        // set element shipping option if element exists
        if(isset($pickupData['elementId']) && !empty($pickupData['elementId'])) {
			$shippingOptions = $projRes->shippingOptions;
		}
    ...
    public function uploadfile($arg_list) {
        ...
        if(!empty($elementData)) {
            ...
            if($indusrtyName == 'educationMaterial') {
                ...
                if($mediaFileType == 0) {
                    redirectPage($this->redirectUrl.'uploadform/'.$projectId.'/'.$elementId);
                }
            }
        }
        //get media file types
        ...
        $this->data['projectId']        =  (isset($projectId))?$projectId:''; // set element file id
        ...
        $this->data['subscriptionType']     =  $subscriptionType; // set subscription type
        $this->data['containerInfo']    =  $containerInfo; // set user's container information
    ...
    */ 
    ...
    private function _ispriceshippingcharge($projectDetails){
        ...
        $projSellType              =  $projectDetails->projSellType;
    ...
    */ 
     ...
     private  function _firstmainelementcreate($arg_list) {
        ...
        if(!empty($getElementId)) {
            ...
            if(empty($elementData)) {
                redirectPage($this->redirectUrl.'/uploadfile/'.$projectId);
            }
        } else {
            $elementPublished = 'f';
            // get main element id if already created
            ...
            if(!empty($elementData)) {
                ...
                $elementAddData     =   array('projId'=>$projectId,'title'=>'Untitled','fileId'=>'0','mediaTypeId'=>NULL,'elementType'=>'0','isPublished'=>$elementPublished);
                $elementId          =   $this->model_common->addDataIntoTabel($elementTable, $elementAddData);
				// set session for edit mode
				//$this->session->set_userdata('isAddElement',1);
                if(!empty($elementId)) {
    ...
    */
    ...
    public function uploadfilepost(){
        ...
        if($this->input->is_ajax_request()) {
            ...
            //next default page url
            $nextUrl  = '/uploadfile/'.$projectId.'/'.$elementId ;
			...
			$prevfileId         =   $this->input->post('fileId'.$browseId);
            $mediaFileData=array();
    ...
    */ 
    ...
    public function uploadimageinfo($arg_list) {
        ...
        $this->data['industry']         = $indusrty; // set industry
    ...
    public function setimageinformation() {
        ...
        if(!empty($postData) && !empty($postData['entityId']) && !empty($postData['elementId'])) {
            // prepare data
            $elementData = array(
                'projGenre'         => (isset($postData['projGenre']) && !empty($postData['projGenre'])) ? $postData['projGenre'] : 0,
                ...
                'projLanguage'      => (isset($postData['projLanguage']))?$postData['projLanguage']:0,
                'productionHouse'   => (isset($postData['productionHouse']))?$postData['productionHouse']:'',
            );
            // set realese date
            ...
            // set industry id for education material
            if(isset($postData['industryId']) && !empty($postData['industryId'])) {
                $elementData['industryId'] = $postData['industryId'];
				$elementData['wordCount']  = (!empty($postData['wordCount']))?$postData['wordCount']:0;
            }
        ...
        if(!empty($teamElementId) && !empty($elementId) && !empty($entityId)) {
            ...
            if(!empty($elementRes) && count($elementRes) > 0)  {
                ...
                $elementData = array(
                    ...
                    'projLanguage'      => $elementRes->projLanguage,
                    'productionHouse'   => $elementRes->productionHouse,
                );
                // update element information 
   ...
   //-----------------------------------------------------------------------
     ...
     * @return void
     */ 
    public function savedomesticprimarydata($arg_list) {
        ...
        set_global_messages($msg, $type, $is_multiple=true);
        echo json_encode(array('nextStep'=>''));
    }
    ...
    public function internationalshipping($arg_list) {
        ...
        if(!empty($postData) && $postData['isCopy'] == 't') {
            ...
            if(isset($spId) && !empty($spId)) {
                redirectPage($this->redirectUrl.'addshippingzone/'.$projectId.'/'.$spId.'/'.$elementId);
            }
        }
    ...
    public function saveshippingzone($arg_list) {
        ...
        if(!empty($zoneData)) {
            ...
            if(!empty($spId)) {
                ...
                redirectPage($this->redirectUrl.'addshippingzoneprice/'.$projectId.'/'.$spId.'/'.$elementId);
            } else {
                redirectPage($this->redirectUrl.'internationalshipping/'.$projectId.'/'.$elementId);
            }
        } else {
           redirectPage($this->redirectUrl.'internationalshipping/'.$arg_list[1].'/'.$arg_list[2]);
        }
    }
    ...
    public function addshippingzoneprice( $arg_list ) {
        ...
        if($interationalShipping) {
            ...
            redirectPage($this->redirectUrl.'internationalshipping/'.$elementId);
        }
    }
    ...
    public function sellerconsumptiontax( $arg_list ) {
        ...
        $this->data['ispriceShippingCharge']   = $this->_ispriceshippingcharge($projRes); // get price shipping page is show
        $this->data['sellPriceType']   =  $projRes->sellPriceType;
        $this->data['userId'] = $userId;
   ...
   //-----------------------------------------------------------------------
    ...
    public function saveConsumptionTax($arg_list) {
        ...
        if(!empty($consumptionCharge) || !empty($consumptionStateTax)) {
            ...
            redirectPage($this->redirectUrl.'sellerpaypal/'.$arg_list[1]);
        }
    ...
    private function manageConsumptionStateTax($userSellerData) {
        ...
        if(is_array($StateWise) && count($StateWise) > 0){
            ...
            if(is_array($ConsumptionTax)) {
                ...
                $msg = 'Please fill Tax name and pecentage details!';
                set_global_messages($msg, $type='error', $is_multiple=true);
                redirectPage(site_url().'dashboard/globalsettings/4');
            }
            /* update user seller territory data */
   ...
   //-----------------------------------------------------------------------
    ...
    private function isprojectexists( $projectId , $industry = '') {
        ...
        if(!empty($projectId) && is_numeric($projectId)) {
            ...
            if(isset($projRes[0]->projId) && $projRes[0]->projId > 0)  {
                ...
                $this->data['projCategory']  = $projRes[0]->projCategory;
                return $projRes[0];
             } else {
                set_global_messages($msg, $type='error', $is_multiple=true);
                redirectPage($this->redirectUrl);
    ...
    public function setselectedimage( $arg_list ) {
        ...
        if(!empty($imagePost) && !empty($imagePost['album_image'])) {
            ...
            if(!empty($elementId)) {
                redirectPage($this->redirectUrl.'newsreviewtitlendesc/'.$projectId.'/'.$elementId);
            } else {
                redirectPage($this->redirectUrl.'selecttitlendesc/'.$projectId);
            }
        } else {
            redirectPage($this->redirectUrl);
        }
    ...
    public function setcollectioninfo( $arg_list ) {
		...
		if($this->input->is_ajax_request()) {
			...
			$nextStepUrl = formBaseUrl().$nextStep;
			 redirectPage($nextStepUrl);
		}
    }
    ...
    public function selectcreativeteam( $arg_list ) {
        ...
        $this->data['creativeElementRes']   = $creativeElementRes;
        $this->data['projectId']        = $projectId;
        $this->data['projectElementId'] = $projectId;
        $this->data['entityId']         = $this->entityId;
        $this->data['indusrty']         = $indusrty;
        $this->data['creativesCount']   = $creativesResultCount;
        $this->data['innerPage']        = 'media/form/design_cover_page';
    ...
    private function manageMemberRowHtml($crtId,$postData,$Membertype=0) {
        ...
        $rowBgCls = 'bg_f9f9f9';
        $rowPLCls = 'pl23';
        $divId = 'creativeTeam_'.$crtId;
        if($Membertype == 1) {
		  $divId = 'toadcreativeTeam_'.$crtId;
            //$rowBgCls = '';
            //$rowPLCls = 'pl5';
        }
    ...
    */
    ...
    public function uploadelementfilepost($arg_list) {
        ...
        $isExternalFile     =   false;
         ...
         //next default page url
            $nextUrl  = '/setdisplayimage/'.$projectId.'/'.$elementId.'/1';
            $msg = 'Check your uploaded file url';
            $type = 'error';
    ...
    public function displayimageoption($arg_list) {
        ...
        // set form parametres
        $this->data['defaultImageTitle']  = $this->config->item(strtoupper($elementTblPrefix).'TYPE');
    ...
    public function setdisplayimageoption() {
        ...
        redirectPage($this->redirectUrl.$nextUrl);
    }
    ...
    function nextmediaoptions($arg_list) {
        ...
        switch ($indusrtyName)
        {
            ...
            case 'photographyNart':
                ...
                // $projRes->projCategory == 7 for photography album
                if( $projRes->projCategory == 7 ) {
                    $addAnotherMedia = $this->lang->line('addAnotherMediaPhotography');
                    $designCoverPage = $this->lang->line('designCoverPagePaidPA');
                    if($subscriptionType == 1) {
                        $designCoverPage = $this->lang->line('designCoverPageFreePA');
                    }
                }
    ...
    */ 
    private function addprojectelement($projId,$indusrtyName,$elementType=0) {
        ...
        // create element of project
        $elementAddData   =   array('projId'=>$projId,'fileId'=>'0','title'=>'Untitled','mediaTypeId'=>NULL,'elementType'=>$elementType,'isPublished'=>$elementPublished);
        $elementId        =   $this->model_common->addDataIntoTabel($elementTable, $elementAddData);
        if(!empty($elementId)) {
            // add element record in summary log
            $insertedLogId = addDataIntoLogSummary($elementTable,$elementId);
            // manage log  count
    ...
    */ 
        ...
        if(!empty($postData)) {
            ...
            if(!empty($elementId)) {
                ...
                if(isset($shippableStatus) && $shippableStatus != $elementData['isShippable']) {
                    ...
                    mediaFileCount($this->entityId,$projectId,$indusrtyName,$shippableStatus,$mediaFileType,'',$elementId);
                }
            } else {
                ...
                addDataIntoLogSummary($elementTable,$elementId);
                // update media file count tosif
                mediaFileCount($this->entityId,$projectId,$indusrtyName,$isShippable,$mediaFileType,'',$elementId);
            }
           
            // redirect to upload file form
            redirectPage($baseUrl.'/uploadfile/'.$projectId.'/'.$elementId);
        } else {
            // redirect if element not found
            redirectPage($baseUrl);
        }
    ...
    public function publicisemediacollection($arg_list) {
        // unset edit mode session
		$this->session->unset_userdata('isEditMedia');
		// unset session value of element add status
		$this->session->unset_userdata('isAddElement');
    ...
    /*
     * @Description: This method is use to get incomplete media collection for edit
     * @access: public
     * @return: void
     */
     
	public function completeyourcollection($arg_list) {
		// unset edit mode session
		$this->session->unset_userdata('isEditMedia');
		// unset session value of element add status
		$this->session->unset_userdata('isAddElement');
		 // set incomplete  project section
        $this->data['isIncompleteSection'] = 1;
        // manage all media projects
        $this->getmediacollection($arg_list,0,1);
    }
    ...
    public function getmediacollection($arg_list,$isPublished='',$isIncompleteProj='') {
        ...
        $publiciseCheck = isset($this->data['isPubliciseSection'])?$this->data['isPubliciseSection']:'';
        
        if($indusrtyName != 'news' && $indusrtyName != 'reviews') {
			$this->data['packagestageheading'] = $this->lang->line('editYourMediaShowcase'.$publiciseCheck);
			if(!empty($isIncompleteProj)) { 
            ...
            $this->data['mediaCollectionResult'] = $this->getmediacollectionresult(true,$indusrtyName,$isPublished,$isIncompleteProj);
            $this->new_version->load('new_version','form/media_incomplete_collection',$this->data);
        } else {
            redirectPage(base_url(lang().'/home'));
        }
    }
     ...
     * @return: void
     */
    public function getmediacollectionresult($loadView=false,$indusrtyName='',$isPublished='',$isIncompleteProj='') {
        
        $isSetCookie = setPerPageCookie('searchPerPageVal',$this->config->item('perPageRecord'));
        $pages = new Pagination_new_ajax;
        // get project's list
        $projResCounts = $this->model_media->getprojectrecords($indusrtyName,$isPublished,$isIncompleteProj);
        ...
        $projRes = $this->model_media->getprojectrecords($indusrtyName,$isPublished,$isIncompleteProj,$pages->limit, $pages->offst);
        $this->data['items_total'] = $pages->items_total;
        $this->data['items_per_page'] = $pages->items_per_page;
        $this->data['pagination_links'] = $pages->display_pages();
        ...
        //$this->data['projectListingData']   = (object)$projRes;
		$this->data['projectListingData']   = $projRes;
		
        $searchResultView = $this->load->view('form/media_incomplete_collection_result',$this->data,$loadView);
        if($loadView){
    ...
    public function editproject($arg_list) {
        ...
        if(!empty($projectId) && $indusrtyName) {
            ...
            $logSummryDta = $this->model_common->getDataFromTabel('LogSummary','craveCount,viewCount,ratingAvg,reviewCount,imageFileCount,videoFileCount,audioFileCount,docFileCount,cdCount,dvdCount,printCount,docCount',array('entityId'=>$projectEntityId,'elementId'=>$projectId), '','','',1);
            ...
            // get container details
            $containerInfo = getUserContainerSpace($this->dirUser, $this->userId, $subscriptionType);
            // get blocked element list
            $blockedElements = $this->model_common->getDataFromTabel($elementTable, 'elementId,title',  array('projId'=>$projectId,'isBlocked'=>'t'),'','','');
            // set media edit session
            $this->session->set_userdata('isEditMedia',1);
            ...
            $this->data['blockedElements']    = $blockedElements;
            ...
            redirectPage($this->redirectUrl.'/editmediacollection');
        }
    }
    ...
    public function editmediaelements($arg_list) {
        ...
        // check project id is exists or not
        $projRes = $this->isprojectexists( $projectId, $indusrtyName);
		// unset session for edit mode
		$this->session->unset_userdata('isAddElement');
        $this->data['packagestageheading'] = $this->lang->line('editYourMediaShowcase');
       
		$this->data['elementCollectionResult'] = $this->getmediaelementresult(true,$indusrtyName,$projectId);
     ...
     * @return: void
     */
    public function getmediaelementresult($loadView=false,$indusrtyName='',$projectId='') {
        
		// get element's table name
        $elementTblPrefix = $this->config->item( $indusrtyName.'Prifix');
        $elementTable = $elementTblPrefix.'Element';
       
        $projResCounts = $this->model_media->getelementrecords($elementTable,$indusrtyName,$projectId);
       
        $pages                           =  new Pagination_new_ajax;
        $pages->items_total              =  count($projResCounts); // this is the COUNT(*) query that gets the total record count from the table you are querying
        $this->data['perPageRecord']           =  $this->config->item('perPageRecord');
       
        
         // Add by Amit to set cookie for Results per page
        if($this->input->post('ipp')!=''){
            $isCookie = setPerPageCookie('MediaElementPerPageVal',$this->data['perPageRecord']);	
        }else {
            $isCookie = getPerPageCookie('MediaElementPerPageVal',$this->data['perPageRecord']);		
        }
        ...
        $elementRes = $this->model_media->getelementrecords($elementTable,$indusrtyName,$projectId,$pages->limit, $pages->offst);
        ...
        */ 
        ...
        $this->data['elementRes']   = $elementRes;
		$this->data['indusrtyName'] = $indusrtyName;
		$this->data['projectId']    = $projectId;
        $searchResultView = $this->load->view('form/media_element_edit_collection_result',$this->data,$loadView);
        if($loadView){
            return $searchResultView;
        }
    }
    ...
    /*
    * @description: This method is use to  reset review
    * @auther: lokendra meena
    * @return: void
    */ 
    
    private function _resetreviewsession(){
        //unset all session
        $this->session->unset_userdata('newsreviewprojectId');
        $this->session->unset_userdata('newsreviewelementId');
        $this->session->unset_userdata('newsreviewentityId');
        $this->session->unset_userdata('newsreviewindustryId');
    }
    
    //-------------------------------------------------------------------------
    
    /*
    * @description: This method is use to  add review
    * @auther: lokendra meena
    * @return: void
    */ 
    ...
    public function addreviews(){
        ...
        //reset news & review session
        $this->_resetreviewsession();
        
        //redirect to add review page
        redirectPage(base_url_lang('media/reviewswizard'));
    }   
    ...
    /*
    * @description: This method is use to  add news
    * @auther: lokendra meena
    * @return: void
    */ 
    ...
    public function addnews(){
        ...
        redirectPage(base_url_lang('media/newswizard'));
    }  
    ...
    public function savenewsnreviews($arg_list) {
        ...
        // get media industry type
        $indusrty = $arg_list['indusrty'];
        // For add review  default value intailization     
        $reviewProjectId           =  '0'; //set default zero
        $reviewElementId           =  '0'; //set default zero
        $reviewEntityId            =  '0'; //set default zero
        $reviewIndustryId          =  '0'; //set default zero
        $reviewUserId              =  '0';  //set default zero
      
      
      // Edit mode of user
        if(!empty($elementId))
        {
            $getelementData = $this->model_common->getDataFromTabel('ReviewsElement','*',array('elementId'=>$elementId),'','','');
            $reviewProjectId           =  $getelementData[0]->projId; 
            $reviewElementId           =  $getelementData[0]->projectElementId; 
            $reviewEntityId            =  $getelementData[0]->entityId; 
            $reviewIndustryId          =  $getelementData[0]->industryId; 
            $reviewUserId              =  $getelementData[0]->userId;  
        }
        
        //check user review - Frontend user 
        if($this->session->userdata('newsreviewprojectId')){
            $reviewProjectId           =  $this->session->userdata('newsreviewprojectId');
            $reviewElementId           =  $this->session->userdata('newsreviewelementId');
            $reviewEntityId            =  $this->session->userdata('newsreviewentityId');
            $reviewIndustryId          =  $this->session->userdata('newsreviewindustryId');
            $reviewUserId              =  $this->userId;
        }
        ...
        if(!empty($postData)) {
            ...
            //insert one time for managing your toadsquare menu for news & review
            $inserData    =   array(
                        'entityid'              =>  getMasterTableRecord('Project'),
                        'elementid'             =>  $projectId,
                        'projectid'             =>  $projectId,
                        'section'               =>  $arg_list['indusrty'],
                        'sectionid'             =>  $arg_list['sectionId'],
                        'sectionParent'         =>  $arg_list['indusrty'],
                    );
            ...
            if(!empty($projectId)) {
                ...
                $elementData = array(
                    ...
                    'projectElementId'  => $reviewElementId,
                    'entityId'          => $reviewEntityId,
                    'industryId'        => $reviewIndustryId,
                    'userId'            => $reviewUserId,
                );
                if($indusrty == 'reviews') {
                    $elementData['title_subjects'] = $postData['articleSubject'];
                }
                // set element's table prefix
                $elementTblPrefix = $this->config->item($indusrty.'Prifix');
                $elementTable = $elementTblPrefix.'Element';
                ...
                if(!empty($elementId)){
                    ...
                    //if industry element & project id
                    if(!empty($reviewElementId)){
                        $this->model_media->checkLogSummaryNew($reviewEntityId,$reviewElementId,$reviewProjectId);
                    }
                }
    ...
    public function uploaddisplayimage($arg_list) {
        ...
        //call current stage update for review & news complete
        $dataArray = array('entityid'=>getMasterTableRecord('Project'),'projectid'=>$projectId);
    ...
    */
    ...
    public function newsreviewfilepost($arg_list) {
        ...
        //next default page url
        $nextUrl  = '/uploaddisplayimage/'.$projectId.'/'.$elementId.'/1';
        $msg = 'Invalid Embed File';
        $type = 'error';
    ...
    /* Genre listing with respect to industry type */
    ...
    public function getgenrelisting() {
        ...
        {    
            $genreHtml = '<ul class="billing_form width169 mt2 pl10 fl">
                          <li class="select">';
            $genreHtml.= form_dropdown('genreId', $genre, '','id="genreId" class="required width_190" ');
            $genreHtml.= '</li></ul>';
            $genreHtml.=" <script>$('SELECT').selectBox(); </script>";
            $result= 1;
        }
        echo json_encode(array('result'=>$result,'html'=>$genreHtml));
    }
    ...
    public function setnewsreviewinformation() {
        ...
        if(!empty($postData) && !empty($postData['entityId']) && !empty($postData['elementId'])) {
            ...
            //$industryId = ($this->session->userdata('reviewindustryId'))?:$postData['industryId'];
            
            // prepare data
            $elementData = array(
                'industryId'         => (!empty($postData['industryId']))?$postData['industryId']:0,
    ...
    */ 
    ...
    public function publishcollection( $arg_list ) {
        ...
        $this->data['industry']         = $indusrty;
        $this->data['userId']           = $this->userId;
        $this->data['isPublished']      = $projRes->isPublished;
    ...
    public function publicisecollection($arg_list) {
        ...
        $whereCondi         =   array('entityid'=>getMasterTableRecord('Project'),'projectid'=>$projectId);
        ...
        $type = 'success';
        set_global_messages($msg, $type, $is_multiple=true);
        //redirectPage(base_url(lang().'/media/'.$indusrty.'/'.$projectId));
		redirectPage(base_url(lang().'/media/editwizardcollection/'.$indusrty));
    }
    ...
    public function manageelementshortlink($arg_list,$projId) {
        ...
        if(!empty($elementId)) {
            // check project id is exists or not
            if($arg_list['indusrty'] == 'news' || $arg_list['indusrty'] == 'reviews') {
                $this->checknewsnreview($arg_list['indusrty'],$projId,$elementId);
            } else {
                $this->isprojectelementexists( $projId,$elementId,$arg_list['indusrty'] ); 
            }
        } else {
            // get project's elements list
             if($arg_list['indusrty'] == 'news' || $arg_list['indusrty'] == 'reviews') {
                $projElementRes = $this->model_common->getDataFromTabel($elementTable, 'elementId',  array('projId'=>$projId,'isPublished'=>'t','isArchive'=>'f','isBlocked'=>'f'),'',$orderBy='elementId',$order='DESC', $limit=1);
    ...
    public function getgenreselectbox() {
        ...
        $stateHtml = '';
         if($projType > 0) {
            ...
            $genreHtml .= form_dropdown('projGenre', $subgenre,'','id="projGenre" class="required width_190" ');
             /* add default checkbox script */
    ...
    /*
     * @access: public
     * @description: This function is used to manage renew container
     * @return void
     */ 
    public function renewmediacart($arg_list) {
        ...
        if(!empty($indusrtyName) && !empty($projectId)) {
            ...
            $projRes = $this->isprojectexists( $projectId, $indusrtyName);
            // set project id in session for add space
            $this->session->set_userdata('renewProjectId',$projectId);
            // set project status in session
            $this->session->set_userdata('projSellStatus',$projRes->projSellstatus);
        ...
        $this->data['subscriptionType'] = $subscriptionType;
		$this->data['indusrtyName']     = $indusrtyName;
        $this->data['innerPage']        = 'media/form/renew_cart_details';
        $this->data['renew1menu']       = 'TabbedPanelsTabSelected';
		$this->data['projectId']        = $projectId;
		$this->data['packagestageheading'] = $this->lang->line('renewYourMediaShowcase');
		$this->new_version->load('new_version','form/renew_cart_header',$this->data);
    }
    ...
    /*
     * @access: public
     * @description: This function is used to manage billing information for renew
     * @return string
     */ 
    public function renewbillingdetails($argList) {
        ...
        $this->data['userProfileData'] = $userProfileData; # set user profile data 
        $this->data['innerPage'] = 'media/form/renew_billing_details';
        $this->data['renew2menu'] = 'TabbedPanelsTabSelected';
		$this->data['packagestageheading'] = $this->lang->line('renewYourMediaShowcase');
		$this->new_version->load('new_version','form/renew_cart_header',$this->data);
    }
    ...
    /*
     * @access: public
     * @description: This function is used to show purchase summary for renew
     * @return string
     */ 
    public function renewpurchasesummary($argList) {
        ...
        $spacePrice = 0;
        $containerPrice = 0;
        if(!empty($cartId) && !empty($renewProjectId)) {
            // get membership cart data
            $cartData =  $this->model_common->getDataFromTabel('MembershipCart', '*',  array('cartId' => $cartId),'','','',1);
            $buyerBillingData = '';
            if(!empty($cartData)) {
			...
			$indusrtyName = $arg_list['indusrty'];
			// check project id is exists or not
            $projRes = $this->isprojectexists( $renewProjectId, $indusrtyName);
        ...
        $this->data['totalPrice']       = $totalPrice;
        $this->data['vatPrice']         = $vatPrice;
        $this->data['projRes']          = $projRes;
        $this->data['containerPrice']   = $containerPrice;		
        $this->data['buyerSettingData'] = $buyerBillingData;
		$this->data['projectId']        = $renewProjectId;
        $this->data['userSellerData']   = (!empty($userSellerData[0]))?$userSellerData[0]:'';
        $this->data['innerPage'] = 'media/form/renew_purchase_summary';
        $this->data['renew3menu'] = 'TabbedPanelsTabSelected';
        $this->data['packagestageheading'] = $this->lang->line('renewYourMediaShowcase');
		$this->new_version->load('new_version','form/renew_cart_header',$this->data);
  ...
  */  
  ...
  public function renewinvoiceemail($projectId=0,$indusrtyName='',$expiryDate='')
  {
		// check project id is exists or not
		$projRes = $this->isprojectexists( $projectId, $indusrtyName);
		//tool purchase prepare email body and subject
		$whereCondi         =  array('purpose'=>'renew_container','active'=>1);
		...
		$expiry_date        =  date('d F Y',strtotime($expiryDate));
		$project_name       =  $projRes->projName; 
		/* Set Follow us link*/
		...
		$invoice_url        =  base_url().'package/purchases';
		$dashboard_url      =  base_url('dashboard');
		$toolBody           =  $toolPurchaseBody->templates;
		$getArray           =  array("{crave_us}","{facebook_url}","{linkedin_url}","{twitter_url}","{gPlus_url}","{invoice_name}" ,"{purchase_page_url}" , "{dashboard_url}" ,"{site_url}" , "{site_base_url}", "{image_base_url}","{expiry_date}");
		$replaceArray       =  array($crave_us,$facebook_url,$linkedin_url,$twitter_url,$gPlus_url,$invoice_name, $invoice_url, $dashboard_url, $site_url,$site_base_url,$image_base_url,$expiry_date);
		...
		//$userEmail =  'tosifqureshi@cdnsol.com';
        //send email to  
  ...
  //----------------------------------------------------------------------
	...
	public function setmediaaddtype() {
		// get post data
		$postData = $this->input->post();
		$returnUrl =  base_url(lang().'/home');
		if(!empty($postData)) {
			// get media section name
			$section = $this->config->item('industryShowcaseSectionId'.$postData['mediaType']);
			// set media showcase path
			$returnUrl =  base_url(lang().'/media/'.$section);   
		}
    ...
    /*
     * @Description: This method is use to manage convertion
     * @access: public
     * @return: void
     */
    public function checkConvertion() {
		// get project id
        $projId = $this->input->post('projId');
		// get industry name
		...
		// get element's table name
        $elementTblPrefix = $this->config->item( $industry.'Prifix');
        $elementTable = $elementTblPrefix.'Element';
		$status = false;
		$fileType = 'video'; // default type of uploading files
        if(!empty($projId)) {
            // get element file records
            $elementRecords = $this->model_media->getElementsMediaRecord($projId,$elementTable);
            if(!empty($elementRecords) && count($elementRecords) > 0) {
				$uploadStatus = $this->searchconvertstatus($elementRecords);
				if($uploadStatus == 1) {
					$status = true;
				}
				// set uploading file type
				if($elementRecords[0]->fileType == 1) {
					$fileType = 'image';
				} else if($elementRecords[0]->fileType == 3) {
					$fileType = 'audio';
				} else if($elementRecords[0]->fileType == 4) {
					$fileType = 'text';
				}
			}  
        } 
        
        echo json_encode(array('status'=>$status,'fileType'=>$fileType));
	}
	
	function test() {
		$getPendingConvertionFiles = $this->model_media->getElementsMediaRecord(544,'FvElement');
		
		//$camera = $this->searchconvertstatus($userSellerData);
		echo '<pre>';
		print_r($getPendingConvertionFiles);die;
    ...
    /*
     * @Description: This method is use to check status of uploadded element file
     * @access: public
     * @return: void
     */
	private function searchconvertstatus($exif) {
		
		foreach ($exif as $data) {
			
			if( $data->isExternal != 'f' || $data->jobStsatus == 'FAILS' || $data->jobStsatus == 'UPLOADING' || $data->jobStsatus == 'INPROGRESS' ) {
				return 0;
			}
		}
		return 1;
    ...
    /*
    *  @access: public
    *  @description: This method is use to show renew project list
    *  @auther: lokendra meena
    *  @return: void
    */ 
    
    public function  renewmedialist($arg_list){
        ...
        if($indusrtyName != 'news' && $indusrtyName != 'reviews') {
            ...
            $pages = new Pagination_new_ajax;
            // get project's list
            $projResCounts = $this->model_media->renewprojectrecords($indusrtyName,true);
            ...
            $pages->paginate();
            // get project's records list
            $projRes = $this->model_media->renewprojectrecords($indusrtyName,true,$pages->limit, $pages->offst);
            ...
            $this->data['industryId'] = $industryId; // set media industry id
            $this->data['indusrty'] = $indusrtyName; // set media industry type
            $this->data['indusrtymethod'] = $this->uri->segment('3'); // set method name industry type
            $this->data['projList']   = (object)$projRes;
           
            $ajaxRequest = $this->input->post('ajaxRequest');
            
            if($ajaxRequest){
                $this->load->view('form/media_renew_collection_view',$this->data);
            }else{
                $this->new_version->load('new_version','form/media_renew_collection',$this->data);
            }
    ...
    /*
    *  @access: public
    *  @description: This method is use to show preview page as coming soon
    *  @auther: tosif qureshi
    *  @return: void
    */ 
    public function preview() {
		$this->data['comingsoon'] = array('msg'=>'"Coming <br/> Soon"');
		$this->new_version->load('new_version','landingpage/comingsoon',$this->data);
	}
    
    //----------------------
    
    public function projunpublish(){
        
        $projectIdArray = array(342,338,345,351,418,439,489,525,529);
        
        if(!empty($projectIdArray)){
            foreach($projectIdArray as $projectId){
                $billingDataArray = array('isPublished'=>'f');
                $this->model_common->editDataFromTabel('Project', $billingDataArray, 'projId', $projectId);
                $this->model_common->editDataFromTabel('FvElement', $billingDataArray, 'projId', $projectId);
            }
        }
        echo "Update Projec Status";
    }
    ...
    *  @return: string
    */ 
    public function updatecurrentstage($currentStage='',$methodName='',$projectId=0) {
		if(!empty($currentStage) && ((int)$projectId > 0) && $methodName != 'editmediaelements' && $methodName != 'editproject') {
			$post = $this->input->post();
            if(!empty($post)){
                $currentStage = DIRECTORY_SEPARATOR.$this->router->fetch_class().DIRECTORY_SEPARATOR.$this->router->fetch_method().DIRECTORY_SEPARATOR.$currentStage;
                // set update data
                $updateData = array('currentStage'=>$currentStage);
                // update current stage
                $this->model_common->editDataFromTabel('Project', $updateData, 'projId', $projectId);
            }
		}
	}
    ...
    /*
     * @description: This function is used to get listing of not convertes elements
     * @access: public
     * @return void
     */ 
    public function pendingconversion($arg_list) {
		
		// set industry name
        $indusrtyName = $arg_list['indusrty'];
        $fileType = 0;
        if( $indusrtyName == 'filmNvideo' ) {
			$fileType = $this->config->item('media_type_video');
		} elseif( $indusrtyName == 'musicNaudio' ) {
			$fileType = $this->config->item('media_type_audio');;
		} elseif( $indusrtyName == 'writingNpublishing' ) {
			$fileType = $this->config->item('media_type_document');;
		}
        ...
        // get element file records
		$pendingRecords = $this->model_media->getPendingConvertionFiles($this->userId,$elementTable,$fileType);
		
		//call method for plupload css and js add
        $this->_pluploadjsandcss();
        //call method for required data for upload file
        $this->_uploadfilerequiredata($arg_list,$mediaFileType=0);
        
        $this->data['dirUploadMedia']     = $this->dirUploadMedia.$indusrtyName.DIRECTORY_SEPARATOR;
        $this->data['elementEntityId']    = $elementEntityId=getMasterTableRecord($elementTable);
        $this->data['indusrtyName']       = $indusrtyName;
        $this->data['userId']             = $this->userId;
        $this->data['sectionId']          = $arg_list['sectionId'];
        $this->data['elementTable']       = $elementTable;
        $this->data['pendingRecords']     = $pendingRecords;     
        $this->data['packagestageheading']= $this->lang->line('editYourMediaShowcase');
        $this->new_version->load('new_version','form/pending_conversion',$this->data);
    ...
    /*
    *  @description: Tthis functino is use to user review
    *  @auther: lokendra meena
    *  @return: void
    */ 
    
    public function userreviewnews(){
        
        //set default array
        $returnArray  = array('isshowcase'=>false,'issuccess'=>false);
        ...
        if($this->input->post('projectId')){
            ...
            //get user showcase data
            $userShowcaseData = getUserShowcaseId($this->userId);
            
            if(!empty($userShowcaseData->tagwords) || !empty($userShowcaseData->promotionalsection)){
                //set data for review
                $this->session->set_userdata('newsreviewprojectId',$projectId);
                $this->session->set_userdata('newsreviewelementId',$elementId);
                $this->session->set_userdata('newsreviewentityId',$entityId);
                $this->session->set_userdata('newsreviewindustryId',$industryId);
                
                $returnArray  = array('isshowcase'=>true,'issuccess'=>true);
            }else{
                $returnArray  = array('isshowcase'=>false,'issuccess'=>true);
            }
        }else{
            $returnArray  = array('isshowcase'=>false,'issuccess'=>false);
        }
    ...
    /*
    *   @description: This method is use show showcase popup 
    *   @auther: lokendra meena
    *   @return: void
    */
    
    public function showcasecreatepopup(){
        $data['isReview'] = $this->input->get('val1');
        $this->load->view('showcasecreatepopup',$data);
    }
    ...
    /*
    *   @description: This method is check embeded video 
    *   @auther: Amit Neema
    *   @return: void
    */
    
    public function embededVideoCheck(){
    ...
    /* @description: This method is check embeded video 
    *   @auther: Amit Neema
    /* to check paypal Detail of user */
    
    public function checkpaypalDetail()
    {
        $res=$this->model_media->checkPaypalDetail($this->userId);
        if($res)
        echo json_encode(array('result'=>1));
        else
        echo json_encode(array('result'=>0));

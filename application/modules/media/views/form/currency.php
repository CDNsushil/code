...
$isCurrencySet = false;
$currencyText = $this->lang->line('whatSellerCurrency');

if(isset($userProfileData->seller_currency) && $userProfileData->seller_currency != '') {
	$currency = $userProfileData->seller_currency;
	switch($currency) {
		case 1: 
			$currencyText ='You are selling in US Dollars $';
		break;

		default: 
			$currencyText ='You are selling in Euros €';
		break;
	}
	$isCurrencySet = true;
}

$isCurrencySetInput = array(
	'name'	=> 'isCurrencySet',
	'type'	=> 'hidden',
	'id'	=> 'isCurrencySet',
	'value'	=> $isCurrencySet
...
echo form_open(base_url(lang().DIRECTORY_SEPARATOR.$this->router->fetch_class().DIRECTORY_SEPARATOR.$this->router->fetch_method().DIRECTORY_SEPARATOR.'saveCurrency'.DIRECTORY_SEPARATOR.$projId),$currencyForm); 
	...
	echo form_input($isCurrencySetInput);
    ?>
	<div class="c_1 ">
		<h3 class="red fs21 fnt_mouse  bb_aeaeae width635 whitespace_now"><?php echo $currencyText;?></h3>
		<div class="sap_30"></div>
		<?php if(empty($isCurrencySet)) { ?>

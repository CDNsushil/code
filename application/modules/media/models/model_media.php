...
class model_media extends CI_Model {
	...
	public function checkLogSummaryNew($entityId,$elemId,$proId){
				...
				if($table != 'Posts' && $table != 'Blogs') {
					if(strstr($table,'Project')){
						...
						if(strstr($table,'Events')){
							...
							$whereCondition=array('elementId'=>$res['elementid_from']);
    ...
    /*
    *  @access: public
    *  @description: This method is use to show free file listing
    *  @auther: lokendra meena
    *  @return: void
    */ 
    
    public function freedonwloadfilelist($FD =''){
        ...
        if(isset($FD['elementTable'])){
            ...
            $this->db->where($FD['elementTable'].".elementType",0);
    ...
    */ 
    ...
    public function paiddownloadfiles($FD =''){
    ...
    */ 
    ...
    public function ppvfilelisting($FD =''){
    ...
    */ 
    ...
    public function projectlist($userId,$projectType,$projectId=0,$isArchive='f',$offSet=0,$perPage=0){
        ...
        //if preview true then show published/unpublisehd project will show
        if(!previewModeActive()){
            $this->db->where($this->tableProject.'.isPublished','t');
        }
        
        //$this->db->where($this->tableProject.'.isProjectCompleted','t');
    ...
    */ 
    ...
    public function editprojectlist($userId,$projectType,$projectId=0,$isArchive='f',$offSet=0,$perPage=0){
    ...
    */
    ...
    public function projectelementslist($projId=0,$elementTable, $elementId=0)
    {
        ...
        //if preivew true then  published/unpublished will show
        if(!previewModeActive()){
    ...
    */ 
    ...
    //----------------------------------------------------------------------
     ...
     function getAllReviewNew($entityId,$projectElementId,$reviewEntityId,$offSet=0,$perPage=0) {
        ...
        $this->db->where($this->tableLogSummary.'.entityId',$entityId);  // review entity id
        $this->db->where($this->tableReviewsElement.'.isPublished','t');
        $this->db->order_by($this->tableReviewsElement.'.elementId','desc'); 
    ...
    * @return array
    */ 
    public function getprojectrecords($industry,$isPublished='',$isIncompleteProj='',$limit=0,$offset=0) {
		$table              =   $this->db->dbprefix($this->tableProject); //get db prefix
        $entityId           =   getMasterTableRecord($table);	// get entityId of project table
		$tableLogSummary    =   $this->db->dbprefix($this->tableLogSummary); // get db prefix
        $userId = isLoginUser();
        ...
        $this->db->where($this->tableProject.'.projectType', $industry);
        $this->db->where($this->tableProject.'.tdsUid', $userId);
        if(!empty($isPublished) && $isPublished > 0 ) {
            $this->db->where($this->tableProject.'.isPublished', 't');
        }
         if(!empty($isIncompleteProj)) {
            $this->db->where($this->tableProject.'.isProjectCompleted', 'f');
        }
    ...
    */ 
    ...
    public function myplaylist($userId=0, $limit=0, $offset=0, $retrunRow=false){
    ...
    /* 
    * @access: public
    * @description: This function is used to get elements's data 
    * @return array
    */ 
    public function getelementrecords($elementTable,$indusrtyName,$projId=0,$limit=0,$offset=0) {
        $userId = isLoginUser();
        if($indusrtyName == 'reviews' || $indusrtyName == 'news') {
			$this->db->select('*');
		} elseif($indusrtyName == 'educationMaterial') {
			$this->db->select('elementId,title,imagePath,displayImageType,createdDate,mediaFileType,isShippable,isPrice');
		} else {
			$this->db->select('elementId,title,imagePath,displayImageType,createdDate,isShippable,isPrice');
		}
        $this->db->from($elementTable);
    ...
    * @return array
    */ 
    public function renewprojectrecords($industry,$isPublished='',$limit=0,$offset=0) {
        $userId = isLoginUser();
        $this->db->select($this->tableUserContainer.'.*');
        $this->db->select($this->tableProject.'.*');
        $this->db->from($this->tableProject);
    ...
    /* 
    * @access: public
    * @description: This function is used to get project's all element media files
    * @return array
    */ 
    function getElementsMediaRecord($projId,$elementTable)  {
		
		$this->db->select($elementTable.'.elementId,projId,title,isPublished');
		$this->db->select($this->tableMediaFile.'.fileId,filePath,fileName,fileType,isExternal,jobStsatus,statusDetail');
        $this->db->from($elementTable);
		$this->db->join($this->tableMediaFile, $this->tableMediaFile.'.fileId  = '.$elementTable.'.fileId','left');	
        $this->db->where($elementTable.'.projId', $projId);
    ...
    * @return array
    */ 
    function getPendingConvertionFiles($userId,$elementTable,$fileType)  {
    ...
    /* 
    * @access: public
    * @description: This function is used to check paypal detail of user 
    * @return array
    */
    function checkPaypalDetail($logUserid){
        	$sectionRes=$this->model_common->getDataFromTabel('UserSellerSettings','paypalId,seller_zip,territoryCountryId', array('tdsUid'=>$logUserid), '', '', '', 1 );
            
            if(!empty($sectionRes)){
                $sectionRes = $sectionRes[0];
                $paypalId            = (!empty($sectionRes->paypalId))?$sectionRes->paypalId:"";
                $sellerZip           = (!empty($sectionRes->seller_zip))?$sectionRes->seller_zip:"";
                $territoryCountryId  = (!empty($sectionRes->territoryCountryId))?$sectionRes->territoryCountryId:"";
                
                if(!empty($paypalId) && !empty($sellerZip) && !empty($territoryCountryId))
                    return true;
                else
                    return false;
            }else{

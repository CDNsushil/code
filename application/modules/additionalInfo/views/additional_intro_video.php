...
if($showcaseData){
	...
	$introductoryFileId=$showcaseData->introductoryFileId;
	if($introductoryFileId > 0){
		$mediaFile=getDataFromTabel($table='MediaFile', $field='*',  $whereField=array('fileId'=>$introductoryFileId), '', $orderBy='', $order='', $limit=1 );
		if($mediaFile){
			$mediaDataFlag=true;
			$required='';
			
			$introductoryFileId=$showcaseData->introductoryFileId;
			$introductoryTitle=$showcaseData->introductoryTitle;
			$introductoryDescription=$showcaseData->introductoryDescription;
			...
			$imgintroductorySrc = '<img class="formTip ptr maxWH30 ma"  src="'.base_url().'images/stockphoto_FnV.jpg" onclick="openPlayerLightBox(\'popupBoxWp\',\'popup_box\',\'/common/playCommonVideo\',\''.$fileId.'\',\''.$entityId.'\',\''.$showcaseId.'\',\''.$showcaseId.'\');" />';
			$opacity_4="";
...
if(!$mediaDataFlag){
		$required='required';
		$introductoryTitle=$this->lang->line('addIntroductoryVideo');
		$fileId=$introductoryFileId=$fileSize=0;
		$isExternal='f';
		$fileType=2;
		$introductoryDescription=$rawFileName=$fileName=$filePath=$file=$embedCode=$fileDirPath='';
		$filePath=$folderPath;
		$imgintroductorySrc = '<img class="formTip ptr maxWH30 ma ui-state-disabled"  src="'.base_url().'images/stockphoto_FnV.jpg" />';
		//if(empty($showcaseData->introductoryTitle)){
			$opacity_4="opacity_4";
		//}else{
		//	$opacity_4='';
		//}
...
$introductoryTitleInput = array(
		'name'	=> 'introductoryTitle',
		'id'	=> 'introductoryTitle'.$browseId,	
		'class'	=> 'width546px required',
		'value'	=> $showcaseData->introductoryTitle,
		'minlength'	=> 2,
		'maxlength'	=> 100,
		'size'	=> 50
);
     ...
     <div class="row tab_wp pt2">
		<div class="row ">
			<div class="cell tab_left">
			  <div class="tab_heading"> <?php echo $this->lang->line('introductoryVideo');?></div>
			  <!--tab_heading-->
			</div>
				...
				<div id="uploadElementForm<?php echo $browseId;?>" class="<?php echo $showFormIE;?>">
							...
							<?php echo form_open_multipart($this->uri->uri_string(),$formAttributes); ?>
								  ...
								  <div class="row">
									...
									<div class=" cell frm_element_wrapper">
									  <?php echo form_input($introductoryTitleInput); ?>
										<div class="row wordcounter"><?php echo form_error($introductoryTitleInput['name']); ?></div>
									</div>
								  </div>
								  
								 <?php 
									$introductoryDescription=htmlentities($introductoryDescription);
									$data=array('name'=>'introductoryDescription'.$browseId,'id'=>'introductoryDescription'.$browseId,'value'=>$introductoryDescription, 'addclass'=>'width546px rz required', 'required'=>$required, 'labelText'=>'description', 'view'=>'description', 'descLimit'=>'descLimit'.$browseId,'wordOption'=>array('minVal'=>'0','maxVal'=>'100','wordLabel'=>$label['words0-50']));
								 ...
								 ?>
								  ...
								  'filePath'=>$folderPath,
								  'embedCode'=>$embedCode,
								  'class'=>'width480px required '.$required,
								  'required'=>$required, 
				  ...
				  <div class="row">
					  <div class="empty_label_wrapper cell">
						<label class=""><?php //echo $label['interview'] ?></label>
					  </div>
					  ...
					  <div id="rowData<?php echo $browseId;?>" class="pl cell frm_element_wrapper extract_content_bg">
						...
						<div class="extract_img_wp"> 
							<?php echo $imgintroductorySrc;?>
						</div>
						<div class="extract_heading_box"> <?php echo $introductoryTitle;?> </div>
							<div class="clear"></div>
						</div>
	...
	$(document).ready(function(){
		...
		$('#uploadmediaForm'+section).validate({
			submitHandler: function() {
				...
				var data = {"introductoryFileId":"<?php echo $introductoryFileId;?>","introductoryTitle":$('#introductoryTitle'+section).val(),"introductoryDescription":$('#introductoryDescription'+section).val(),"dateModified":"<?php echo currntDateTime();?>"};

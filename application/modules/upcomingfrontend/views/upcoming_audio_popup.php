...
<script type="text/javascript" src="<?php echo base_url('player/flowplayer/flowplayer-3.2.11.min.js')?>"></script>
...
<div id="popup_close_btn" class="popup_close_btn" onclick="$(this).parent().trigger('close');"></div>
    <div class="bg_white width700px">
      <div class="bg_222222 pt20 pb20 pl20 pr20 tac">
      <!--  <div class="AI_table">
          <div class="AI_cell">
			  <?php 
			//	if(@$promoMedias[0]['isExternal'] == 't')
        ...
        <?php
			...
			/*************Here check exnternal and interter media**************/ 
								...
								if($getType[0]['isExternal']=="t")
								{
									...
									{
										$class_player ='class="audio_player_iframe"';
										$src = base_url().'en/player/commonPlayerIframe/'.$mediaId.'_full/'.$entityId.'/'.$elementId.'/'.$projectId;
									}  
									  
								}else
								{
									$class_player ='class="mt52"';
									$src = base_url().'en/player/getPlayerIframe/'.$mediaId.'_full/'.$entityId.'/'.$elementId.'/'.$projectId;
								}  
			
        ?>
        <iframe src="<?php echo $src; ?>" width="279" height="128" frameborder="0"  webkitAllowFullScreen mozallowfullscreen allowFullScreen scrolling="no" <?php if(isset($class_player)) { echo $class_player;} ?>></iframe>
      </div>

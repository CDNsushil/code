...
class model_workprofile extends CI_Model {
	...
	private $tabelWorkProfileLanguage = 'WorkProfileLanguage';
	...
	***/
	...
	/*
	 * Function to get profile language
	 */
	function getProfileLanguage($workProfileId=0)
	{		
		 $this->db->select('*');
		 $this->db->from($this->tabelWorkProfileLanguage);
		 $this->db->where('workProfileId',$workProfileId);
		 $this->db->order_by("profileLangId", "desc");
		 $query = $this->db->get();
		 return $query->result();		 
	}

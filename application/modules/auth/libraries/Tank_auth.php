...
class Tank_auth
{
					...
					if ($user->banned == 1) {									// fail - banned
						...
						if ($user->active == 0) {							// fail - not active
							...
							if ($remember) {
								...
								$this->user_stay_login($login);
							}else{
                                setcookie("rememberMe[login]",'', time()-(3600*24*365),"/");
	...
	/* -------------------------- Stay Login -------------- */
	public function user_stay_login($login)
	{
		...
		$getUserData   =   $this->ci->model_common->getDataFromTabel('UserAuth', 'system_ip,browser_name,tdsUid', $where, '', 'tdsUid', 'DESC', 1);
		
		$browserArray = array();
		if(!empty($getUserData)){
			$getUserData = $getUserData['0'];
			
			if(!empty($getUserData->browser_name)){
				
				//decode updated browser name
				$browserArray = (array)json_decode($getUserData->browser_name);
				
				//check if not exist then insert
				if(!in_array($browser_name,$browserArray)){
					//insert browser name
					array_push($browserArray,$browser_name);
				}
		...
		//encode browser array
		$browserJson = json_encode($browserArray);
		
		// update user Auth status
       $this->ci->model_common->editDataFromTabel('UserAuth', array('system_ip'=>$system_ipaddress,'browser_name'=>$browserJson), 'email',$login);					
	...
	function logout()
	{
		...
		// Unset stay login ( Note - To run remember function Remove this function )  
	    $this->unset_stay_login();
	...
	{	
		...
		$browserArray   = array();
			...
			if(!empty($getUserData)){
				...
				if(in_array($browser_name,$browserArray)){
					//remove browser name
					if (($key = array_search($browser_name, $browserArray)) !== false) {
						unset($browserArray[$key]);
						
						$browserArray = array_values($browserArray);
						
						if(count($browserArray) > 0){
							$browserJson = json_encode($browserArray);
							$system_ipaddress = $getUserData->system_ip;
						}
						else{
							$browserJson = ''; 
							$system_ipaddress = '';
						...
						// update user Auth status
						$this->ci->model_common->editDataFromTabel('UserAuth', array('browser_name'=>$browserJson,'system_ip'=>$system_ipaddress), 'tdsUid',$userid);					

  ...
  function saveSuggestions()
    ...
    */ 
    ...
    public function yourtoadsquaremenu($userId="0"){
        ...
        //call show details data
        $this->_showcasedetails();
        
        //call shopping cart details
        $this->_shoppingcartdetails();
        
        //get user nagivation
        //$this->data['userNavigations']      =   $this->model_common->userNavigations($userId);
        $where                                =   array('tdsUid'=>$userId);
        $this->data['userNavigations']        =   $this->model_common->getDataFromTabel('YourToadsquare', $field='section,isPublished,isCompleted,currentStage,sectionparent,projectid,elementid', $where, '', 'id', 'DESC', 0,0,true);

        //check user complete media list
        $this->_checkcompletemedialist($userId);
        
        //check user incompleted upcoming media list
        $this->_checkcompleteupcomingmedialist($userId);
        
        // check not converted media counts
        $this->_getfailedconversionmedia($userId);
    ...
    /*
    * @description: This method is use to show in completed upcoming media showcase
    * @auther: lokendra meena
    * @return: void
    */ 
    
    private function _checkcompleteupcomingmedialist($userId){
        
        $industryIdArray = array(1,2,3,4,10);//media id
        $incompletedUpcomingMediaList = array();
        $completedUpcomingMediaList = array();
        $where           =   array('tdsUid'=>$userId,'projArchived'=>'f');
        $getProjectList  =   $this->model_common->getDataFromTabel('UpcomingProject', $field='projId,projIndustry,isPublished', $where, '', 'projId', 'DESC', 0,0,true);

        if(!empty($getProjectList)){
            
            foreach($getProjectList as $getProject) {
				if($getProject['isPublished'] == 'f') {
					 if(in_array($getProject['projIndustry'],$industryIdArray) && !in_array($getProject['projIndustry'],$incompletedUpcomingMediaList)){
						array_push($incompletedUpcomingMediaList, $getProject['projIndustry']); 
					}
				} else {
					if(in_array($getProject['projIndustry'],$industryIdArray) && !in_array($getProject['projIndustry'],$completedUpcomingMediaList)){
						array_push($completedUpcomingMediaList, $getProject['projIndustry']); 
					}
				}
            }
        }
        $this->data['incompletedUpcomingMediaList'] =  $incompletedUpcomingMediaList;
        $this->data['completedUpcomingMediaList']   =  $completedUpcomingMediaList;
    ...
    /*
    * @description: This method is use to show complete project menu list
    * @auther: lokendra meena
    * @return: void
    */ 
    
    private function _checkcompletemedialist($userId){
        
        $industryIdArray = array(1,2,3,4,10);//media id
        $completetMediaList = array();
        $where              =   array('tdsUid'=>$userId,'isProjectCompleted'=>'f');
        $getProjectList     =   $this->model_common->getDataFromTabel('Project', $field='projId,IndustryId', $where, '', 'projId', 'DESC', 0,0,true);

        if(!empty($getProjectList)){
            
            foreach($getProjectList as $getProject){
            
                if(in_array($getProject['IndustryId'],$industryIdArray) && !in_array($getProject['IndustryId'],$completetMediaList)){
                   array_push($completetMediaList, $getProject['IndustryId']); 
                }
            }
            
        }
        $this->data['completetMediaList']      =  $completetMediaList;
    ...
    /*
    * @access: private
    * @descrption: This method is use to show showcasedetails
    * @return void
    */ 
    
    private function _shoppingcartdetails(){
        
        $userId = isLoginUser();
        
        
        // get wishlist count
        $wishlistCount     =   countResult('Wishlist',array('tdsUid'=>$userId));
        $wishlistCount     =   ($wishlistCount > 0)?$wishlistCount:0;
        
        // get purchase count
        $purchaseCount     =   countResult('SalesOrder',array('customerUid'=>$userId));
        $purchaseCount     =   ($purchaseCount > 0)?$purchaseCount:0;
        
        // get sales count
        $salesCount     =   countResult('SalesOrderItem',array('sellerId'=>$userId));
        $salesCount     =   ($salesCount > 0)?$salesCount:0;
        
        $this->data['wishlistCount']                 =  $wishlistCount;
    ...
    */ 
    ...
    public function _showcasedetails(){
        // get user showcase id
        $showcaseId  =   LoginUserDetails('showcaseId');
        $where              =   array('showcaseId'=>$showcaseId);
        $showcaseDetails  =   $this->model_common->getDataFromTabel($table='UserShowcase', $field='isPublished,tagwords,promotinalSection', $where, '', 'showcaseId', 'DESC', 0,0,true);
        
        if(!empty($showcaseDetails)){
            $showcaseDetails  =  $showcaseDetails[0];
        }
        
        $this->data['showcaseDetails']      =  $showcaseDetails;
    ...
    */  
    ...
    private function _yourmessagecentermenu($userId){
        ...
        $unreadMsgCount     =   ($unreadMsgCount > 0)?$unreadMsgCount:0;
         ...
         // get unread & new tmail of user
        $readMsgCount     =   countResult('tmail_participants',array('user_id'=>$userId,'is_sender'=>'f'));
        $readMsgCount     =   ($readMsgCount > 0)?$readMsgCount:0;
        ...
        $this->data['readMsgCount']           =  $readMsgCount;
    ...
    */  
    ...
    private function _yourcravesmenu($userId){
        ...
        $this->data['myCravesCount']       =  count($myCravesList);
        $this->data['userId']              =  $userId;
    }
    ...
    */ 
    ...
    /*
    * @access: public
    * @description: This method is use to save user feedback
    * @auther: lokendra meena
    * @return: void
    */ 
    ...
    public function feedbackSave(){
        ...
        $data['subject']            =  $this->input->post('feedbacksubject');
        $data['suggestion_for']     =  $this->input->post('currentUrl');
        $data['suggestion']         =  $this->input->post('feedbackmessage');
        $data['is_feedback']        =  '1';
        $data['sender_id']          =  $this->userId;			
        ...
        /*****************feedback email to toadsquare start**************/
        $getUserShowcase	= showCaseUserDetails($data['sender_id']);
        $username =  $getUserShowcase['userFullName'];
        $this->load->library('email');
        $this->email->from($this->config->item('webmaster_email', ''), $this->config->item('website_name', ''));
        ...
        $this->email->to($this->config->item('suggestions_email', ''));
        //$this->email->to('lokendrameena@cdnsol.com');	
        $this->email->subject(sprintf('Feedback ', $this->config->item('website_name', '')));
        $where=array('purpose'=>'feedback','active'=>1);
        $reportTemplateRes=getDataFromTabel('EmailTemplates','templates',  $where, '','', '', 1 );
        
        $reportTemplate=$reportTemplateRes[0]->templates;
        $searchArray = array("{feedback_description}", "{user_name}");
        $replaceArray = array($data['suggestion'], $username);
        $abusiveReportTemplate=str_replace($searchArray, $replaceArray, $reportTemplate);
        $this->email->message($abusiveReportTemplate);
        $this->email->send();
        ...
        //set message
        $msg=$this->lang->line('feedbackMsg');
        set_global_messages($msg, $type='success', $is_multiple=true);
            
        /*****************feedback email to toadsquare end**************/
   ...
   /*
    * @access: public
    * @description: This method is use to invite friend 
    * @auther: lokendra meena
    * @return: void
    */ 
    ...
    public function invitefriend(){
        ...
        $joinUrl              =   base_url('package');
        
        /*****************invite friend from toadsquare start**************/
        $getUserShowcase	= showCaseUserDetails($this->userId);
        $username =  $getUserShowcase['userFullName'];
        $this->load->library('email');
        $this->email->from($this->config->item('webmaster_email', ''), $this->config->item('website_name', ''));
        $this->email->reply_to($this->config->item('webmaster_email', ''), $this->config->item('website_name', ''));
        ...
        //$this->email->to('lokendrameena@cdnsol.com');	
        $this->email->subject(sprintf('Invite from toadsquare'));
        $where=array('purpose'=>'invite_from_toadsquare','active'=>1);
        $reportTemplateRes=getDataFromTabel('EmailTemplates','templates',  $where, '','', '', 1 );
        $reportTemplate=$reportTemplateRes[0]->templates;
        $searchArray = array("{toadsquare_url}", "{from_name}");
        $replaceArray = array($joinUrl, $username);
        $abusiveReportTemplate=str_replace($searchArray, $replaceArray, $reportTemplate);
        $this->email->message($abusiveReportTemplate);
        $this->email->send();
       
        //set message
        $msg=$this->lang->line('invitationsent');
        set_global_messages($msg, $type='success', $is_multiple=true);
        /*****************invite friend from toadsquare end**************/
    ...
    /*
    * @access: public
    * @description: This method is use to create crop image thumb
    * @auther: lokendra meena
    * @return: void
    */ 
    
    public function createthumbcropimages($targetDir="",$fileName="",$createWaterMarkFlag="")
	{		
		ini_set('memory_limit', '-1');
		...
		if(is_file($filePathName)){
            ...
            $thumb_config = $this->config->item('crop_thumb_config');
            
            $imgThumbFolder = $filePath;
            $orignalImagPath = $filePath;
    ...
    /*
    * @access: private
    * @descrption: This method is use to get fail media file records
    * @return void
    */ 
    
    private function _getfailedconversionmedia($userId){
        
        // get failed video files
        $videoData = $this->model_common->getPendingConvertionFiles($userId,'FvElement');
        // get failed audio files
        $audioData = $this->model_common->getPendingConvertionFiles($userId,'MaElement');
        // get failed text files
        $textData = $this->model_common->getPendingConvertionFiles($userId,'WpElement');
        // set not converted media file counts
        $this->data['failedVideoCount']     =  (!empty($videoData)) ? count($videoData) : 0;
        $this->data['failedAudioCount']     =  (!empty($audioData)) ? count($audioData) : 0;
        $this->data['failedTextCount']      =  (!empty($textData)) ? count($textData) : 0;
	}
    ...
    /*
    *  @description: This method is for existing user for your toadsquare navigation  data
    *  @auther: lokendra meena
    *  @return: void
    */ 
    
    public function showcaseatuoscript(){
        
        $getUserDataList = $this->model_common->usershowcaselist();
        if(!empty($getUserDataList)){
            
            $showcaseParent                =    $this->config->item('showcaseParentName');
            $entityid                      =    getMasterTableRecord('UserShowcase');
             
            foreach($getUserDataList as $getUserData){
                
                if(empty($getUserData->isCompleted))
                {
                    $showcaseId     =    $getUserData->showcaseId;
                    $tdsUid         =    $getUserData->tdsUid;
                    $isPublished    =    $getUserData->isPublished;
                    
                    if($getUserData->creative == 't') {
                        $sectionId = "6";
                    }elseif($getUserData->associatedProfessional == 't'){
                        $sectionId = "7";
                    }elseif($getUserData->enterprise == 't'){
                         $sectionId = "8";
                    }elseif($getUserData->fans == 't'){
                        $sectionId = "34";
                    }
                    
                    //if not published then redirect to showcase page
                    $currentStage  =  ($isPublished=="f")?"en/showcase/showcasetype":"";
                   
                    //get showcasetion section
                    ...
                    $inserData    =   array(
                        ...
                        'tdsUid'                =>  $tdsUid,
                        'sectionparent'         =>  $showcaseParent
                    );
                    //insert your toadsquare data
                    $this->model_common->addDataIntoTabel('YourToadsquare', $inserData);
                    unset($inserData);
               }
            }
        }
   ...
   /*
    *  @description: This method is for existing user blog for your toadsquare navigation  data
    *  @auther: lokendra meena 
    *  @return: void
    */ 
     
    
     public function blogatuoscript(){
        
        $getBlogDataList = $this->model_common->userbloglist();
        if(!empty($getBlogDataList)){
            
            $section                       =    $this->config->item('sectionId13');
            $sectionId                     =    $this->config->item('blogsSectionId');
            $entityid                      =    getMasterTableRecord('Blogs');
             
            foreach($getBlogDataList as $getBlogData){
                
                 if(empty($getBlogData->isCompleted)){
                        $blogId         =    $getBlogData->blogId;
                        $tdsUid         =    $getBlogData->custId;
                        $isPublished    =    $getBlogData->isPublished;
                        
                        //if not published then redirect to blog  page
                        $currentStage  =  ($isPublished=="f")?"en/blog/addpost":"";
   ...
   /*
    *  @description: This method is for existing user review for your toadsquare navigation  data
    *  @auther: lokendra meena 
    *  @return: void
    */ 
     
    
     public function reviewatuoscript(){
        
        $getReviewDataList = $this->model_common->userreviewlist();
        if(!empty($getReviewDataList)){
            
            $section                       =    $this->config->item('industryForSectionId3_2');
            $sectionId                     =    $this->config->item('reviewsSectionId');
            $entityid                      =    getMasterTableRecord('Project');
             
            foreach($getReviewDataList as $getReviewData){
                
                if(empty($getReviewData->isreviewscompleted)){
                        $projId         =    $getReviewData->projId;
                        $tdsUid         =    $getReviewData->tdsUid;
                        $isPublished    =    $getReviewData->isPublished;
                        
                        //if not published then redirect to review  page
                        $currentStage  =  ($isPublished=="f")?"en/media/addreviews":"";
   ...
   /*
    *  @description: This method is for existing user news for your toadsquare navigation  data
    *  @auther: lokendra meena 
    *  @return: void
    */ 
     
    
     public function newsatuoscript(){
        
        $getNewsDataList = $this->model_common->usernewslist();
        
        if(!empty($getNewsDataList)){
            
            $section                       =    $this->config->item('industryForSectionId3_1');
            $sectionId                     =    $this->config->item('newsSectionId');
            $entityid                      =    getMasterTableRecord('Project');
             
            foreach($getNewsDataList as $getNewsData){
                
                if(empty($getNewsData->isnewscompleted)){
                        $projId         =    $getNewsData->projId;
                        $tdsUid         =    $getNewsData->tdsUid;
                        $isPublished    =    $getNewsData->isPublished;
                        
                        //if not published then redirect to news  page
                        $currentStage  =  ($isPublished=="f")?"en/media/addnews":"";
   ...
   /*
    *  @description: This method is for existing user upcomming for your toadsquare navigation  data
    *  @auther: lokendra meena 
    *  @return: void
    */ 
     
    
     public function upcommingatuoscript(){
        
        $getUpcommingDataList = $this->model_common->userupcomminglist();
        if(!empty($getUpcommingDataList)){
            ...
            foreach($getUpcommingDataList as $getUpcommingData){
                
                if(empty($getUpcommingData->isCompleted) && !empty($getUpcommingData->tdsUid)){
                        $projId         =    $getUpcommingData->projId;
                        $tdsUid         =    $getUpcommingData->tdsUid;
                        $isPublished    =    (!empty($getUpcommingData->isPublished))?$getUpcommingData->isPublished:'f';
                        
                        //if not published then redirect to news  page
                        $currentStage  =  ($isPublished=="f")?"/en/upcomingprojects/donation/0/1":"";

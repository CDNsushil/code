...
class model_cart extends CI_Model { 
    ...
    private $tableProjectShippingDomestic		= 'TDS_ProjectShippingDomestic';
 ...
 function getAllUserBasketItems($itemId=0,$currency=0){	  
  ...
  function getShippingAmount($entityId=0,$elementId=0,$countryId=0){	  	
    ...
    if($query->num_rows() > 0) {
               ...
               return $this->_getdomesticShippingAmount($entityId,$elementId,$countryId);
        }
  } 
  
  
  
  /* get Domenstic Shipping amount */
   private function _getdomesticShippingAmount($entityId=0,$elementId=0,$countryId=0){
    $this->db->select('price as amount');	
    $this->db->from($this->tableProjectShippingDomestic);	
    $this->db->where('entityId',$entityId);
    $this->db->where('elementId',$elementId);	
...
function deleteCartItem($Id,$entityId,$elementId,$purchaseType)  {	   		
    ...
    function get_purchased_details($userId,$offset=0,$limit=0)
    {
        ...
        SalesOrderItem.itemQty,SalesOrderItem.basePrice,SalesOrderItem.itemName,BuyerComments.id as buyercommentsid,SalesOrderItem.transactionId,SalesOrderItem.invoiceId,
        SalesOrderItem.purchaseType,SalesOrderItem.sellerInfo,SalesOrderItem.isProductAuction,SalesOrderItem.itemValue,SalesOrderItem.fileSize');
    ...
    */   
    ...
    function get_sales_details($userId,$offset=0,$limit=0)
    {
        ...
        SalesOrderItem.invoiceId,SalesOrderItem.purchaseType,BuyerComments.rateSeller,BuyerComments.id as buyercommentsid,BuyerComments.comments,SalesOrderItem.sellerInfo,SalesOrderItem.isProductAuction,SalesOrderItem.itemValue');
         $this->db->select('SalesItemDownload.dwnId,SalesItemDownload.dwnDate,SalesItemDownload.dwnMaxday,SalesItemDownload.dwnStatus,SalesItemDownload.userId,SalesItemDownload.ownerId,SalesItemDownload.purchaseType as itempurchasetype');
        $this->db->select('SalesItemShipping.shpStatus,SalesItemShipping.shippingDetails');	    	
        $this->db->select('LogSummary.*');
         $this->db->select('ProjCategory.IndustryId, ProjCategory.category, ProjCategory.entityId as elemententityid');	  	
 ...
 /*******
  ...
  function get_membership_item($orderId)
    ...
    */
    ...
    /*
     * @access: pu bluc
     * @description: This method is use to project media file size
     * @auther: lokendra meena
     * @return: void
     */ 
    
    public function projectmediafilesize($projId=0,$elementId=0,$ElementTable = 'FvElement' ){
        
        $this->db->select_sum('MediaFile.fileSize');
        //$this->db->select('MediaFile.*');
        $this->db->from($this->tableProject);
        $this->db->join($ElementTable,$ElementTable.'.projId = Project.projId','left');
        $this->db->join('MediaFile','MediaFile.fileId = '.$ElementTable.'.fileId','left');
        $this->db->where('Project.projId',$projId);	
        $this->db->where($ElementTable.'.elementType','0');
        if($elementId!=0){
    ...
    /*
    * @access: public
    * @description: This method is use to get buyer comment listing
    * @auther: lokendra meena
    * @return: void
    */ 
    
    public function buyercommentslist($itemId){
        $this->db->select('BuyerComments.comments,BuyerComments.rateSeller');
        ...
        $this->db->join($this->tableUserAuth, $this->tableUserAuth.'.tdsUid = '.$this->tableBuyerComments.'.tdsUid','left');
        ...
        $this->db->where($this->tableBuyerComments.'.itemId',$itemId);	
        $this->db->where($this->tableBuyerComments.'.status','t');
        $query = $this->db->get();
        $result = $query->row_array();
        return $result;
    }

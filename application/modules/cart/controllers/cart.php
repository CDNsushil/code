...
class Cart extends MX_Controller {
    ...
    */ 
            ...
            if($cartDetails){
                   $sendpaypal= 't';	
                    ...
                    //if($sectionId=="1"){
                        ...
                        $mediaFileSize  = $this->_mediafilesize($mediaFileSizeArray);
                    //}            
                    $customerBasketItem = array(
                    'basketId' =>$thirdPartyCartId,
                    'entityId'=> $entityId,
    ...
    /*
    *  @access: 
    *  @description: This method is use to get media file size 
    *  @auther: lokendra meena
    *  @return: void
    */  
    
    private function _mediafilesize($mediaFileSizeArray=false){
        
        $filesize = 0;
        $sectionId = $mediaFileSizeArray['sectionId'];
        
        $ElementTable = $this->config->item('sectionTable'.$sectionId);// Get Element table name
        
        if(!empty($mediaFileSizeArray)){
            //project media file size
            if($mediaFileSizeArray['entityId']==54){
                $mediaFileListing = $this->model_cart->projectmediafilesize($mediaFileSizeArray['projId'],0,$ElementTable);
                if(!empty($mediaFileListing['filesize'])){
                    $filesize   =   $mediaFileListing['filesize'];
                }
            }else{
                //element media file size
                $mediaFileListing = $this->model_cart->projectmediafilesize($mediaFileSizeArray['projId'],$mediaFileSizeArray['elementId'],$ElementTable);
                if(!empty($mediaFileListing['filesize'])){
                    $filesize   =   $mediaFileListing['filesize'];
                }
            }
    ...
    //------------------------------------------------------------------------
    function test(){
        $arra =array('entityId'=>'54','projId'=>'351','elementId'=>'351');
        echo $this->_mediafilesize($arra);
    }
    ...
    */  
    ...
    function mywishlist($currency='euro'){
        ...
        $wishlistItemCountData     =   $this->model_common->getDataFromTabel('Wishlist', '*',  array('tdsUid'=>$userId),'','','','');	 
        if(!empty($wishlistItemCountData)){
            foreach($wishlistItemCountData as $wishlistdata){
                if($wishlistdata->currency=='0'){
                    $euroCount++;
                }elseif($wishlistdata->currency=='1'){
                    $dollarCount++;
                }
        ...
        //if euro then go direct to euro page
        if($currency=='euro' && $euroCount > 0){
            $currencyId = 0;
        }
        else if($currency=='dollar' && $dollarCount > 0){
            //if dollar then go direct to dollar page
            $currencyId = 1;
        }
        else if($currency=='euro' && $euroCount == 0 && $dollarCount > 0)
        {
            redirect(base_url_lang('cart/mywishlist/dollar'));   
        }
        else if($currency=='dollar' && $dollarCount == 0 && $euroCount > 0)
        {
            redirect(base_url_lang('cart/mywishlist/euro'));
        }
        else
        {
            $currencyId = 0;
        ...
        //set curreny menu count
        $data['dollarCount']               =   $dollarCount; 
        $data['euroCount']                 =   $euroCount; 
            ...
            foreach ($wishlistItem as $items) {
                ...
                if(($isAuction=="t" && $daysRemaining>0) || $isAuction=="f"){
                    ...
                    if(!empty($productDetails)){
                        ...
                        $userWishlistItem[$i]['sectionId']         =  $sectionId;
                        $i++;
                    }
        ...
        if(!empty($wishlistItem)){                
            ...
            foreach ($wishlistItem as $items) {					
                    ...
                    if($productDetails){
                        ...
                        $cartDetails=$this->getProductInfo($entityId,$elementId,$purchaseType);// Category Detail
    ...
    */ 
    ...
    function shoppingcartbilling($purchaseType=1){
        ...
        // when Come Direct by using url    
        if(empty($thirdPartyCartId))
        {
        ...
        // get Customer basket status
        $salesCustomerBasketData     =   $this->model_common->getDataFromTabel('SalesCustomersBasket', 'is_billingcheck,is_shippingcheck,is_samedetailcheck',  array('basketId'=>$thirdPartyCartId),'','','',1);     
        $data['globelBilling'] = ($salesCustomerBasketData[0]->is_billingcheck == "t")?$salesCustomerBasketData[0]->is_billingcheck:'';
        $data['globelShipping'] = ($salesCustomerBasketData[0]->is_shippingcheck == "t")?$salesCustomerBasketData[0]->is_shippingcheck:'';
        $data['globelBothShipping'] = ($salesCustomerBasketData[0]->is_samedetailcheck == "t")?$salesCustomerBasketData[0]->is_samedetailcheck:'';
  ...
  function saveBillingDetails(){	  
    ...
    $countryNameShipping = $this->model_cart->getUserCountryName($countryIdShipping);  
        ...
        $UserBuyerSettings = array(
            ...
            //$thirdPartyCartId=9;
            $json = array('billingdetails'=>json_encode($UserBuyerSettings ),'is_billingcheck'=>$billingsaveglobalSettings,'is_shippingcheck'=> $shippingsaveglobalSettings);
            $this->model_common->editDataFromTabel('SalesCustomersBasket', $json, 'basketId', $thirdPartyCartId);
    ...
    */ 
    ...
    function postshoppingcartbilling(){	  
       ...
       // print_r($_POST);die();
        $userId = isLoginUser();
        // For check billing and shipping status     
        $billingsaveglobalSettings = ($this->input->post('billingsaveglobalSettings'))?'t':'f';
        ...
        $shipping_isSameAsBilling=($this->input->post('shipping_isSameAsBilling'))?'t':'f'; 
         
        //check shipping same as billing for country id
        if($shipping_isSameAsBilling == 't'){		
            $countryIdShipping = $this->input->post('billing_country');
        }else{  
            $countryIdShipping = $this->input->post('shipping_country');	
        }
        ...
        if($this->input->post())
        {
                ...
                if($shipping_isSameAsBilling == 't'){
                    ...
                    $shipping_phone         =  $this->input->post('shipping_phone');
                    $shipping_email         =  $this->input->post('shipping_email');
                    $shippingsaveglobalSettings = ($this->input->post('shippingsaveglobalSettings'))?'t':'f';
                }
            
            
                                
          
            if(isset($billingsaveglobalSettings) && $billingsaveglobalSettings=='t'){
                ...
                $UserBuyerSettings = array(
                    ...
                    'otherAboutConsumptionTax' => $res[0]->otherAboutConsumptionTax	
                      ...
                      //$thirdPartyCartId=9;
                $json = array('billingdetails'=>json_encode($UserBuyerSettings),'is_billingcheck'=>$billingsaveglobalSettings,'is_shippingcheck'=> $shippingsaveglobalSettings,'is_samedetailcheck'=>$shipping_isSameAsBilling);
                $this->model_common->editDataFromTabel('SalesCustomersBasket', $json, 'basketId', $thirdPartyCartId);

                 $msg = "Update Billing Address";
                 set_global_messages($msg, $type='success', $is_multiple=true);		
                 
            // Check shippng detail Existance
             
            if(isset($shipping_firstName)&& $shipping_firstName != '')
            { 
                if(isset($shippingsaveglobalSettings) && $shippingsaveglobalSettings=='t'){
                   ...
                   // $json = array('shippingdetails'=>'','is_billingcheck'=>$billingsaveglobalSettings,'is_shippingcheck'=> $shippingsaveglobalSettings);
                    //$this->model_common->editDataFromTabel('SalesCustomersBasket', $json, 'basketId', $thirdPartyCartId);
                    ...
                    //$thirdPartyCartId=9;
                    $json = array('shippingdetails'=>json_encode($UserShippingJson),'is_billingcheck'=>$billingsaveglobalSettings,'is_shippingcheck'=> $shippingsaveglobalSettings,'is_samedetailcheck'=>$shipping_isSameAsBilling);
                    $this->model_common->editDataFromTabel('SalesCustomersBasket', $json, 'basketId', $thirdPartyCartId);
                    $msg = "Update Shipping Jason Address";
                    // set_global_messages($msg, $type='success', $is_multiple=true);
            }
                ...
                if($productDetails){
                    ...
                    if($purchaseType ==4){ // for donate
                        ...
                        if($isProductAuction=="t"){
                            //auction product/project price
                            $productPrice =  $this->getAuctionProductPrice($basePrice,$currency,$productDetails->tdsUid);
                            $itemValue    =  $productPrice['price'];
                        }else{
                            $productPrice = $this->getPriceInfo($itemValue,$currency,$ownerId);	
                        }
                    ...
                    $cartDetails[$i]['sellerInfo']      =  $sellerInfo;
                    $cartDetails[$i]['sendpaypal']      =  $items->sendpaypal;
                   // sectionId to check industry type // 
                    $cartDetails[$i]['sectionId']      =  $sectionId;
...
// create random no's
 ...
 function Pay_parallel($data) {
    ...
    */ 
    ...
    /*
    * @description: This method is use to get wishlist, purchase and sales count
    * @auther: lokendra meena
    * @return: void
    */ 
    ...
    public function shoppingcartdata(){
        ...
        $countTotalArray        =   $this->model_cart->get_purchased_details($userId,0,0);
        $countTotal             =   $countTotalArray['get_num_rows'];
        $data['purchaseCount']  =   $countTotal;
        
        //set sales count
        $countTotalArray    =   $this->model_cart->get_sales_details($userId,0,0);
        $countTotal         =   $countTotalArray['get_num_rows'];
        $data['salesCount'] =   $countTotal;
        
        //set wishlist count
        $wishlistItem           =   $this->model_common->getDataFromTabel('Wishlist', '*',  array('tdsUid'=>$userId),'','','','');
        $data['wishlistCount']    =   count($wishlistItem);
        
        $this->load->view('purchase_common_menu_new',$data); 
    }
    ...
    */ 
    ...
    /*
    *  @access: public
    *  @description: This method is use to show buyer comment
    *  @auther: lokendra meena
    *  @return: void
    */ 
    ...
    public function buyercomments(){
        $itemId = $this->input->get('val1');
        $buyerCommentsData = $this->model_cart->buyercommentslist($itemId);
        $data['buyerCommentsData'] = $buyerCommentsData;		
        $this->load->view('buyer_comments',$data);
    }
    ...
    function sales_record_print($getItemId=0)
    {
        ...
        if(!empty($getSellerDetail)){
            if($getSellerDetail->territoryCountryId != "")
                {
                    $userCountryName = $this->model_cart->getUserCountryName($getSellerDetail->territoryCountryId);
                    $getSellerDetail->territoryCountryId = $userCountryName->countryName;
                }
            if($getSellerDetail->seller_state != "")
              { 
                $getSellerDetail->seller_state = getstateName($getSellerDetail->seller_state);
              }
        }

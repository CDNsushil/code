 ...
 * 
 * @author 		Chris Baines - Dove Help Dev Team
 * @package 	DoveHelp
 * @subpackage 	Users Module
 * @category	Modules
 * @copyright © 2010 - 2011 Dove Help 
 */
class Help_users extends CI_Model{
    ...
    public function build_avatar($email, $username)
    {
        ...
        if($this->sSettings[0]['useGravatars'] == '1')
        {
			
			
			
            // Build the users avatar from a file.
            $avatar = '<img src="'.base_url().'images/user.png" alt="'.$this->lang->line('avatarAltText').'" title="'.$username.''.$this->lang->line('avatarTitle').'" height="45px" width="45px"/>';
        }else{
    ...
    public function get_userid($username)
    {
        ...
        $options = array('username' => $username);
        $q = $this->db->get_where('UserAuth', $options, 1);
	...
	public function update_settings($username)
	{
		...
		set_global_messages($this->lang->line('messageAccountUpdated'), 'success');

        // [HOOK] run after everything else
        do_action('post.update.user.settings');
        
		redirect('help/settings/'.$username.'');	
	}
}

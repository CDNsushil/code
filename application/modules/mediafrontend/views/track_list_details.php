<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    if($currentKeyPosition > 0):
        ...
        $previousPageLink       =     'mediafrontend/tracklist/'.$frentendUserId.'/'.$projectId.'/'.$previousElementId.$previewWord;
    ...
    if($currentKeyPosition < ($elementNumberCount-1)):
        ...
        $nextPageLink       =    'mediafrontend/tracklist/'.$frentendUserId.'/'.$projectId.'/'.$nextElementId.$previewWord;
    ...
    $mediaFileType          =    $this->config->item('media_type_audio'); // set default media type for audio
    ...
    switch($uploadFileType){
        case "CD":
            $headingName  = $this->lang->line('musicNaudio_headingName_0_'.$categoryId);
            $craveDetails = $this->lang->line('musicNaudio_craveDetails_0_'.$categoryId);
            $infoDetails  = $this->lang->line('musicNaudio_infoDetails_0_'.$categoryId);
        break;
        
        case "media_file":
            $headingName  = $this->lang->line('musicNaudio_headingName_1_'.$categoryId);
            $craveDetails = $this->lang->line('musicNaudio_craveDetails_1_'.$categoryId);
            $infoDetails  = $this->lang->line('musicNaudio_infoDetails');
        break;
        
        default:
            if($elementType=='1'){
                //condition for sample
                $headingName  = $this->lang->line('musicNaudio_headingName_2_'.$categoryId);
...
?>
      <div class="row content_wrap" >     
        <?php
            //$this->lang->line('musicNaudio_detail_heading_'.$categoryId)
        ...
        <div class="clearbox ">
            ...
            <div class=" music_discription fl"> <span class="header_dark lineH26   fs20  mr20" id="gettracktitle" ><?php echo $title;?> </span>
                    <div class="music_collection position_relative mr20 mb20 fl display_table"> <span class="table_cell"> 
                        <?php 
                            $thumbImage      = addThumbFolder($elementImagePath,'_l');	
            ...
            <div class="player_box fr width342 display_inline mb20">
                    <div class="lineH14 fs13">
                  <script>
                        var mediaObject = <?php echo json_encode(array('projectId'=>$projectId,'elementId'=>$elementId,'elementEntityId'=>$elementEntityId)); ?>;
                   </script> 
                    <span  class=" red ptr" href="javascript:void(0)" onclick="addtoplaylist(mediaObject)">Add to Playlist </span> <span href="javascript:void(0)" onclick="openLightBox('popupBoxWp','popup_box','/mediafrontend/albumplaylist','<?php echo $projectId; ?>');" class="fr pr10 ptr ">View  List</span>
                <div class="track mt10">
                        <?php
                        ...
                        /*************Here check exnternal and interter media**************/ 
                            ...
                            if($isvalidUrl)   {    ?> 
                                ...
                                <?php } else { 
                                    ...
                                    $imagetype      =    $this->config->item('musicNaudioImage');
            ...
            <?php 
                          ...
                          //$this->load->view('rating/rating_form_design',array('elementId'=>$elementId,'entityId'=>$entityId));
                            ...
                            $this->load->view('media/reviewViewNew',array('reviewdesign'=>'1','reviewProjectId'=>$projectId,'reviewElementId'=>$elementId,'reviewEntityId'=>$elementEntityId,'reviewIndustryId' =>'2','isPublished'=>$isPublished)); 
                        ?>
                      </div>
            ...
            <span class="left_w  fl lineH18  mr pr60">
                <div class="fl mt20 open_sans mb15">
     
                    <?php if(!empty($projGenreFree)){ ?>
                        <span class="green fs18"><?php echo $projGenreFree; ?></span><span class="fr lineH22 pl20">This is sub gener</span>
                    <?php } ?>
                  ...
                  <div class="clearb">
              <p class="red font_bold "> <?php echo $infoDetails; ?> Information</p>
              <ul class="edit_list  ">
                    
                    <?php if(!empty($fileLength)){ ?>
                        <li>
                            <p class="red opens_light ">LENGTH</p>
                   ...
                   <?php } ?>
                      <li>
                  <p class="red opens_light ">DATE RELEASED</p>
                  <p><?php echo date("d F Y", strtotime($projReleaseDate)); ?></p>
                </li>
                <!--
                <li>
                  <p class="red opens_light ">PUBLISHER</p>
                  <p>Self-published </p>
                </li>-->
    ...
    ?>
            ...
            <div class="rightbox width560 position_relative fr ">
                    <div class="nav open_sans  width513 display_inline_block bt_ebeb  pt20 pb10  width100_per  text_alighC  "> 
                      <?php if(!empty($previousPageLink)){ ?>  
                        <a class=" fl text_alighL previous_collection" href="<?php echo $previousPageLink; ?>">Previous <?php echo $this->lang->line('musicNaudio_media_type_'.$categoryId); ?> </a> 
                      <?php }else{ ?>
                        <a class=" fl text_alighL disable_link previous_collection" href="javascript:void(0)">Previous <?php echo $this->lang->line('musicNaudio_media_type_'.$categoryId); ?> </a> 
                      <?php  } ?>
                      ...
                      <?php if(!empty($nextPageLink)){ ?> 
                        <a class=" fr text_alignR next_collection" href="<?php echo $nextPageLink; ?>">Next <?php echo $this->lang->line('musicNaudio_media_type_'.$categoryId); ?></a>
                      <?php }else{ ?>
                        <a class=" fr text_alignR disable_link next_collection" href="javascript:void(0)">Next <?php echo $this->lang->line('musicNaudio_media_type_'.$categoryId); ?></a>
                       <?php  } ?> 
                    </div>
                    <div ></div>
                     <ul class="socail_list pt10 bt_ebeb ">
                      <li>  
                           <?php 
                                echo ' <span class="fl">';
...
<script>
    ...
    $('#playershow').load(function(){
        ...
        //set player title
        var gettracktitle= $("#gettracktitle").html();
        iframe.find(".hulu  .trackname").html(gettracktitle);
    });

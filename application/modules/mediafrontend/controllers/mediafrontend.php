...
class mediafrontend extends MX_Controller {
	 ...
	 */
	 function __construct() {
		//Load required Model, Library, language and Helper files
            ...
            $this->projEntityId = getMasterTableRecord('Project');
    ...
    */ 
    ...
    function downloadfilefree($fileDescription=''){
		$this->userId= $this->isLoginUser();
		...
		if($userId > 0 && $projectId > 0 && $industryType != ''){
			...
			$this->data['elements']=$this->model_media->freedonwloadfilelist($FD);
			...
			$this->new_version->load('new_version','download_file_list_free',$this->data);
		}else{
			redirect('home');
		}
	}
    ...
    */ 
    ...
    function downloadfilenew($fileDescription=''){
        ...
        //check downloadId and get data
        if(is_numeric($dwnId) && $dwnId >0){
            $where          =   array('dwnId'=>$dwnId,'userId'=>$this->userId);
            $downloadData   =   $this->model_common->getDataFromTabel('SalesItemDownload', '*', $where, '', 'dwnId', 'DESC', 1);
            
            if(!empty($downloadData )){
                $downloadData           =   $downloadData[0];
                
                $dwnMaxday = $downloadData->dwnMaxday;
                $expiryDate= getPreviousOrFututrDate($downloadData->dwnDate, '+'.$dwnMaxday.' day' ,$format='d F Y H:i:s');
		...
		if($userId > 0 && $projectId > 0 && $industryType != ''){
			...
			$this->data['elements']=$this->model_media->paiddownloadfiles($FD);
    ...
    */ 
    ...
    function ppvfile($fileDescription=''){
        ...
        $userId=(isset($FD[0]) && ($FD[0] >0))?$FD[0]:0; 
        $projectId=(isset($FD[1]) && ($FD[1] >1))?$FD[1]:0; 
        $industryType=isset($FD[2])?$FD[2]:'';
        ...
        if($userId > 0 && $projectId > 0 && $industryType != ''){
            ...
            $FD=array(
                ...
                'userId'=>$userId,
                'elementTable'=>$elemetTable,
                'elementId'=>($elementId==$projectId)?0:$elementId,
                'dwnId'=>$dwnId,
                'isPPV'=>1
            );
            
            
            $this->data['elements']=$this->model_media->ppvfilelisting($FD);
    ...
    public function selecteddownload(){
        ...
        if($this->input->post()){
           ...
           $elementHeading  = $this->input->post('elementHeading');
            if(!empty($fileId)){
                
                $prepareFileList    =   array();
                $downloadFileList   =   $this->model_common->getDataFromTabelWhereIn('MediaFile', '*', 'fileId', $fileId, 'fileId', 'ASC');
                
                if(!empty($downloadFileList)){
                    
                    foreach($downloadFileList as $downloadFile){
                        
                        $prepareFileList[] = $downloadFile['filePath'].$downloadFile['fileName'];
                    }
                   
                    //prepare folder name
                    $fileName = str_replace(" ","_",$elementHeading);
                    $fileName = strtolower($fileName)."_".time();
                    //call file zip create helper
                    selectedfileszip($prepareFileList,$fileName);
    ...
    /*
    * @access: public
    * @description: This method is use to play pay per view file view
    ...
    */ 
    ...
    public function editfilmvideocollection() {
        ...
        $this->_geteditprojectcollection('filmNvideo','media_project_new_view_list');
    }
    ...
    */ 
    ...
    public function editmusicaudiocollection() {
        //set require data
        $this->data['navigationLink1']                =   'musicaudiocollection';
        $this->data['navigationLink2']                =   'musicaudiodeleted';
         $this->data['isEdit']                         =   TRUE;
        
        $this->_geteditprojectcollection('musicNaudio','music_audio_project_list');
    }
    ...
    */ 
    ...
    public function editphotoartcollection() {
        //set require data
        $this->data['navigationLink1']                =   'photoartcollection';
        $this->data['navigationLink2']                =   'photoartdeleted';
         $this->data['isEdit']                         =   TRUE;
        
        $this->_geteditprojectcollection('photographyNart','photography_art_project_list');
    }
    ...
    */ 
    ...
    public function editwritingpubcollection() {
        //set require data
        $this->data['navigationLink1']                =   'writingpubcollection';
        $this->data['navigationLink2']                =   'writingpubdeleted';
         $this->data['isEdit']                         =   TRUE;
        
        $this->_geteditprojectcollection('writingNpublishing','writing_publishing_project_list');
    }
    ...
    */ 
    ...
    public function editeducationalcollection() {
        //set require data
        $this->data['navigationLink1']                =   'educationalcollection';
        $this->data['navigationLink2']                =   'educationaldeleted';
         $this->data['isEdit']                         =   TRUE;
        
        $this->_geteditprojectcollection('educationMaterial','educational_project_list');
    }
    ...
    */ 
    ...
    private function _getprojectcollection($frentendUserId=0,$industryType="filmNvideo",$innerViewName){
        ...
        $fullProjectListData           =   $this->model_media->projectlist($loggedUserId,$industryType,$projectId); 
        
        //Paginaation functionality
        $pages                           =  new Pagination_new_ajax;
        $pages->items_total              =  count($fullProjectListData); // this is the COUNT(*) query that gets the total record count from the table you are querying
        $this->data['perPageRecord']     =  $this->config->item('perPageRecord');
        
        
         // Add by Amit to set cookie for Results per page
        if($this->input->post('ipp')!=''){
            $isCookie = setPerPageCookie('mediaListingView',$this->data['perPageRecord']);	
        }else {
            $isCookie = getPerPageCookie('mediaListingView',$this->data['perPageRecord']);		
        }
        ...
        $this->data['projectListingData']           =   $this->model_media->projectlist($frentendUserId,$industryType,$projectId,'f',$pages->offst,$pages->limit); 
        ...
        if($this->input->post('ajaxRequest')){
            $innerViewName =  $this->input->get('innerViewName');
            $this->load->view($innerViewName,$this->data);
        }else{
    ...
    */ 
    ...
    private function _geteditprojectcollection($industryType="filmNvideo",$innerViewName){
        ...
        $fullProjectListData           =   $this->model_media->editprojectlist($loggedUserId,$industryType,$projectId); 
        $fullProjectListCount          =   count($fullProjectListData);
        
        //check single record then redirect direct to project edit page
        if($fullProjectListCount == 1 && !empty($fullProjectListData)){
            $fullProjectListData = $fullProjectListData[0];
            $this->_singlerecordredirect($industryType,$fullProjectListData['projId']);
        }
        
        //Paginaation functionality
        $pages                           =  new Pagination_new_ajax;
        $pages->items_total              =  $fullProjectListCount; // this is the COUNT(*) query that gets the total record count from the table you are querying
        $this->data['perPageRecord']     =  $this->config->item('perPageRecord');
        ...
        //call model for project data
        $this->data['projectListingData']           =   $this->model_media->editprojectlist($loggedUserId,$industryType,$projectId,'f',$pages->offst,$pages->limit); 
        $this->data['frentendUserId']               =   $frentendUserId; // set frentendUserId
        $this->data['industryType']                 =   $industryType; // set industryType
        $this->data['fileConfig']                   =   $this->config->item($industryType.'FileConfig');	//load language data
        $this->data['loggedUserId']                 =   $loggedUserId;	//load language data
        $this->data['packagestageheading']          =   $this->lang->line($industryType.'_showcase');
        $this->data['innerViewName']                =   $innerViewName;
        $this->data['navigationMenu1']              =   true;
        $this->data['isDeleteView']                 =   false;
    ...
    /*
    * @description: This method is use to check single project and redirect
    * @param: $industryType
    * @param: projectId
    * @auther: lokendra meena
    * @return: void
    */ 
    ...
    private function _singlerecordredirect($industryType,$projectId){
        
        switch($industryType){
            
            case "filmNvideo":
                $methodName = 'filmvideo';
            break;
            
            case "musicNaudio":
                $methodName = 'musicaudio';
            break;
            
            case "photographyNart":
                $methodName = 'photographyart';
            break;
            
            case "writingNpublishing":
                $methodName = 'writingpublishing';
            break;
            
            case "educationMaterial":
                $methodName = 'educationmaterials';
            break;
        ...
        $redirecUrl = base_url_lang('media/'.$methodName.'/editproject/'.$projectId);
    ...
    */ 
    ...
    public function writingpubdeleted() {
        ...
        $this->_getdeletedcollection('writingNpublishing','writing_publishing_project_list');
    }
    ...
    */ 
    ...
    private function _getdeletedcollection($industryType,$innerViewName){
        ...
        //call model for project data
        $this->data['projectListingData']           =   $this->model_media->projectlist($loggedUserId,$industryType,$projectId,'t'); 
    ...
    */ 
    ...
    public function _projectdata($userId,$projectId){
        ...
        if(!previewModeActive()){
            ...
            $where              =   array('tdsUid'=>$userId,'projId'=>$projectId);
        }
    ...
    */ 
    ...
    public function mediashowcases($frentendUserId=0, $projectId=0){
        ...
        //only logged user can see his project preivew
        $frentendUserId =  (previewModeActive())?isLoginUser():$frentendUserId;
    ...
    */ 
    ...
    public function collectionmedialist($frentendUserId,$projectId){
        ...
        $pages->items_per_page=($this->input->post('ipp') > 0) ? $this->input->post('ipp'):$isCookie ;
        $pages->paginate($isShowButton=true,$isShowNumbers=true,$showActiveNumberonly=true);
    ...
    */ 
    ...
    private function _mediashowcasesdetails($frentendUserId=0, $projectId=0,$industryType,$viewName,$offSet=0,$perPage=0){
        ...
        /*For Manage view counts */
		if((isset($projectId)) && (!empty($projectId)) && (isset($industryType)) && (!empty($industryType))){
			/*Get section Id*/
			$sectionId = $this->config->item($industryType.'SectionId');
			manageViewCount($this->projEntityId,$projectId,$frentendUserId,$projectId,$sectionId);
		}	
    ...
    */ 
    ...
    public function albumplaylist(){
        ...
        $getProjectDetails       =   $this->model_common->getDataFromTabel($table='Project', $field='tdsUid,projId, projCategory', $where, '', 'projId', 'DESC', $limit=1 );
        ...
        $this->data['frentendUserId']                =   $getProjectDetails->tdsUid;	//load language data
    ...
    */ 
    ...
    private function _mediadetailsdata($frentendUserId, $projectId,$elementId,$industryType,$viewName){
		...
		if((isset($projectId)) && (!empty($projectId)) && (isset($industryType)) && (!empty($industryType))){
			/*Get section Id*/
			$sectionId = $this->config->item($industryType.'SectionId');    
            $entityId = ($elementId > 0) ? $elementEntityId : $this->projEntityId;
            $projelementId = ($elementId > 0) ? $elementId : $projectId;
			manageViewCount($entityId,$projelementId,$frentendUserId,$projectId,$sectionId);
		}
    ...
    */ 
    ...
    public function mediagallery($frentendUserId=0, $projectId=0, $elementId=0){
        ...
        //gallery image view
        if($this->input->post('galleryelementview')===false){
            $loadViewName = 'media_gallery_list';
        }else{
            $loadViewName = 'media_gallery_list_view';
        }
        
        //call method for film and video
        $this->_mediagallerydata($frentendUserId, $projectId, $elementId, $industryType,$loadViewName);
    }
    ...
    */ 
    ...
    public function photoartgallery($frentendUserId=0, $projectId=0, $elementId=0){
        ...
        if($this->input->post('galleryelementview')===false){
            ...
            //load view name
            $loadViewName = 'photo_art_gallery_list';
        }else{
            //load view name
            $loadViewName = 'photo_art_gallery_list_view';
        }
    ...
    */ 
    ...
    private function _mediagallerydata($frentendUserId, $projectId,$elementId,$industryType,$viewName){
        ...
        //check if "viewgallery" post then load only view
        
        if($this->input->post('galleryelementview')){
            $this->data['galleryelementview']  = $this->input->post('galleryelementview');
            $this->load->view($viewName,$this->data);
        }else{
            $this->new_version->load('new_version',$viewName,$this->data);
        }
    ...
    */ 
    ...
    /*
     * @description: This method is use to upload media by crop
     * @auther:lokendra meena
     * @return:void
     */ 
    ...
    public function uploaddemo(){
        ...
        //add css for guillotine
        $this->head->add_css($this->config->item('template_new_css').'jquery.guillotine.css');	
        
        //js add for guillotine
        $this->head->add_js($this->config->item('template_new_js').'jquery.guillotine.js',NULL,'lastAdd');
        
        $data['dirUploadMedia'] = 'media/'.LoginUserDetails('username').'/dempupload/';
        $data['dirUploadMedia'] = 'media/dempupload/';
        
        $imgThumbPath =  $data['dirUploadMedia'];
        ...
        $this->new_version->load('new_version','uploaddemo',$data);
    }
    
    
    public function cropupload(){
        $fileName       =  $this->input->post("fileName");
        $fileUploadPath =  $this->input->post("fileUploadPath");
        ...
        $newImageName = basename($fileName, $ext) . '_crop' . $ext;
        file_put_contents($fileUploadPath.$newImageName, $imageData);
    ...
    */
    ...
    private function _pluploadjsandcss(){
        ...
        $this->head->add_js($this->config->item('system_js').'new-jquery-upload/plupload.full.min.js');
    }
    ...
    public function uploadgallery(){
        ...
        $this->new_version->load('new_version','uploaddemogallery',$data);
    }

...
$editUrl = base_url('event/eventwithlaunch/eventform/'.$maineventid.'/'.$NatureId.'/'.@$launchEventData['LaunchEventId']);
$previewUrl = base_url('event/previewEvent');
$addUrl = $sessionUrl = base_url('event/eventwithlaunch/eventsession/'.$maineventid);
...
if(isset($EventId) && @$EventId>0) {
	...
	$dirnameEvent = 'media/'.LoginUserDetails('username').'/events/'.$maineventid.'/';
	
	$dirSize=0;
	
	// in case of launch with event also get used size of associted launch
	if(isset($launchEventData['LaunchEventId']) && $launchEventData['LaunchEventId'] > 0){
		$dirnameLaunch = 'media/'.LoginUserDetails('username').'/launchevents/'.$launchEventData['LaunchEventId'].'/';
...
?>
<div class="row">
<div class="main_project_heading"><div class="Main_heading_new fl"> <?php echo @$Title;?></div>
<div class="btn_outer_wrapper fr width_auto pl5 mr14">
<div class="fr">
<?php 
	if($isArchive=='t'){
		$eventLaunchUrl=base_url("event/launchwithevent/deletedItems/".$EventId);
	}else{
		$eventLaunchUrl=base_url("event/launchwithevent/eventlaunchdetail/".$EventId);
	}
?>
<div class="tds-button-big Fleft">
	<a onmouseover="mouseover_big_button(this)" onmouseup="mouseup_big_button(this)" onmousedown="mousedown_big_button(this)" href="<?php echo $eventLaunchUrl;?>" ><span >Launch</span></a>
	<a style="background-position: 0 -96px;" href="javascript://void(0);"><span style="background-position: right -96px;">Event</span></a>
</div>
...
<?php
	...
	$sectionArray = array('NatureId'=>$NatureId,'activeEventId'=>@$EventId,'isArchive'=>$isArchive);
	echo Modules::run("event/eventsLeftSection",$sectionArray);
	
if(!isset($EventId) || $EventId<=0) {
$sectionId=$this->config->item('eventswithLaunchSectionId');
$returnUrl='/event/eventwithlaunch/eventform';	
?>
...
<?php
	...
	//Line Or Online Event with Launch
           ...
           <div class="toggle_btn"></div>
              <div class="blog_box bg-non-color">
                ...
                <div class="cell blog_left_wrapper width_395  pr16">
                  ...
                  <div class="row"> <b class="orange_color"><?php echo $this->lang->line('eventOrganiser');?></b> <span class="event_organiser_name"><?php echo $userInfo['userFullName'];?></span>
                  ...
                  <div class="row"> <b class="orange_color"><?php echo $this->lang->line('project_logLineDescription');?></b>
                    ...
                    <p><?php echo getSubString($Tagwords, 125);?></p>
                  </div>
                ...
                <div class="cell blog_right_wrapper">
                  <div class="blog_link2_wrapper">
                    ...
                    <div class="tds-button-top modifyBtnWrapper"> 
						<?php
						$session_url='event/'.$this->router->method.'/eventsession/'.$EventId;

						$addsessionForm = array('id'=>'addsessionForm','name'=>'addsessionForm');
						echo form_open($session_url,$addsessionForm); 	
						echo '<input name="currentSessionId" value="addsession" type ="hidden" />';
						...
						if($isBlocked != 't' && $isExpired != 't'){ ?>
							 ...
							 <!--Replace un archived icon with restore icon -->
									 <a class="formTip" href="javascript:void(0);" title="<?php echo $this->lang->line('restore');?>" onclick="moveFromArchive('','Events','EventId','<?php echo $maineventid; ?>','EventArchive','/event/eventwithlaunch/deletedItems','','LaunchEvent','EventId','isArchive');"><span><div class="restore_btn_icon"></div></span></a> 
						
								<!--<a class="formTip" href="javascript:void(0);" title="<?php //echo $this->lang->line('unArchive');?>" onclick="moveFromArchive('','Events','EventId','<?php //echo $maineventid; ?>','EventArchive','/event/eventwithlaunch/deletedItems','','LaunchEvent','EventId','isArchive');"><span><div class="blogUnArchiveIcon"></div></span></a>-->
								<?php
							}
							if($isBlocked != 't'){
							...
							echo anchor('javascript://void(0);', '<div class="projectAddIcon"></div>',array('onclick'=>"$('#addsessionForm').submit();"));
							?>	
							...
							<a href="javascript://void(0);" class="formTip" onclick="moveInArchive('','Events','EventId','<?php echo $maineventid; ?>','EventArchive','isPublished','/event/eventwithlaunch/eventwithlaunchdetail/','','LaunchEvent','EventId','isArchive','isPublished')"  title="<?php echo $this->lang->line('delete');?>"><span><div class="projectDeleteIcon"></div></span></a>
                  ...
                  <div class="blog_link3_wrapper">
                    ...
                    <div class="blog_link3_box pt8 pl7">
                      <?php
						...
						if(@$launchEventData['LaunchEventId']>0) 
							$thisLaunchId = $launchEventData['LaunchEventId'];
						else 
							$thisLaunchId = null;
							
						$meetingPointUrl = $this->config->site_url().$this->router->class.'/eventwithlaunch/meetingpoint/'.$EventId;
                ...
                <?php
					...
					if($isArchive=='f' && $isBlocked=='f'){
						$whereArray = array('eventId'=>$maineventid);
						...
						$notificationArray = array('entityId'=>$entityId,'elementId'=>$maineventid,'industryId'=>$Industry,'projectType'=>'performancesevents','alert_type'=>'event_with_launch');

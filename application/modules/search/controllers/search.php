...
class Search extends MX_Controller{
    ...
    public function searchMediaResult($loadView=false,$viewPage='result',$prosectionId=0) {
     ...
     public function advance(){
        $this->data['sectionList'] = $this->config->item('search_section');
        $this->data['languageList'] = getlanguageList();
        $this->data['countryList'] = getCountryList();
        $this->data['sectionCategoryList']=false;
        $this->data['sectionCategoryList']=false;
        ...
        $search_section_id = $this->config->item('search_section_id');
        if($search_section_id && is_array($search_section_id) && count($search_section_id) > 0 ){
            foreach($search_section_id as $sectionId ){
                if($sectionId>0){
                    $this->data['sectionCategoryList'][$sectionId]=getProjCategoryList($sectionId, $lang,false);
                    if($this->data['sectionCategoryList'][$sectionId] && is_array($this->data['sectionCategoryList'][$sectionId]) && count($this->data['sectionCategoryList'][$sectionId]) > 0 ){
                        foreach($this->data['sectionCategoryList'][$sectionId] as $catId=>$category ){
		...
		$this->data['result']=$this->result(true);
        $this->new_version->load('new_version','advance/search',$this->data);
    }
    
   
    public function result($loadView=false){
        $post=$this->input->post();
        ...
        $table='"'.$tableSearch.'" ';
        $table.=' LEFT JOIN "'.$tableLogSummary.'" ON ("'.$tableLogSummary.'"."entityId" = "'.$tableSearch.'"."entityid" AND "'.$tableLogSummary.'"."elementId" = "'.$tableSearch.'"."elementid")';
        $field='(item).*, "'.$tableSearch.'".*, "'.$tableLogSummary.'"."reviewCount", "'.$tableLogSummary.'"."viewCount", "'.$tableLogSummary.'"."craveCount", "'.$tableLogSummary.'"."ratingAvg", "'.$tableLogSummary.'"."imageFileCount", "'.$tableLogSummary.'"."printCount", "'.$tableLogSummary.'"."docFileCount", "'.$tableLogSummary.'"."docCount", "'.$tableLogSummary.'"."audioFileCount", "'.$tableLogSummary.'"."cdCount", "'.$tableLogSummary.'"."videoFileCount", "'.$tableLogSummary.'"."dvdCount"';
        
        if($this->userId > 0){
            $table.=' LEFT JOIN "'.$tableLogCrave.'" ON ("'.$tableLogCrave.'"."entityId" = "'.$tableSearch.'"."entityid" AND "'.$tableLogCrave.'"."elementId" = "'.$tableSearch.'"."elementid" AND "'.$tableLogCrave.'"."tdsUid" = '.$this->userId.')';
            $field.=', "'.$tableLogCrave.'"."craveId" ';
        }
        $where=" WHERE ispublished='t'  "; 
        ...
        if(isset($post) && is_array($post) && !empty($post)){    
            ...
            if(isset($post['languageId']) && ((int)$post['languageId'] > 0)  ){
				$where.=" AND (item).languageid =  ".$post['languageId']." ";
			}
            
            if(isset($post['producedInCountry']) && ((int)$post['producedInCountry'] > 0)  ){
				$where.=" AND (item).countryid =  ".$post['producedInCountry']." ";
			}
            
            $sectionId = isset($post['sectionId'])?trim($post['sectionId']):'';
            if(!empty($sectionId) && ($sectionId !== 0)){
                $city = isset($post['city'])?trim($post['city']):'';
                $projectPart = isset($post['projectPart'])?trim($post['projectPart']):'';
                $sectionId2 = isset($post['sectionId2'])?trim($post['sectionId2']):0;
                $industryId = isset($post['industryId'])?trim($post['industryId']):0;
                $typeId = isset($post['typeId'])?$post['typeId']:false;
                $GenreId = isset($post['GenreId'])?$post['GenreId']:false;
                switch($sectionId){
                    case 'member':
                        if(!empty($city)){
                            $where.=" AND (item).city ILIKE  '%".$city."%' ";
                        }
                        if(!empty($projectPart) && ($projectPart !== 0)){
                            if(is_numeric($projectPart) && ($projectPart > 0) ){
                                $memberSection = $this->config->item('sectionId'.$projectPart);
                                $where.=" AND sectionid = ".$projectPart."  AND section = '".$memberSection."' ";
                                if(is_numeric($industryId) && ($industryId > 0)){
                                    $where.=" AND (item).industryid =  ".$industryId." ";
                                }
                            }elseif(strpos($projectPart, '-') !== false){
                                $parts=explode('-',$projectPart);
                                if($parts[1] == 'top10craved'){
                                    $memberSection = $this->config->item('sectionId'.$parts[0]);
                                    $where.=' AND sectionid = '.$parts[0].' AND section = \''.$memberSection.'\' AND ("'.$tableLogSummary.'"."craveCount" > 0) ';
                                    $orderBy=' ORDER BY "'.$tableLogSummary.'"."craveCount" DESC';
                                    $limit=' limit 10 ';
                                }else{
                                    $where.=" AND sectionid = ".$parts[0]."  AND section = '".$parts[1]."' ";
                                }
                            }else{
                                $where.=" AND sectionid IN(6,7,8,34) AND section IN('creatives','associatedprofessionals','enterprises','fans') ";
                            }
                        }else{
                            $where.=" AND sectionid IN(6,7,8,34) AND section IN('creatives','associatedprofessionals','enterprises','fans') ";
                        }
                    ...
                    case 'media':
                       if(is_numeric($sectionId2) && ($sectionId2 > 0) ){
                            if(!empty($projectPart) && ($projectPart !== 0)){
                                $mediaSection = $this->config->item('sectionId'.$sectionId2);
                                if(is_numeric($projectPart) && ($projectPart > 0) ){
                                    $where.=" AND sectionid = ".$sectionId2."  AND section = '".$mediaSection."' ";
                                }elseif(strpos($projectPart, '-') !== false){
                                    $parts=explode('-',$projectPart);
                                    ...
                                    switch($partsLength){
                                        case 4:
                                            $where.=" AND sectionid = ".$sectionId2." AND section = '".$mediaSection."' ";
                                            //$where.=" AND (item).categoryid =  '".$parts[1]."' ";
                                            if($parts[3] == 'elements' && $GenreId && is_array($GenreId) && !empty($GenreId)){
                                                $genreid = implode(',',$GenreId);
                                                $where.=" AND (item).genreid IN(". $genreid.") ";
                                            }else{
                                                $where.=" (item).typeid = '".$parts[2]."' ";
                                            }
                                        break;
                                        
                                        case 3:
                                            $where.=" AND sectionid = ".$sectionId2." AND section = '".$mediaSection."' ";
                                            if($parts[2] == 'elements' && $typeId && is_array($typeId) && !empty($typeId)){
                                                $typeid = implode(',',$typeId);
                                                
                                                $where.=" AND CAST((item).typeid as int) IN(". $typeid.") ";
                                                if($GenreId && is_array($GenreId) && !empty($GenreId)){
                                                    $genreid = implode(',',$GenreId);
                                                    $where.=" AND CAST((item).genreid as int) IN(". $genreid.") ";
                                                }
                                            }elseif($parts[2] == 'free'){
                                                $where.=" AND (item).sell_option =  '".$parts[2]."' ";
                                                $where.=" AND (item).categoryid =  '".$parts[1]."' ";
                                            }
                                        break;
                                        ...
                                        case 2:
                                            if(is_numeric($parts[1]) && ($parts[1] > 0)){
                                                $where.=" AND sectionid = ".$sectionId2." AND section = '".$mediaSection."' ";
                                                $where.=" AND (item).categoryid =  '".$parts[1]."' ";
                                            }elseif($parts[1] == 'top10craved'){
                                                 $where.=' AND sectionid = '.$sectionId2.' AND section = \''.$mediaSection.'\' AND ("'.$tableLogSummary.'"."craveCount" > 0) ';
                                                 $orderBy=' ORDER BY "'.$tableLogSummary.'"."craveCount" DESC';
                                                 $limit=' limit 10 ';
                                            }else{
                                                $where.=" AND sectionid = '".$parts[0]."'  AND section = '".$parts[1]."' ";
                            ...
                            $where.=" AND sectionid IN(1,2,3,4,10) AND section IN('filmNvideo','musicNaudio','writingNpublishing','photographyNart','educationMaterial') ";
                        }
                    break;
                    
                    case 13:
                        $blogSection = $this->config->item('sectionId'.$sectionId);
                        $where.=" AND sectionid = ".$sectionId."  AND section = '".$blogSection."' ";
                       
                        if(is_numeric($industryId) && ($industryId > 0) && ($industryId != $sectionId)){
                            $where.=" AND (item).industryid =  ".$industryId." ";
                        }elseif($industryId == 'others'){
                            $where.=" AND (item).industryid NOT IN(1,2,3,4,5,10) ";
                        }
                    
                    break;
                    
                    case 35:
                        $ENsection = $this->config->item('sectionId'.$sectionId);
                        $where.=" AND sectionid = ".$sectionId."  AND section = '".$ENsection."' ";
                    ...
                    case 36:
                        $FSsection = $this->config->item('sectionId'.$sectionId);
                        $where.=" AND sectionid = ".$sectionId."  AND section = '".$FSsection."' ";
                ...
                $where.=" AND sectionid IN(1,2,3,4,6,7,8,10,13,34) AND section IN('filmNvideo','musicNaudio','writingNpublishing','photographyNart','educationMaterial','creatives','associatedprofessionals','enterprises','fans','blog') ";
            }
        }else{
            $where.=" AND sectionid IN(1,2,3,4,6,7,8,10,13,34) AND section IN('filmNvideo','musicNaudio','writingNpublishing','photographyNart','educationMaterial','creatives','associatedprofessionals','enterprises','fans','blog') ";
        }
        ...
        $pages = new Pagination_new_ajax;
        $pages->items_total = $this->model_common->getDataFromMixTabel($table, 'id',  $where.$limit, '', '',true ); // this is the COUNT(*) query that gets the total record count from the table you are querying
        // echo $this->db->last_query();die();
        $this->data['perPageRecord'] = $this->config->item('perPageRecord');
       ...
       $this->data['advertSectionId'] = $advertSectionId; //set advert section id
        ...
        $result = $this->load->view('search/advance/result',$this->data,$loadView);
        if($loadView){
           return  $result;
        }
	}
    ...
    */ 
    ...
    public function reportaproblemindustry(){
        
        $sectionValue    = $this->input->post('sectionValue');
        $sectionValueArray = explode(",",$sectionValue);
        if(count($sectionValueArray) > 1 || $sectionValue=="0"){
            
            $sectionList     =  getSelectedSearchIndustryList($sectionValueArray);
            echo form_dropdown('sectionId', $sectionList, $sectionId,'id="sectionId" class="width258"');
        }else{

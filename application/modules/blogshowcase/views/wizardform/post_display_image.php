...
$mediaFileTypes     =   $this->config->item('imageAccept');
 $fileNameInput = array(
    ...
    //next page url 
    $nextUrl  =  base_url(lang().'/blogshowcase/posttitlendescription/'.$blogUserId.'/'.$parentPostId.'/'.$blogId.'/'.$postId);
?>
...
<div class="content TabbedPanelsContent width635 m_auto">
    ...
    <?php 
    // set back url
    $data['backPage'] =  '/blogshowcase/addpost/'.$postId;
    // set skip url
	if(!empty($postData['fileId']) || !empty($postData['rawFileName'])) {
        $data['skipPage'] = '/blogshowcase/posttitlendescription/'.$blogUserId.'/'.$parentPostId.'/'.$blogId.'/'.$postId;
    }
    // set next form name

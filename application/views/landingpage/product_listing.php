...
if(isset($data) && is_object($data) && count($data) > 0){
			...
			if($data->catId==1){
				$section =$this->lang->line('forsale');
				$defaultImage = $this->config->item('defaultProductForSale_m');
			} elseif($data->catId==2){
				$section = $this->lang->line('wanted');
				$defaultImage = $this->config->item('defaultProductWanted_m');
			} else {
				$section = $this->lang->line('freeStuff');
				$defaultImage = $this->config->item('defaultProductFree_m');
			}
		...
		//----------make element default project image code start---------//
			...
			<a href="<?php echo $recordDetailUrl;?>"  target="_blank" >
				<div class="cnt_box  bg_fff  ">
                  ...
                  <div class="p15">
                     <h4 class="fs17 pt3 font_bold lineH20"> <?php echo getSubString($data->productTitle,40);?></h4>
                     <p><?php echo $data->productOneLineDesc;?> 
                     </p>
                  </div>

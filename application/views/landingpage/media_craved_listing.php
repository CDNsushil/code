...
if(isset($data) && is_object($data) && count($data) > 0){
    ...
    switch($frontendMathod){
        case 'filmvideo':
            $recordDetailUrl = base_url(lang().'/mediafrontend/mediadetails/'.$data->tdsUid.'/'.$data->projId.'/'.$data->elementId);
        break;
        
        case 'musicaudio':
            $recordDetailUrl = base_url(lang().'/mediafrontend/tracklist/'.$data->tdsUid.'/'.$data->projId.'/'.$data->elementId);
        break;
        
        case 'photographyart':
            $recordDetailUrl = base_url(lang().'/mediafrontend/photoartelement/'.$data->tdsUid.'/'.$data->projId.'/'.$data->elementId);
        break;
        
        case 'writingpublishing':
            $recordDetailUrl = base_url(lang().'/mediafrontend/writingelement/'.$data->tdsUid.'/'.$data->projId.'/'.$data->elementId);
        break;
        
        case 'educationmaterial':
            $recordDetailUrl = base_url(lang().'/mediafrontend/educationelement/'.$data->tdsUid.'/'.$data->projId.'/'.$data->elementId);
        break;

...
class aanaBase{
	...
	*/
	public function isLoginUser(){
		...
		if($user_id > 0){
			$userPackageKey=$this->ci->input->post('userPackageKey');
			if(is_numeric($userPackageKey)){
				$this->ci->session->set_userdata('userSelectedPackageKey',$userPackageKey);			
			}else{
				//echo "unset";
			}
			return $user_id;
		}else{
			$isAjaxHit=$this->ci->input->get('ajaxHit')>0?$this->ci->input->get('ajaxHit'):$this->ci->input->post('ajaxHit');
			if($isAjaxHit > 0){
				redirect('home');
				...
				set_global_messages($this->ci->lang->line('loginFirst'), 'error');
				redirect('home');
			}
		}
	}

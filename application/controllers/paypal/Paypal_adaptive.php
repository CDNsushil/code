...
class PayPal_Adaptive 
{
	...
	function __construct($DataArray)
	{
		...
		$this -> Sandbox = isset($DataArray['Sandbox']) ? $DataArray['Sandbox'] : '';
	...
	function BuildHeaders($PrintHeaders)
	{
	 ...
	 function BuildJsonHeaders($PrintHeaders)	{
		$headers = array(
						'X-PAYPAL-SECURITY-USERID: ' . $this -> APIUsername, 
						'X-PAYPAL-SECURITY-PASSWORD: ' . $this -> APIPassword, 
						'X-PAYPAL-SECURITY-SIGNATURE: ' . $this -> APISignature, 
	 ...
	 * @return	string
	 */
	function CURLRequest($Request, $APIName, $APIOperation,$format=false)
	{
		$curl = curl_init();
				...
				if($format==true){
				       curl_setopt($curl, CURLOPT_HTTPHEADER, $this -> BuildJsonHeaders(false)); 
				}else {                           
				      curl_setopt($curl, CURLOPT_HTTPHEADER, $this -> BuildHeaders(false));
				}		
	...
	function PaymentDetails($DataArray)
	{
		...
		// Call the API and load XML response into DOM
		$XMLResponse = $this -> CURLRequest($XMLRequest, 'AdaptivePayments', 'PaymentDetails',$format=true);
		return $XMLResponse;
		$DOM = new DOMDocument();
		$DOM -> loadXML($XMLResponse);

...
class Head
{
	...
	private function _render_meta()
	{
        ...
        if(ENVIRONMENT=='production'){
            //add pinterest meta code all website show only in production [added by lokendra] 
            $out .= meta('p:domain_verify', 'b8a30172bf81845aaf5ce57eb434a5e9').$this->_indent();
        }
	...
	private function _render_items($type)
	{
		...
		if ($type == 'css')
		{
			foreach($this->$type as $it)
			{
                ...
                $file = $it[0]!=NULL ? $it[0] : 'all';
				$media = $it[1]!=NULL ? $it[1] : 'all';
				$condition = isset($it[2])?$it[2]:NULL;
		...
		elseif ($type == 'js')
		{
			foreach($this->$type as $it)
			{
				...
				$file = isset($it[0])?$it[0]:NULL;
				$condition = isset($it[1])?$it[1]:NULL;
				$lastAdd = isset($it[2])?$it[2]:NULL;

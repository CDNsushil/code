...
/* LOG VISITOR ADD IP AND REMOVE AADED BY AMIT */
    ...
    */  
    ...
    /*
    *  @access: public
    *  @description: This method is use to get container project data
    *  @auther: tosif qureshi
    *  @return: void
    */  
    
    public function getContainerProjData($containerId=''){		
        $result = false;
        if(!empty($containerId)){
            $this->db->select('uc.elementId');
            $this->db->select('mtp.title');
            $this->db->from($this->tableUserContainer.' as uc');
            $this->db->join($this->tableMasterTsProduct.' as mtp','mtp.tsProductId = uc.tsProductId');
            $this->db->where('uc.userContainerId',$containerId);
            $result = $this->db->get()->result();
        }
        return $result;
	} 
    ...
    /*
    *  @access: public
    *  @description: This method is use to show mixed element
    *  @auther: lokendra
    *  @return: array
    */ 
    
    public function randomuserdata($userType='creative',$limit=1){
        $this->db->select($this->tableUserAuth.'.*');
        $this->db->select($this->tableUserShowcase.'.optionAreaName,'.$this->tableUserShowcase.'.profileImageName,'.$this->tableUserShowcase.'.stockImageId,'.$this->tableUserShowcase.'.creative,'.$this->tableUserShowcase.'.associatedProfessional,'.$this->tableUserShowcase.'.enterprise,'.$this->tableUserShowcase.'.enterpriseName,'.$this->tableUserShowcase.'.websiteUrl,'.$this->tableUserShowcase.'.showcaseId');
        $this->db->select($this->UserSellerSettings.'.seller_currency');
        $this->db->select($this->tableUserProfile.'.countryId,'.$this->tableUserProfile.'.image,'.$this->tableUserProfile.'.firstName,'.$this->tableUserProfile.'.lastName');
        ...
        $this->db->where($this->tableUserAuth.'.active','1');
        $this->db->where($this->tableUserProfile.'.image',NULL);
        $this->db->where($this->tableStockImages.'.stockFilename',NULL);
		$this->db->where($this->tableUserShowcase.'.isBlocked','f');
		$this->db->where($this->tableUserShowcase.'.isPublished','t');
        $this->db->where($this->tableUserShowcase.'.profileImageName !=',"");
        $this->db->where($this->tableUserShowcase.'.'.$userType,'t');
      
        $this->db->limit($limit);
        $this->db->order_by($this->tableUserAuth.'.tdsUid', 'RANDOM');
        $query = $this->db->get();
		return $query->result_array();  
    }
    ...
    /* 
    * @access: public
    * @description: This function is used to get project
    * @return array
    */ 
    function getmediaprojectrecord($elementTable='',$limit=1,$projectType='') { 
        $this->db->select('p.projName,p.elementImageId,p.projId,p.projTag,p.tdsUid');
        $this->db->select('et.elementId,et.imagePath');		
		$this->db->select('up.firstName,up.lastName');
		//$this->db->select('mf.filePath,mf.fileName,mf.rawFileName,mf.isExternal');	
        $this->db->from($this->tableProject. ' p');
        $this->db->join($elementTable. ' et', 'et.elementId = p.elementImageId');
        $this->db->join($this->tableUserProfile. ' up', 'up.tdsUid = p.tdsUid','left');
		//$this->db->join($this->tableMediaFile. ' mf', "mf.fileId = et.fileId");
        $this->db->where('p.elementImageId >', '0');
		// $this->db->where('mf.isExternal', 'f');
		$this->db->where('et.imagePath !=', '');
		$this->db->where('p.isBlocked', 'f');
		$this->db->where('p.isPublished', 't');
		$this->db->where('p.isArchive', 'f');
		if(!empty($projectType)) { 
			$this->db->where('p.projectType', $projectType);
		}
		$this->db->limit($limit);
        $this->db->order_by('p.projId', 'RANDOM');
        $query = $this->db->get();
        return $query->result_array();  
    }
    ...
    * @return array
    */ 
    function getramdonpostrecord($limit=1) { 
        $this->db->select('p.*');
    	$this->db->select('mf.filePath,mf.fileName,mf.rawFileName,mf.isExternal');	
        $this->db->select('up.firstName,up.lastName');
        $this->db->from($this->tablePost. ' p');
        $this->db->join($this->tableMediaFile. ' mf', "mf.fileId = p.postFileId");
        $this->db->join($this->tableUserProfile. ' up', 'up.tdsUid = p.custId','left');
        $this->db->where('p.postFileId >', '0');
		$this->db->where('mf.isExternal', 'f');
		$this->db->where('p.isBlocked', 'f');
		$this->db->where('p.isPublished', 't');
		$this->db->limit($limit);
        $this->db->order_by('p.postId', 'RANDOM');
        $query = $this->db->get();
        return $query->result_array();  
    }
    ...
    /* 
    * @access: public
    * @description: This function is used to get user's pending element files 
    * @return array
    */ 
    function getPendingConvertionFiles($userId,$elementTable)  {
		
		$this->db->select($elementTable.'.elementId,projId,title,isPublished');
		$this->db->select($this->tableMediaFile.'.fileId,filePath,fileName,fileType,isExternal,jobStsatus,statusDetail,tdsUid');
        $this->db->from($elementTable);
		$this->db->join($this->tableMediaFile, $this->tableMediaFile.'.fileId  = '.$elementTable.'.fileId');	
        $this->db->where($this->tableMediaFile.'.tdsUid', $userId);
        $this->db->where($this->tableMediaFile.'.jobStsatus', 'FAILS');
        //$this->db->where($this->tableMediaFile.'.jobStsatus !=', 'NEW');
        $this->db->where($this->tableMediaFile.'.isExternal', 'f');
        $this->db->order_by($elementTable.'.elementId','DESC');
        $query = $this->db->get();
        return $query->result(); 
	}
    ...
    */ 
    ...
    function updateSearch($data=array()){
        if(!empty($data['data']) && is_array($data['data']) ){
            $countData = count($data['data']);
            $table=$this->db->dbprefix('search');
            if(isset($data['searchId']) && (int)$data['searchId'] > 0){
                //update case
                $i=0;
                $updateString = '';
                foreach($data['data'] as $key=>$value){
                    ...
                    $field ="".trim($key)."";
                    $fieldValue ="'".trim($value)."'";
                    $updateString.=$field.' = '.$fieldValue;
                    if($i < $countData){
                        $updateString.=',';
                    }
                }
                if(!empty($updateString)){
                    $updatesql = 'UPDATE "'.$table.'" SET '.$updateString.' WHERE id = '.$data['searchId'].' ' ;
                    $query = $this->db->query($updatesql);
                    return $data['searchId'];
                } 
                
            }elseif(!isset($data['data']['isdeleted']) || $data['data']['isdeleted'] != 't'){
                // insert case
                $i=0;
                $field = '';
                $fieldValue='';
    ...
    function getProjStatusOfElement($tbl='', $where=array()){
        $ispublished = 'f';
       if(!empty($tbl) && is_array($where) && !empty($where)){
            $this->db->select('p.isPublished');
            $this->db->from($tbl.' e');
            $this->db->join('Project p', 'p.projId = e.projId');
            $this->db->where($where);
            $this->db->limit(1);
            $query = $this->db->get();
            $result=$query->result();
            if(isset($result[0]->isPublished)){
                $ispublished = ($result[0]->isPublished == 't')?'t':'f';
            }
        }
        return $ispublished;
    }
    function changeProjStatusOfElement($up=array(), $upWhere=array()){
        if(is_array($up) && !empty($up) && is_array($upWhere) && !empty($upWhere)){
            $this->db->where($upWhere);
			return $this->db->update('search' , $up);
       }
    }
    
    /*
    *  @description: This method is use to get list of user for your toadsqurae navigation
    *  @auther: lokendra meena
    *  @return: array
    */ 
    
    function usershowcaselist(){
        $this->db->select($this->tableUserAuth.'.*');
		$this->db->select($this->tableUserShowcase.'.*');
		$this->db->select($this->tableYourToadsquare.'.isCompleted');
		$this->db->from($this->tableUserAuth);
		$this->db->join($this->tableUserShowcase, $this->tableUserShowcase.".tdsUid = ".$this->tableUserAuth.".tdsUid", 'left');
		$this->db->join($this->tableYourToadsquare, $this->tableYourToadsquare.'.tdsUid = '.$this->tableUserAuth.'.tdsUid AND "TDS_YourToadsquare"."entityid" = 93', 'left');
		$this->db->where($this->tableUserAuth.'.active', '1');
        $this->db->order_by($this->tableUserAuth.".tdsUid", "DESC");
		$query = $this->db->get();
     	return $query->result();
    }
    ...
    */ 
    ...
    function userbloglist(){
        $this->db->select($this->tableBlogs.'.*');
		$this->db->select($this->tableYourToadsquare.'.isCompleted');
		$this->db->from($this->tableBlogs);
		$this->db->join($this->tableYourToadsquare, $this->tableYourToadsquare.'.tdsUid = '.$this->tableBlogs.'.custId AND "TDS_YourToadsquare"."entityid" = 97', 'left');
		$this->db->order_by($this->tableBlogs.".blogId", "DESC");
		$query = $this->db->get();
     	return $query->result();
    }
    
     /*
    *  @description: This method is use to get list of user review for your toadsqurae navigation
    *  @auther: lokendra meena
    *  @return: array
    */ 
    ...
    function userreviewlist(){
		...
		$this->db->select($this->tableYourToadsquare.'.isCompleted as isReviewsCompleted');
		$this->db->from($this->tableProject);
		$this->db->join($this->tableYourToadsquare, $this->tableYourToadsquare.'.tdsUid = '.$this->tableProject.'.tdsUid AND "TDS_YourToadsquare"."sectionid" =  '."'3:2'", 'left');
		$this->db->order_by($this->tableProject.".projId", "DESC");
        $this->db->where($this->tableProject.'.projectType', 'reviews');
		$query = $this->db->get();
     	return $query->result();
    }
   ...
   /*
    *  @description: This method is use to get list of user news for your toadsqurae navigation
    *  @auther: lokendra meena
    *  @return: array
    */ 
    ...
    function usernewslist(){
        $this->db->select($this->tableProject.'.*');
		$this->db->select($this->tableYourToadsquare.'.isCompleted as isNewsCompleted');
		$this->db->from($this->tableProject);
		$this->db->join($this->tableYourToadsquare, $this->tableYourToadsquare.'.tdsUid = '.$this->tableProject.'.tdsUid AND "TDS_YourToadsquare"."sectionid" =  '."'3:1'", 'left');
		$this->db->order_by($this->tableProject.".projId", "DESC");
        $this->db->where($this->tableProject.'.projectType', 'news');
		$query = $this->db->get();
     	return $query->result();
    }
    ...
    */ 
    ...
    function userupcomminglist(){
        $this->db->select($this->upcomingProjectTableName.'.*');
		$this->db->select($this->tableYourToadsquare.'.isCompleted');
		$this->db->from($this->upcomingProjectTableName);
		$this->db->join($this->tableYourToadsquare, $this->tableYourToadsquare.'.tdsUid = '.$this->upcomingProjectTableName.'.tdsUid AND "TDS_YourToadsquare"."entityid" = 71', 'left');
		$this->db->order_by($this->upcomingProjectTableName.".projId", "DESC");
    	$query = $this->db->get();
     	return $query->result();
    }
   ...
   //----------------------------------------------------------------------
	...
	/*
	 * @access: public
	 * @description: This function is used to get showcase language
	 * @return array
	 */ 
   function getShowcaseLang($showcaseLangId=0) {	
		$result = false;
        if((int)$showcaseLangId > 0){
            $this->db->select('l.langId, l.Language_local');
            $this->db->from('UserShowcaseLang sl');
            $this->db->join('MasterLang l','l.langId = sl.langId');
            $this->db->where('sl.showcaseLangId',$showcaseLangId);	
            $this->db->limit(1);	
            $query = $this->db->get();
            $result = $query->result();
         }
   ...
   //----------------------------------------------------------------------
	 ...
	 * @return array
	 */ 
   function getBlogDataForPost($postId=0) {	
		$result = false;
        if((int)$postId > 0){
			$this->db->select('p.isUserProfileImage,p.custId');
            $this->db->select('b.blogId, b.rating, b.blogIndustry, l.langId, l.Language_local');
            $this->db->select('m.filePath,m.fileName,m.isExternal');
            $this->db->from('Posts p');
            $this->db->join('TDS_Blogs b','b.blogId = p.blogId');
            $this->db->join('MasterLang l','l.langId = CAST(b."blogLanguage" as int) ','left');
			$this->db->join('MediaFile m','m.fileId = p.postFileId', 'left');
            $this->db->where('p.postId',$postId);	
            $this->db->limit(1);	
            $query = $this->db->get();
            $result = $query->result();
         }
	...
	/*
	 * @access: public
	 * @description: This function is used to get workprofile data
	 * @return array
	 */ 
	function getWorkProfileDetails($tdsUid=0) {
		if($tdsUid > 0){
			 $tabelWorkprofile = $this->db->dbprefix($this->tabelWorkprofile);
			 $this->db->select($this->tabelWorkprofile.'.*');
			 $this->db->select($this->tableMediaFile.'.fileId,'.$this->tableMediaFile.'.filePath,'.$this->tableMediaFile.'.fileName');
			
			 $this->db->from($this->tabelWorkprofile);
			 $this->db->join($this->tableMediaFile,$this->tableMediaFile.'.fileId = '.$this->tabelWorkprofile.'.fileId', 'left');
			 $this->db->where($this->tabelWorkprofile.'.tdsUid',$tdsUid);

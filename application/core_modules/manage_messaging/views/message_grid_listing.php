...
<script type="text/javascript" charset="utf-8">
	...
	$(document).ready(function() {
	   ...
	   $.extend($.validator.messages, {
		...
		$('#example a.remove').live('click', function (e) {
			var removeConfirm = confirm('Are you sure you want to delete this message?');
	...
	function remove_msg_row(form_data) {
		...
		({
			type: "POST",
			url: BASEPATH+"manage_messaging/delete_msg",
			data: form_data,
			success: function(res)
			{	
				$("#show_updated").html('You have successfully remove the Message.').show().fadeOut(5000);
			}
		});
	}
			...
			<form action="<?php echo base_url('admin/settings/manage_countries/country_list_new'); ?>" method="post" accept-charset="utf-8" name="msgForm" id="msgForm" novalidate="novalidate">
				<table cellpadding="0" cellspacing="0" border="0" class="display inline-block" id="example">
					...
					if(!empty($messageLog))  {
						...
						{
							$msgLink = site_url().'admin/settings/manage_messaging/view_message/'.$messageLogDetail['id'];
						?>

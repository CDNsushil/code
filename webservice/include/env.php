...
final class env
{
  ...
  private static $ref;
	...
	*/
	...
	//$this->_con=pg_connect($conf->getKeyValue("SQL_HOST_NAME"),$conf->getKeyValue("SQL_USER_NAME"),$conf->getKeyValue("SQL_PASSWORD")) or die("can't connect to DB");
		
	$this->_con = pg_connect("host=".$conf->getKeyValue("SQL_HOST_NAME")." dbname=".$conf->getKeyValue("SQL_DB")." user=".$conf->getKeyValue("SQL_USER_NAME")  ." password=".$conf->getKeyValue("SQL_PASSWORD"));
		
		
		if($this->_con)
		{
		   // $link=pg_select_db($conf->getKeyValue("SQL_DB")) or die("cant use db".$conf->getKeyValue("SQL_DB"));
		   // log_message("debug","env::connect_db <==> ");
		}
		else
		{
				echo "DB Failed";
   ...
   /**
	...
	public function my_query($query, $errorMsg=''){
		...
		$result = pg_exec($this->_con,$query) or die($errorMsg.' '.mysql_error());         
	...
	*/
	 ...
	 * @RETURN ResultSet
	 */
	public function my_query_result($query, $errorMsg=''){
           $result = mysql_query($query) or die($errorMsg.' '.mysql_error());
           log_message("debug",__FILE__."<=>".__LINE__."::Query => ' ".$query." '\n");
           $data =array();
           if($this->my_num_rows($result) >= 1){
	 ...
	 * @RETURN ResultSet
	 */
	public function my_query_Object_result($query, $errorMsg=''){
           $result = mysql_query($query) or die($errorMsg.' '.mysql_error());
           log_message("debug",__FILE__."<=>".__LINE__."::Query => ' ".$query." '\n");
           $data =array();
           if($this->my_num_rows($result) >= 1){
			   while($row = $this->my_fetch_object($result)){
				   $data[]=$row;
			   }
		   }	
           return  $data;
	}//End my_query_result()
	//public function my_affected_rows($query, $errorMsg)
	//added by piyush jain
	public function my_affected_rows(){
	 ...
	 * @RETURN ResultSet
	 */
	public function select_assoc($val, $table, $where)
	{
	$query = 'SELECT  "'.$val.'" FROM  "'.$table.'" WHERE '.$where.'';
	...
	/**
	 * env::select_fetch_all
	 * @ACCESS public
         * @PARAM  (String)$query
         * @PARAM  (String)$errorMsg
	 * @RETURN ResultSet
	 */
	public function select_fetch_all($val, $table, $where)
	{
	$query = 'SELECT  "'.$val.'" FROM  "'.$table.'" WHERE '.$where.'';
	$result=$this->my_query($query);

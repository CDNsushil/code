...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : baseUrl+'templates/system/javascript/jquery-plugin/niceditor/nicEditIcons-latest.gif',
	buttonList : ['html','save','bold','italic','underline','left','center','right','justify','ol','ul','fontSize','fontFamily','fontFormat','indent','outdent','image','link','unlink','forecolor','bgcolor','gallimage'],
iconList : {"xhtml":1,"bgcolor":2,"forecolor":3,"bold":4,"center":5,"hr":6,"indent":7,"italic":8,"justify":9,"left":10,"ol":11,"outdent":12,"removeformat":13,"right":14,"save":25,"strikethrough":16,"subscript":17,"superscript":18,"ul":19,"underline":20,"image":21,"link":22,"unlink":23,"close":24,"gallimage":27}
});
;
...
function selectFile(page, parameter) {
	 	page = page + "?use=" + parameter;
		myWin = window.open(baseUrl+language+'/blog/viewGallery', 'tdFiles', 'width=640, height=430, status=no, menubar=no, location=no, resizable=yes, toolbar=no, scrollbars=yes');
	}
//var nicImageButton=nicEditorAdvancedButton.extend({addPane:function(){this.im=this.ne.selectedInstance.selElm().parentTag("IMG");this.addForm({"":{type:"title",txt:"Add/Edit Image"},src:{type:"text",txt:"URL",value:"http://",style:{width:"150px"}},alt1:{type:"title",txt:'<input type = "button" value = "Choose file" onclick="selectFile(\'listFiles.php\',\'src\');" />'},alt:{type:"text",txt:"Alt Text",style:{width:"100px"}},align:{type:"select",txt:"Align",options:{none:"Default",left:"Left",right:"Right"}}},this.im)},submit:function(B){var C=this.inputs.src.value;if(C==""||C=="http://"){alert("You must enter a Image URL to insert");return false}this.removePane();if(!this.im){var A="javascript:nicImTemp();";this.ne.nicCommand("insertImage",A);this.im=this.findElm("IMG","src",A)}if(this.im){this.im.setAttributes({src:this.inputs.src.value,align:this.inputs.align.value})}}});nicEditors.registerPlugin(nicPlugin,nicImageOptions);
...
/* START CONFIG */
var nicGallImageOptions = {
	buttons : {
		'gallimage' : {name : 'Insert Gallery Image', type : 'nicGallImageButton', tags : ['IMG']}
	}
...
var nicGallImageButton = nicEditorAdvancedButton.extend({
	width:"290px",	
	addPane : function() {
		this.im = this.ne.selectedInstance.selElm().parentTag('IMG');
		
		this.addForm({
			'' : {type : 'title', txt : '<div id="responseDiv" class="mt20"><img  class="ma" id="loadImg" align="absmiddle" src="'+baseUrl+'images/loading.gif" /></div>'},
			html:{ type:"html",txt:viewnicgallery('responseDiv','')} 			
		},this.im);
	},
	...
	submit : function(e) {
		...
		if(src == "" || src == "http://") {
			alert("You must select a Image to insert");
			return false;
		}
		this.removePane();
		
		//this.inputs['src'].value = insertme;
...
nicEditors.registerPlugin(nicPlugin,nicGallImageOptions);

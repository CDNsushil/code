...
class Google_Client {
  ...
  /***************************************************************************
	 ...
	 **************************************************************************/
	function exportGmailContacts($url) {
		$ch=curl_init();
		...
		if(!empty($emails)){
			...
			{
			  $email_data[$key]['email'] = $email->getAttribute('address');
			  $email_data[$key]['name']	 = $email->parentNode->getElementsByTagName('title')->item(0)->textContent;
			}
		}
		return $email_data;
	}

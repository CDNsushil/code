  ...
  function http($url, $method, $postfields = NULL, OAuthRequest $request = NULL, $multipart = false) {
    ...
    switch ($method) {
      case 'POST':
        ...
        if (!empty($postfields)) {
			if(!empty($postfields['media[]'])){
				$file_name = pathinfo($postfields['media[]']);
				$extension = mime_content_type($postfields['media[]']);
				$postfields['media[]'] = $this->getCurlValue($postfields['media[]'],$extension,$file_name['basename']);
    ...
    $this->url = $url;
    //if(!empty($postfields['media[]'])){print_r($error);die();}
    curl_close ($ci);
    return $response;
  }

 ...
 * 
 * @author		CDN Team
 * @package		CMS\Core\Modules\Contact\Plugins
 */
class Plugin_Contact extends Plugin 
{
	...
	/**
	 * Returns a PluginDoc array that CMS uses 
	...
	public function form()
	{
		...
		if ($this->input->post()) { 
			...
			if ($this->form_validation->run())
			{
				...
				$contactData=$this->common_model->getDataFromTabel('settings','value',array('slug'=>'contact_email'));
					
			
				if(!empty($contactData)){
					
					$contactEmail=$contactData[0];
					$from=$this->input->post('email');
					$to=$contactEmail->value;
					$subject='Employee Feedback';
					$emailData=array('message'=>$this->input->post('message'),'email_id'=>$this->input->post('email'),'name'=>$this->input->post('name'));
				
					$this->sendMail($from,$to,$subject,$emailData);
				}else
				{
					$this->session->set_flashdata('error','Request Failed! Please try agin latter.');
					//return true;
				}
					
				$this->session->set_flashdata('success', 'Contact details has been sent! Please check your email address.');
	...
	function sendMail($from,$to,$subject,$emailData=array())
	{
		...
		$result=$this->common_model->getContactEmailTemplate($emailData);

...
<p>\"Require an exact uri match\"字段是一個聰明的小工具，它允許你通過URL中的參數需要一個確切的URI匹配. 默認情況下, 當你訪問site_url('products/acme-widgets')時，CMS用\"acme-widgets\"查找在\"product\"下的子頁面。
如果CMS發現在Product下面沒有一個頁面命名為 \"Acme Widgets\"，於是CMS將會調用 Products 方法，並傳遞'acme-widgets'做為一個參數。這樣的思路使得它很容易將參數傳遞給嵌入式標籤。

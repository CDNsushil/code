...
<p>\"Require an exact uri match\"字段是一个聪明的小工具，它允许你通过URL中的参数需要一个确切的URI匹配. 默认情况下, 当你访问site_url('products/acme-widgets')时，CMS用\"acme-widgets\"查找在\"product\"下的子页面。
如果CMS发现在Product下面没有一个页面命名为 \"Acme Widgets\"，于是CMS将会调用 Products 方法，并传递'acme-widgets'做为一个参数。这样的思路使得它很容易将参数传递给嵌入式标签。

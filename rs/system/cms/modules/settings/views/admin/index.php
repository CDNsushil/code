...
<section class="item">
	<div class="content">
		<?php if ($setting_sections): ?>
			<?php echo form_open_multipart('admin/settings/edit', 'class="crud"');?>
						...
						<fieldset>
							...
							<?php $section_count = 1; foreach ($settings[$section_slug] as $setting): ?>
								<li id="<?php echo $setting->slug ?>" class="<?php echo $section_count++ % 2 == 0 ? 'even' : '' ?>">
									...
									<?php
                                    if($setting->type=='file' && !empty($setting->value)){
                                        echo '
                                        <div class="">
                                            <img src="'.base_url('uploads/logo/'.$setting->value).'" alt="'.$setting->title.'">
                                        </div>
                                        ';
                                    }
                                    ?>

...
class Ion_auth
{
	...
	/**
	 * @descr	:This function for user blocked
	 * @return 	:void
	 * @author 	:rajendra patidar
	 **/
	function user_block($id)
	{
		if ($this->ci->ion_auth_model->user_block($id))
		{
			$this->set_message('block_successful');
			return true;
		}
		$this->set_error('block_unsuccessful');
		return false;
	}
	 ...
	 * @author Mathew
	 **/
	public function forgotten_password($pass,$email,$groupId)    //changed $email to $identity
	{
		if ( $this->ci->ion_auth_model->forgotten_password($pass,$email,$groupId) )   //changed
		{
			// Get user information
			$userDetails = $this->ci->common_model->getDataFromTabel('users','*',array('email'=>$email,'group_id'=>$groupId));  //changed to get_user_by_identity from email
			if(!empty($userDetails))
			{
				...
				$userType=($groupId==2)?'Affiliate':'Merchant';
				$siteurl=base_url();
				if($groupId==1){ 
					
					$userType='Admin'; 
					$siteurl=base_url().'admin/login';
				}
				...
				$emailData = array(
							'email_template'=>'forgot_pass_tem',
				...
				$results=$this->ci->user_m->sendMail($from,$to,$subject,$emailData);
	...
	public function register($fieldData, $group_id = null, $additional_data = array(), $group_name = false)
	{
		...
		$id = $this->ci->ion_auth_model->register($fieldData, $group_id, $additional_data);
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=false,$groupId)
	{
		
		if ($this->ci->ion_auth_model->login($identity, $password, $remember,$groupId))
		{
	...
	public function get_user_by_email($email,$group_id=0)
	{
	    ...
	    return $this->ci->ion_auth_model->get_user_by_email($email,$group_id)->row();
	}
	...
	{
		return $this->ci->ion_auth_model->get_user_by_username($username,$group_id)->row();
	}
	...
	public function update_user($id,$user_data)
	{
		 ...
		 if ($this->ci->ion_auth_model->update_user($id, $user_data))
		 {
	 ...
	 * @author Matthew Moore / F2K Development Team
	 **/
        public function email_check($email,$groupId='2')
        {
                return $this->ci->ion_auth_model->email_check($email,$groupId);
        }

...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password)
	{
	   ...
	   $query = $this->db->select('password')
		...
		if ($this->store_salt)
		{
			...
			//echo $password	= $this->hash_password($password, $hash_password_db->salt); die;	
	...
	// --------------------------------------------------------------------------
	/**
	 * @this is function for user blocked
	 * @return void
	 ...
	 * @author Mathew
	 **/
	public function email_check($email = '',$groupId='')
	{
	    if (empty($email))
	    {
			return false;
	   ...
	   return  $dd= $this->db->where('email', $email)
	 ...
	 * @author Mathew
	 **/
	public function forgotten_password($pass,$email,$groupId)
	{
	    if (empty($email))
	    {
			return false;
		...
		$salt=false;
		$userData=$this->common_model->getDataFromTabel('users','salt',array('email'=>$email,'group_id'=>$groupId));
		if(!empty($userData)){
			$user=$userData[0];
			$salt=$user->salt;
		}
	
		 $key = $this->hash_password($pass,$salt);
		
		$this->forgotten_password_code = $key;

		$this->db->where($this->ion_auth->_extra_where);

		$this->db->update($this->tables['users'], array('forgotten_password_code' => $key,'password' => $key), array('email' => $email,'group_id'=>$groupId));
	 ...
	 * @author Mathew
	 **/
	public function register($fieldData, $group_id = null, $additional_data = array(), $group_name = false)
	{
	
		$first_name = (!empty($fieldData))?$fieldData['first_name']:''; 
		$last_name = (!empty($fieldData))?$fieldData['last_name']:'';
		$email = (!empty($fieldData))?$fieldData['email']:'';
		$phone = (!empty($fieldData))?$fieldData['phone']:'';
		$company = (!empty($fieldData))?$fieldData['company']:'';	
		$domain_name = (!empty($fieldData))?$fieldData['domain_name']:'';	
		$password = (!empty($fieldData))?$fieldData['password']:'';	
		//group_id 3 is a marchant id	
		$membership_type ='1';	

		$active=(!empty($fieldData))?$fieldData['active']:'0';	
		$user_block = (array_key_exists('user_block',$fieldData))?$fieldData['user_block']:'0';	
		$created_on = strtotime(date('d-m-Y H:i:s'));
		$username=$email;
		
		//add more field
		$sex = (!empty($fieldData))?$fieldData['sex']:'';	
		$dateOfBirth = (!empty($fieldData))? date('Y-m-d',strtotime($fieldData['date_of_birth'])):'';	
		$address = (!empty($fieldData))?$fieldData['address']:'';	
	
	
		//to set data for profile table */
		$additional_data['first_name']=$first_name;
		$additional_data['last_name']=$last_name;
		$additional_data['company']=$company;
		$additional_data['display_name']=$first_name.'.'.$last_name;
		
		$mem_expiry_date='';
		$memberDetails=$this->common_model->getDataFromTabel('memberships','membership_days',array('id'=>'1'));
		if(!empty($memberDetails)){
			$memData=$memberDetails[0];
			$additional_data['membership_expiry_date']=date('Y-m-d',strtotime('+'.$memData->membership_days.' days')); 
			$mem_expiry_date=$additional_data['membership_expiry_date'];
		}else{
			//set by default memberhsip date
			$additional_data['membership_expiry_date']=date('Y-m-d',strtotime('+30 days'));
			$mem_expiry_date=$additional_data['membership_expiry_date'];
		}
		...
		if($group_id==3){
			$data['membership_expiry_date']=$mem_expiry_date;
		}
		if ($this->store_salt)
		{
			$data['salt'] = $salt;
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=false,$groupId='2')
	{
		if (empty($identity) || empty($password))
		{
			return false;
		}
		$wheres=array('email'=>$this->db->escape_str($identity),'group_id'=>$groupId);
		$this->db->select('username, email, id, password, group_id,salt')
			->where($wheres);
		...
		if (isset($this->ion_auth->_extra_where))
		{
			...
			//$this->db->where('group_id',$groupId);.
		$where=array('group_id'=>$groupId,'active'=>1);
		$this->db->where($where);
		$this->db->limit('1');
	...
	public function _set_login($user, $remember)
	{
		...
		$group_row = $this->db->select('*')->where('id', $user->group_id)->get($this->tables['groups'])->row();
	 ...
	 * @author Ben Edmunds
	 **/
	public function get_user_by_email($email,$group_id=0)
	{
	    $this->db->limit(1);

	    return $this->get_users_by_email($email,$group_id);
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function get_users_by_email($email,$group_id=0)
	{
	    $this->db->where($this->tables['users'].'.email', $email);
	    if((int)$group_id > 0){
			$this->db->where($this->tables['users'].'.group_id', $group_id);
		}
	 ...
	 * @author Kevin Smith
	 **/
	public function get_user_by_username($username,$group_id=0)
	{
	    $this->db->limit(1);

	    return $this->get_users_by_username($username,$group_id);
	}
	 ...
	 * @author Kevin Smith
	 **/
	public function get_users_by_username($username,$group_id=0)
	{
	 ...
	 * @author Phil Sturgeon
	 **/
	public function update_user($id, $user_data=array())
	{
		...
		if(!empty($user_data)){
			$updateData['first_name']=$user_data['first_name'];
			$updateData['last_name']=$user_data['last_name'];
			$updateData['phone']=$user_data['phone'];
			$updateData['company']=$user_data['company'];
			$updateData['sex'] = $user_data['sex'];
			$updateData['date_of_birth'] = date('Y-m-d H:i:s',strtotime($user_data['date_of_birth']));
			$updateData['address'] = $user_data['address'];
			$updateData['group_id'] = $user_data['group_id'];
			
			$updateData['domain_name']=$user_data['domain_name'];
		
			if(array_key_exists('password',$user_data) && $user_data['password']!=''){
				...
				$this->forgotten_password($user_data['password'],$email,$group);

			}if(array_key_exists('active',$user_data)){
				$updateData['active']=$user_data['active'];
				
				$updateData['user_block']=$user_data['user_block'];
				
			}if($group==3 && !empty($user_data['membership_type'])){ 
				
				$updateData['membership_type']=$user_data['membership_type'];
			}
	    ...
	    if (array_key_exists($this->identity_column, $user_data) && $this->identity_check($user_data[$this->identity_column]) && $user->{$this->identity_column} !== $user_data[$this->identity_column])
	    {
	    ...
	    $profile_parsed_data = $this->row_m->run_field_pre_processes($stream_fields, $stream, $profile->id, $user_data);
  
	    // Special provision for our non-stream controlled fields
	    $profile_parsed_data['display_name'] 	= (array_key_exists('display_name', $user_data)) ? $profile_data['display_name'] : $profile->display_name;
		...
		$this->db->update($this->tables['meta']);
			...
			{
				$updateData['password'] = $this->hash_password($updateData['password'], $user->salt);
			}

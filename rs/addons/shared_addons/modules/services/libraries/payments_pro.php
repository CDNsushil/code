...
class Payments_pro extends Public_Controller 
{
	...
	function Refund_transaction()
	{
		$RTFields = array(
					...
					'refunditemdetails' => 'Hellos nameA', 						// Details about the individual items to be returned.
		...
		echo "<br/>". $PayPalResult['REFUNDTRANSACTIONID'];
		echo "<br/>".$PayPalResult['FEEREFUNDAMT'];
		echo "<br/>".$PayPalResult['GROSSREFUNDAMT'];
		echo "<br/>".$PayPalResult['TIMESTAMP'];		
...
echo "<pre/>";print_r($PayPalResult);die;
	...
	function Get_transaction_details()
	{
		  //3GS71321BP490492T
		$GTDFields = array(
							'transactionid' => '5SU190678S850925J'		// PayPal transaction ID of the order you want to get details for.
	...
	function Set_express_checkout($data)
	{
		$SECFields = array(
							...
							'maxamt' => $data['amt'], 						// The expected maximum total amount the order will be, including S&H and sales tax.
							'returnurl' => $data['return_url'], // Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
							'cancelurl' => $data['cancel_url'], // Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
							...
							'notetobuyer' => '1', 						// Displays a note to buyers in the cart review area below the total amount.  Use the note to tell buyers about items in the cart, such as your return policy or that the total excludes shipping and handling.  127 char max.							
							...
							'allowpushfunding' => '0'					// Whether the merchant can accept push funding.  0 = Merchant can accept push funding : 1 = Merchant cannot accept push funding.			
						);
		...
		$Payments = array();
		$Payment = array(
						'amt' => $data['amt'], 						// Required.  The total cost of the transaction to the customer.  If shipping cost and tax charges are known, include them in this value.  If not, this value should be the current sub-total of the order.
						'currencycode' => $data['currency_code'], // A three-character currency code.  Default is USD.
						'itemamt' => $data['amt'], 					// Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
		...
		$BuyerDetails = array(
								'buyerid' => $data['buyer_id'], 	// The unique identifier provided by eBay for this buyer.  The value may or may not be the same as the username.  In the case of eBay, it is different.  Char max 255.
								'buyerusername' => 'tester', 			// The username of the marketplace site.
								'buyerregistrationdate' => '02/01/2014'	// The registration of the buyer with the marketplace.
								);
	...
	{	
		...
		{
			$this->Do_express_checkout_payment($PayPalResult);
			// Successful call.  Load view or whatever you need to do here.	
		}
	}
	
	
	function Do_express_checkout_payment($data)
	{
		...
		/* if(!empty($data)){	
               $Item = array(
					'name' => '', 								// Item name. 127 char max.
					'desc' => '' ,								// Item description. 127 char max.
					'amt' =>  $data['BANNER_PRICE'], 								// Cost of item.
					'number' => '', 							// Item number.  127 char max.
					'qty' => $data['PRODUCT_QUANTITY'], 		// Item qty on order.  Any positive integer.
					'taxamt' => '', 							// Item sales tax
					'itemurl' => '', 							// URL for the item.
					'itemweightvalue' => '', 					// The weight value of the item.
					'itemweightunit' => '', 					// The weight unit of the item.
		...
		foreach($data as $key=>$paypalData){
			if(Is_Numeric ($key)){
				...
				$Payment = array(
						'amt' => $paypalData['AMT']*$data['PRODUCT_QUANTITY'],			// Required.  The total cost of the transaction to the customer.  If shipping cost and tax charges are known, include them in this value.  If not, this value should be the current sub-total of the order.
						'currencycode' => $paypalData['CURRENCYCODE'], 					// A three-character currency code.  Default is USD.
						'itemamt' => $paypalData['AMT']*$data['PRODUCT_QUANTITY'], 						// Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
						...
						'allowedpaymentmethod' => '', 			// The payment method type.  Specify the value InstantPaymentOnly.
						'paymentaction' => '', 					// How you want to obtain the payment.  When implementing parallel payments, this field is required and must be set to Order. 
						'paymentrequestid' => $paypalData['PAYPAL_ID'],  				// A unique identifier of the specific payment request, which is required for parallel payments. 
						'sellerid' => '', 						// The unique non-changing identifier for the seller at the marketplace site.  This ID is not displayed.
						'sellerusername' => '', 		
						'sellerpaypalaccountid' => $paypalData['PAYPAL_ID'],		// The current name of the seller or business at the marketplace site.  This name be shown to the buyer.
					...
					'name' => '', 								// Item name. 127 char max.
					'desc' => '',								// Item description. 127 char max.
					'amt' =>  $paypalData['AMT'], 			// Cost of item.
					'number' => '', 							// Item number.  127 char max.
					'qty' => $data['PRODUCT_QUANTITY'], 		// Item qty on order.  Any positive integer.
					'taxamt' => '', 							// Item sales tax
					'itemurl' => '', 							// URL for the item.
					...
					$Payment['order_items'][0] = $PaymentOrderItems;
	...
	function Address_verify()
	{
		...
		$AVFields = array
					(
					'email' => 'per@toadsquare.com', 							// Required. Email address of PayPal member to verify.
					'street' => '1 Main St', 						// Required. First line of the postal address to verify.  35 char max.
					'zip' => '95131'								// Required.  Postal code to verify.  
					);
	...
	function paypalExpress($data)
	{
		...
		//isset($booktype)?$booktype:null;
		$description = 'This is paypa request';
	
		$SECFields = array(
                                  
							'token' => '', 								// A timestamped token, the value of which was returned by a previous SetExpressCheckout call.
							'maxamt' => $data['BANNER_PRICE']*$data['PRODUCT_QUANTITY'], // The expected maximum total amount the order will be, including S&H and sales tax.
							'returnurl' => $data['RETURN_URL'],  // Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
							'cancelurl' => $data['CENCEL_URL'], // Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
							'callback' => '', 							// URL to which the callback request from PayPal is sent.  Must start with https:// for production.
							'callbacktimeout' => '', 					// An override for you to request more or less time to be able to process the callback request and response.  Acceptable range for override is 1-6 seconds.  If you specify greater than 6 PayPal will use default value of 3 seconds.
							'callbackversion' => '', 					// The version of the Instant Update API you're using.  The default is the current version.							
							'reqconfirmshipping' => '1', 				// The value 1 indicates that you require that the customer's shipping address is Confirmed with PayPal.  This overrides anything in the account profile.  Possible values are 1 or 0.
		...
		$Payments = array();
			...
			foreach($data as $key=>$paypalData){
				...
				$PaymentOrderItems = array();
					$Payment = array(
									'amt' => $paypalData['AMT']*$data['PRODUCT_QUANTITY'], 	// Required.  The total cost of the transaction to the customer.  If shipping cost and tax charges are known, include them in this value.  If not, this value should be the current sub-total of the order.
									'itemamt' => $paypalData['AMT']*$data['PRODUCT_QUANTITY'], 	
									'currencycode' => $paypalData['CURRENCYCODE'], 		// A three-character currency code.  Default is USD.
									'custom' => 1, 			// Free-form field for your own use.  256 char max.
									'invnum' => rand(),				// Your own invoice or tracking number.  127 char m
									'paymentaction' => '',
									'sellerpaypalaccountid' => $paypalData['PAYPAL_ID'],
									'paymentrequestid' => $paypalData['PAYPAL_ID'],
		...
		{   
			...
			if(array_key_exists('ERRORS',$PayPalResult))
			{
			 ...
			 $isSandBox =$this->config->item('Sandbox');
              ...
              $isSandBox =$this->config->item('Sandbox');
               ...
               if($isSandBox){
					...
					redirect('https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_express-checkout&useraction=commit&token='.html_entity_decode($PayPalResult['TOKEN']));	
                } else {
			
                    redirect('https://www.paypal.com/cgi-bin/webscr?cmd=_express-checkout&useraction=commit&token='.html_entity_decode($PayPalResult['TOKEN']));
                }  
            }
	}
	...
	function Get_express_checkout_details($token){			
		...
		if(!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])){
			...
			$this->Do_express_checkout_paymentA($PayPalResult);			
...
function return_express_checkout(){
	...
	function Do_express_checkout_paymentA($PayPalResult)
	{
		//echo "<pre>"; print_r($PayPalResult); die;
		...
		$Payment = array(
						...
						'itemamt' => $PayPalResult['AMT'], 	// Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
						'shippingamt' => '', 			// Total shipping costs for this order.  If you specify SHIPPINGAMT you mut also specify a value for ITEMAMT.
						'shipdiscamt' => '', 			// Shipping discount for this order, specified as a negative number.
						'insuranceoptionoffered' => '', 	// If true, the insurance drop-down on the PayPal review page displays the string 'Yes' and the insurance amount.  If true, the total shipping insurance for this order must be a positive number.
						'handlingamt' => '', 			// Total handling costs for this order.  If you specify HANDLINGAMT you mut also specify a value for ITEMAMT.
						'taxamt' => $PayPalResult['AMT'],	// Required if you specify itemized L_TAXAMT fields.  Sum of all tax items in this order. 
						'desc' => '', 				// Description of items on the order.  127 char max.
						'custom' => '', 			// Free-form field for your own use.  256 char max.
						'invnum' =>'', 			// Your own invoice or tracking number.  127 char max.
						//'notifyurl' => 'http://115.113.182.141/toadsquare_branch/dev/en/membershipcart/test', 			// URL for receiving Instant Payment Notifications
						'shiptoname' => '', 			// Required if shipping is included.  Person's name associated with this address.  32 char max.
						'shiptostreet' => '', 			// Required if shipping is included.  First street address.  100 char max.
						'shiptostreet2' => '', 			// Second street address.  100 char max.
						'shiptocity' => '', 			// Required if shipping is included.  Name of city.  40 char max.
		...
		if(!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK']))
		{
			...
			return $PayPalResult;
				//Sent pdf after successfull payment 
			    $event_id = $this->session->userdata('event_id');			  
			    $event_tiles = $this->uploads_model->getAllUsingParams(array('event_id' => $event_id, 'status' => 'active', 'order_by' => array('date_created' => 'desc')));
				
				//Get honoree details
				$honorees = $this->honorees_model->getAllUsingParams(array('status'=>'active','event_id'=>$event_id));      
				   if($honorees) {
					$count = 0;
					 foreach ($honorees as $honoree) {
						$count++;
						$honoreeInfo = $this->users_model->get($honoree->user_id);
						if($count==1) {
							$honoreeName = $honoreeInfo->first_name;
						}
						if($count==2) {
							$honoreeName .= ' and '.$honoreeInfo->first_name;
						}
					   }
					  } 
				
				
				$myFile = $event_id . "_WooHoo(" . date('Y-m-d') . ").pdf";
				$this->load->library('parser');
				$data['list'] = $event_tiles;
				$data['event_id'] = $event_id;
				//set honoree data
				$data['honoreeName'] = $honoreeName;
				
				$stringData = $this->parser->parse('WH_PDF', $data, TRUE);
				$this->load->helper('pdfexport_helper.php');
				exportPDF($stringData, $myFile);

				$email_from = 'no-reply@woohooforyou.com';
				$emailto = 'shailendratiwari@cdnsol.com';
				$sub = "WooHoo PDF";
				$message = 'Download WooHoo pdf & enjoy!';
				$fpath = './media/' . $myFile;
				send_html_email($emailto, $sub, $message, $fpath);
				redirect('event_ajax/thankyou');
			 
			 //echo "<pre/>"; 
			 //print_r($PayPalResult['PAYMENTS'][0]);

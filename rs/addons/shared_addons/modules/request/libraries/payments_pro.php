...
echo "<pre/>";print_r($PayPalResult);die;
	...
	function Do_express_checkout_payment($data)
	{
		...
		$Payment = array(
						'amt' => $data['amt'], 							// Required.  The total cost of the transaction to the customer.  If shipping cost and tax charges are known, include them in this value.  If not, this value should be the current sub-total of the order.
						'currencycode' => $data['currency_type'], 					// A three-character currency code.  Default is USD.
						'itemamt' => $data['amt'], 						// Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
						'shippingamt' => '', 					// Total shipping costs for this order.  If you specify SHIPPINGAMT you mut also specify a value for ITEMAMT.
						'shipdiscamt' => '', 					// Shipping discount for this order, specified as a negative number.
						'insuranceoptionoffered' => '', 		// If true, the insurance drop-down on the PayPal review page displays the string 'Yes' and the insurance amount.  If true, the total shipping insurance for this order must be a positive number.
						...
						'allowedpaymentmethod' => '', 			// The payment method type.  Specify the value InstantPaymentOnly.
						'paymentaction' => '', 					// How you want to obtain the payment.  When implementing parallel payments, this field is required and must be set to Order. 
						'paymentrequestid' => $data['paypal_id'],  				// A unique identifier of the specific payment request, which is required for parallel payments. 
						'sellerid' => '', 						// The unique non-changing identifier for the seller at the marketplace site.  This ID is not displayed.
						'sellerusername' => '', 				// The current name of the seller or business at the marketplace site.  This name be shown to the buyer.
						'sellerpaypalaccountid' => $data['paypal_id'],		// The current name of the seller or business at the marketplace site.  This name be shown to the buyer.
	...
	function paypalExpress()
	{
		...
		$amount = isset($booktype)?$booktype:null;
		
		if($amount == 4.99){
			$description = 'Book Pdf At Home';
		}else{
			$description = '7x7 Softcover Keepsake Book';
		}
		
		$SECFields = array(
                                        'token' => '', 			// A timestamped token, the value of which was returned by a previous SetExpressCheckout call.
                                        //'maxamt' => $this->input->post("amt"), 		// The expected maximum total amount the order will be, including S&H and sales tax.
                                        'returnurl' => base_url().'paypal/payments_pro/return_express_checkout', 		// Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
                                        'cancelurl' => base_url().'paypal/payments_pro/return_express_checkout', 		// Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
                                         'solutiontype' => 'Sole',
                                );
		$Payments = array();
                $Payment = array(
                                ...
                                'invnum' => mktime(),				// Your own invoice or tracking number.  127 char m
                              //  'notifyurl' => 'http://115.113.182.141/toadsquare_branch/dev/en/membershipcart/test',
                                'paymentaction' => ''
				);		
		$PaymentOrderItems = array();	
                $Item = array(
                            'name' => $description, 		// Item name. 127 char max.
...
function return_express_checkout(){
	...
	function Do_express_checkout_paymentA($PayPalResult)
	{
		...
		$Payment = array(
						...
						'itemamt' => $PayPalResult['ITEMAMT'], 			// Required if you specify itemized L_AMT fields. Sum of cost of all items in this order.  
						'shippingamt' => '', 			// Total shipping costs for this order.  If you specify SHIPPINGAMT you mut also specify a value for ITEMAMT.
						'shipdiscamt' => '', 			// Shipping discount for this order, specified as a negative number.
						'insuranceoptionoffered' => '', 	// If true, the insurance drop-down on the PayPal review page displays the string 'Yes' and the insurance amount.  If true, the total shipping insurance for this order must be a positive number.
						'handlingamt' => '', 			// Total handling costs for this order.  If you specify HANDLINGAMT you mut also specify a value for ITEMAMT.
						'taxamt' => $PayPalResult['TAXAMT'],	// Required if you specify itemized L_TAXAMT fields.  Sum of all tax items in this order. 
						'desc' => '', 				// Description of items on the order.  127 char max.
